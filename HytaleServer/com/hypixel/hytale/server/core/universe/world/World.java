...
public class World extends TickingThread implements Executor, ExecutorMetricsRegistry.ExecutorMetric, ChunkAccessor<WorldChunk>, IWorldChunks, IMessageReceiver {
    ...
    @Nonnull
    private final EventRegistry eventRegistry = new EventRegistry(new CopyOnWriteArrayList(), () -> true, (String)null, HytaleServer.get().getEventBus());
    @Nonnull
    ...
    public World(@Nonnull String name, @Nonnull Path savePath, @Nonnull WorldConfig worldConfig) throws IOException {
        ...
        this.playerRefs = Collections.unmodifiableCollection(this.players.values());
        this.features = Collections.synchronizedMap(new EnumMap(ClientFeature.class));
        this.name = name;
        this.logger = HytaleLogger.get("World|" + name);
        this.savePath = savePath;
        this.worldConfig = worldConfig;
        this.logger.at(Level.INFO).log("Loading world '%s' with generator type: '%s' and chunk storage: '%s'...", name, worldConfig.getWorldGenProvider(), worldConfig.getChunkStorageProvider());
    ...
    public CompletableFuture<World> init() {
        ...
        if (this.worldConfig.isSavingConfig()) {
            ...
            savingFuture = CompletableFuture.completedFuture((Object)null);
        }
        ...
        CompletableFuture<World> loadWorldGen = CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return this;
            } catch (WorldGenLoadException var3) {
                ...
                throw new SkipSentryException("Failed to load WorldGen!", var3);
        ...
        return this.worldConfig.getSpawnProvider() != null ? CompletableFuture.allOf(savingFuture, loadPaths).thenApply((v) -> this) : CompletableFuture.allOf(savingFuture, loadPaths).thenCompose((v) -> loadWorldGen);
    }
    ...
    protected void onStart() {
        ...
        if (Files.exists(rffPath, new LinkOption[0])) {
            throw new RuntimeException(String.valueOf(rffPath) + " directory exists but this version of the server doesn't support migrating RFF worlds!");
        } else {
    ...
    public void stopIndividualWorld() {
        ...
        if (this.alive.getAndSet(false)) {
            try {
                ...
                ((HytaleLogger.Api)this.logger.at(Level.SEVERE).withCause(t)).log("Exception while shutting down world:");
            }
        }
    ...
    protected void tick(float dt) {
        if (this.alive.get()) {
            TimeResource worldTimeResource = (TimeResource)this.entityStore.getStore().getResource(TimeResource.getResourceType());
    ...
    public void setTps(int tps) {
        ...
        this.entityStore.getStore().forEachEntityParallel(PlayerRef.getComponentType(), (index, archetypeChunk, commandBuffer) -> ((PlayerRef)archetypeChunk.getComponent(index, PlayerRef.getComponentType())).getPacketHandler().writeNoCache(setUpdateRatePacket));
    }
    ...
    public static void setTimeDilation(float timeDilationModifier, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
        ...
        if (!((double)timeDilationModifier <= 0.01) && !(timeDilationModifier > 4.0F)) {
            TimeResource worldTimeResource = (TimeResource)componentAccessor.getResource(TimeResource.getResourceType());
    ...
    public <T> void getBlockBulkRelative(@Nonnull Long2ObjectMap<T> blocks, @Nonnull IntUnaryOperator xConvert, @Nonnull IntUnaryOperator yConvert, @Nonnull IntUnaryOperator zConvert, @Nonnull GenericBlockBulkUpdater<T> consumer) {
        ...
        blocks.forEach((a, b) -> {
            ...
            WorldChunk chunk = (WorldChunk)chunks.get(chunkIndex);
            if (chunk == null) {
                chunk = this.getNonTickingChunk(chunkIndex);
                chunks.put(chunkIndex, chunk);
            }

            consumer.apply(this, b, chunkIndex, chunk, x, y, z, localX, localY, localZ);
        });
    }
    ...
    public WorldChunk loadChunkIfInMemory(long index) {
        if (!this.isInThread()) {
            return (WorldChunk)CompletableFuture.supplyAsync(() -> this.loadChunkIfInMemory(index), this).join();
        } else {
            Ref<ChunkStore> reference = this.chunkStore.getChunkReference(index);
            if (reference == null) {
                return null;
            } else {
                WorldChunk worldChunkComponent = (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType());
    ...
    public WorldChunk getChunkIfInMemory(long index) {
        ...
        if (reference == null) {
            ...
            return !this.isInThread() ? (WorldChunk)CompletableFuture.supplyAsync(() -> this.getChunkIfInMemory(index), this).join() : (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType());
        }
    }
    ...
    public WorldChunk getChunkIfLoaded(long index) {
        if (!this.isInThread()) {
            return (WorldChunk)CompletableFuture.supplyAsync(() -> this.getChunkIfLoaded(index), this).join();
        } else {
            Ref<ChunkStore> reference = this.chunkStore.getChunkReference(index);
            if (reference == null) {
                return null;
    ...
    public WorldChunk getChunkIfNonTicking(long index) {
        if (!this.isInThread()) {
            return (WorldChunk)CompletableFuture.supplyAsync(() -> this.getChunkIfNonTicking(index), this).join();
        } else {
            Ref<ChunkStore> reference = this.chunkStore.getChunkReference(index);
            if (reference == null) {
                return null;
    ...
    public CompletableFuture<WorldChunk> getChunkAsync(long index) {
        return this.chunkStore.getChunkReferenceAsync(index, 4).thenApplyAsync((reference) -> reference == null ? null : (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType()), this);
    }

    @Nonnull
    public CompletableFuture<WorldChunk> getNonTickingChunkAsync(long index) {
        return this.chunkStore.getChunkReferenceAsync(index).thenApplyAsync((reference) -> reference == null ? null : (WorldChunk)this.chunkStore.getStore().getComponent(reference, WorldChunk.getComponentType()), this);
    }
    ...
    public List<Player> getPlayers() {
        if (!this.isInThread()) {
            return !this.isStarted() ? Collections.emptyList() : (List)CompletableFuture.supplyAsync(this::getPlayers, this).join();
        } else {
    ...
    public Entity getEntity(@Nonnull UUID uuid) {
        if (!this.isInThread()) {
            return (Entity)CompletableFuture.supplyAsync(() -> this.getEntity(uuid), this).join();
        } else {
    ...
    public Ref<EntityStore> getEntityRef(@Nonnull UUID uuid) {
        return !this.isInThread() ? (Ref)CompletableFuture.supplyAsync(() -> this.getEntityRef(uuid), this).join() : this.entityStore.getRefFromUUID(uuid);
    }
    ...
    public <T extends Entity> T spawnEntity(T entity, @Nonnull Vector3d position, Vector3f rotation) {
        return (T)this.addEntity(entity, position, rotation, AddReason.SPAWN);
    }
    ...
    public <T extends Entity> T addEntity(T entity, @Nonnull Vector3d position, @Nullable Vector3f rotation, @Nonnull AddReason reason) {
        if (!EntityModule.get().isKnown(entity)) {
            ...
            throw new IllegalStateException("Expected entity to already have its world set to " + var10002 + " but it has " + String.valueOf(entity.getWorld()));
        } else if (entity.getReference() != null && entity.getReference().isValid()) {
            throw new IllegalArgumentException("Entity already has a valid EntityReference: " + String.valueOf(entity.getReference()));
        } else if (position.getY() < (double)-32.0F) {
            throw new IllegalArgumentException("Unable to spawn entity below the world! -32 < " + String.valueOf(position));
            ...
            HeadRotation headRotation = (HeadRotation)holder.ensureAndGetComponent(HeadRotation.getComponentType());
            if (rotation != null) {
    ...
    public CompletableFuture<PlayerRef> addPlayer(@Nonnull PlayerRef playerRef) {
        return this.addPlayer(playerRef, (Transform)null);
    }

    @Nullable
    public CompletableFuture<PlayerRef> addPlayer(@Nonnull PlayerRef playerRef, @Nullable Transform transform) {
        return this.addPlayer(playerRef, transform, (Boolean)null, (Boolean)null);
    }

    @Nullable
    public CompletableFuture<PlayerRef> addPlayer(@Nonnull PlayerRef playerRef, @Deprecated(forRemoval = true) @Nullable Transform transform, @Nullable Boolean clearWorldOverride, @Nullable Boolean fadeInOutOverride) {
        if (!this.alive.get()) {
            ...
            if (!packetHandler.stillActive()) {
                ...
                AddPlayerToWorldEvent event = (AddPlayerToWorldEvent)HytaleServer.get().getEventBus().dispatchFor(AddPlayerToWorldEvent.class, this.name).dispatch(new AddPlayerToWorldEvent(holder, this));
                ChunkTracker chunkTrackerComponent = (ChunkTracker)holder.getComponent(ChunkTracker.getComponentType());
                ...
                CompletableFuture<Void> loadTargetChunkFuture = this.chunkStore.getChunkReferenceAsync(chunkIndex).thenAccept((v) -> playerComponent.startClientReadyTimeout());
                ...
                return CompletableFuture.allOf(setupPlayerFuture, playerReadyFuture, loadTargetChunkFuture).thenApplyAsync((aVoid) -> this.onFinishPlayerJoining(playerComponent, playerRef, packetHandler, event.shouldBroadcastJoinMessage()), this).exceptionally((throwable) -> {
    ...
    private PlayerRef onFinishPlayerJoining(@Nonnull Player playerComponent, @Nonnull PlayerRef playerRefComponent, @Nonnull PacketHandler packetHandler, boolean broadcastJoin) {
        TimeResource timeResource = (TimeResource)this.entityStore.getStore().getResource(TimeResource.getResourceType());
        ...
        packetHandler.write(new Packet[]{new ViewRadius(maxViewRadius * 32), new SetEntitySeed(this.entitySeed.get()), new SetClientId(playerComponent.getNetworkId()), new SetTimeDilation(timeDilationModifier)});
    ...
    private void onSetupPlayerJoining(@Nonnull Holder<EntityStore> holder, @Nonnull Player playerComponent, @Nonnull PlayerRef playerRefComponent, @Nonnull PacketHandler packetHandler, @Nullable Transform transform, boolean clearWorld, boolean fadeInOut) {
        UUID playerUuid = playerRefComponent.getUuid();
        this.logger.at(Level.INFO).log("Adding player '%s' to world '%s' at location %s (%s)", playerRefComponent.getUsername(), this.name, transform, playerUuid);
    ...
    public CompletableFuture<Void> drainPlayersTo(@Nonnull World fallbackTargetWorld) {
        return CompletableFuture.completedFuture((Void)null).thenComposeAsync((aVoid) -> {
            ...
            for(PlayerRef playerRef : this.playerRefs) {
                ...
                DrainPlayerFromWorldEvent event = (DrainPlayerFromWorldEvent)HytaleServer.get().getEventBus().dispatchFor(DrainPlayerFromWorldEvent.class, this.name).dispatch(new DrainPlayerFromWorldEvent(holder, fallbackTargetWorld, (Transform)null));
    ...
    public boolean isFeatureEnabled(@Nonnull ClientFeature feature) {
        return (Boolean)this.features.getOrDefault(feature, false);
    }
    ...
    public String toString() {
        ...
        return "World{name='" + var10000 + "', alive=" + this.alive.get() + ", loadedChunksCount=" + this.chunkStore.getLoadedChunksCount() + ", totalLoadedChunksCount=" + this.chunkStore.getTotalLoadedChunksCount() + ", totalGeneratedChunksCount=" + this.chunkStore.getTotalGeneratedChunksCount() + ", entityCount=" + this.entityStore.getStore().getEntityCount() + "}";
    }
    ...
    public void validate(@Nonnull StringBuilder errors, @Nonnull IPrefabBuffer.RawBlockConsumer<Void> blockValidator, @Nonnull EnumSet<ValidationOption> options) throws IOException {
        ...
        while(var6.hasNext()) {
            ...
            try {
                ...
                if (reference != null && reference.isValid()) {
                    WorldChunk chunk = (WorldChunk)store.getComponent(reference, WorldChunk.getComponentType());
                    ChunkColumn chunkColumn = (ChunkColumn)store.getComponent(reference, ChunkColumn.getComponentType());
                    if (chunkColumn != null) {
                        for(Ref<ChunkStore> section : chunkColumn.getSections()) {
                            final ChunkSection sectionInfo = (ChunkSection)store.getComponent(section, ChunkSection.getComponentType());
                            BlockSection blockSection = (BlockSection)store.getComponent(section, BlockSection.getComponentType());
                            if (blockSection != null) {
                                BlockPhysics blockPhys = (BlockPhysics)store.getComponent(section, BlockPhysics.getComponentType());
                                ...
                                for(int y = 0; y < 32; ++y) {
                                    ...
                                    for(int z = 0; z < 32; ++z) {
                                        for(int x = 0; x < 32; ++x) {
                                            ...
                                            blockValidator.accept(worldX, y, worldZ, blockId, 0, 1.0F, holder, blockPhys != null ? blockPhys.get(x, y, z) : 0, rotation, filler, (Object)null);
                                            if (options.contains(ValidationOption.BLOCK_FILLER)) {
                                                <undefinedtype> fetcher = new FillerBlockUtil.FillerFetcher<BlockSection, ChunkStore>() {
                                                    public int getBlock(BlockSection blockSection, ChunkStore chunkStore, int x, int y, int z) {
                                                        if (x >= 0 && y >= 0 && z >= 0 && x < 32 && y < 32 && z < 32) {
                                                            ...
                                                            int nx = sectionInfo.getX() + ChunkUtil.chunkCoordinate(x);
                                                            int ny = sectionInfo.getY() + ChunkUtil.chunkCoordinate(y);
                                                            int nz = sectionInfo.getZ() + ChunkUtil.chunkCoordinate(z);
                                                ...
                                                FillerBlockUtil.ValidationResult fillerResult = FillerBlockUtil.validateBlock(x, y, z, blockId, rotation, filler, blockSection, this.chunkStore, fetcher);
                                                switch (fillerResult) {
                                                    case OK:
                                                    default:
                                                        break;
                                                    case INVALID_BLOCK:
                                                        ...
                                                        tempBuilder.append("\tBlock ").append(blockType != null ? blockType.getId() : "<missing>").append(" at ").append(x).append(", ").append(y).append(", ").append(z).append(" is not valid filler").append('\n');
                                                        break;
                                                    case INVALID_FILLER:
                                                        BlockType blockType = (BlockType)BlockType.getAssetMap().getAsset(blockId);
                                                        tempBuilder.append("\tBlock ").append(blockType != null ? blockType.getId() : "<missing>").append(" at ").append(x).append(", ").append(y).append(", ").append(z).append(" has invalid/missing filler blocks").append('\n');
                                                }
                                            }
                                        }
                                    }
                                ...
                                if (!tempBuilder.isEmpty()) {
                                    errors.append("\tChunk ").append(sectionInfo.getX()).append(", ").append(sectionInfo.getY()).append(", ").append(sectionInfo.getZ()).append(" validation errors:").append(tempBuilder);
                ...
                ((HytaleLogger.Api)this.getLogger().at(Level.SEVERE).withCause(e)).log("Failed to validate chunk: %d, %d", chunkX, chunkZ);
                errors.append('\t').append("Exception validating chunk: ").append(chunkX).append(", ").append(chunkZ).append('\n').append("\t\t").append(e.getCause().getMessage()).append('\n');
            }
        }
        ...
        this.setThread((Thread)null);
    }

    static {
        METRICS_REGISTRY = (new ExecutorMetricsRegistry()).register("Name", (world) -> world.name, Codec.STRING).register("Alive", (world) -> world.alive.get(), Codec.BOOLEAN).register("TickLength", TickingThread::getBufferedTickLengthMetricSet, HistoricMetric.METRICS_CODEC).register("EntityStore", World::getEntityStore, EntityStore.METRICS_REGISTRY).register("ChunkStore", World::getChunkStore, ChunkStore.METRICS_REGISTRY);
    }
