...
/*     */     
/* 471 */     return CompletableFuture.runAsync(() -> { timeProbe.start(); for (int x_voxelGrid = 0; x_voxelGrid < 32; x_voxelGrid++) { for (int z_voxelGrid = 0; z_voxelGrid < 32; z_voxelGrid++) { int minY_voxelGrid = 0; int maxY_voxelGrid = 320; for (int y_voxelGrid = 0; y_voxelGrid < 320; y_voxelGrid++) { int worldX_voxelGrid = x_voxelGrid + chunkBounds_voxelGrid.min.x; int worldY_voxelGrid = y_voxelGrid + chunkBounds_voxelGrid.min.y; int worldZ_voxelGrid = z_voxelGrid + chunkBounds_voxelGrid.min.z; Integer environment = (Integer)environmentVoxelSpace.getContent(worldX_voxelGrid, worldY_voxelGrid, worldZ_voxelGrid); blockChunk.setEnvironment(x_voxelGrid, y_voxelGrid, z_voxelGrid, ((Integer)Objects.requireNonNullElse(environment, Integer.valueOf(0))).intValue()); }  }  }  timeProbe.stop(); }this.concurrentExecutor)
/*     */ 
/*     */ 
/*     */ 
/*     */ 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\com\hypixel\hytale\builtin\hytalegenerator\newsystem\NStagedChunkGenerator.class
