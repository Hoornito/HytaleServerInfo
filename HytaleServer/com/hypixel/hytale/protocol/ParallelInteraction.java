...
/*     */   
/*     */   public ParallelInteraction(@Nonnull WaitForDataFrom waitForDataFrom, @Nullable InteractionEffects effects, float horizontalSpeedMultiplier, float runTime, boolean cancelOnItemChange, @Nullable Map<GameMode, InteractionSettings> settings, @Nullable InteractionRules rules, @Nullable int[] tags, @Nullable InteractionCameraSettings camera, @Nullable int[] next) {
...
/*     */   }
/*     */   
/*     */   public ParallelInteraction(@Nonnull ParallelInteraction other) {
...
/*     */   
/*     */   @Nonnull
/*     */   public static ParallelInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
/*  53 */     ParallelInteraction obj = new ParallelInteraction();
...
/* 103 */       int nextCount = VarInt.peek(buf, varPos5);
/* 104 */       if (nextCount < 0) throw ProtocolException.negativeLength("Next", nextCount); 
/* 105 */       if (nextCount > 4096000) throw ProtocolException.arrayTooLong("Next", nextCount, 4096000); 
/* 106 */       int varIntLen = VarInt.length(buf, varPos5);
/* 107 */       if ((varPos5 + varIntLen) + nextCount * 4L > buf.readableBytes())
/* 108 */         throw ProtocolException.bufferTooSmall("Next", varPos5 + varIntLen + nextCount * 4, buf.readableBytes()); 
/* 109 */       obj.next = new int[nextCount];
/* 110 */       for (int i = 0; i < nextCount; i++) {
/* 111 */         obj.next[i] = buf.getIntLE(varPos5 + varIntLen + i * 4);
/*     */       }
/*     */     } 
/*     */     
/* 115 */     return obj;
...
/* 171 */     if (this.next != null) nullBits = (byte)(nullBits | 0x20); 
...
/* 189 */     int nextOffsetSlot = buf.writerIndex();
...
/*     */     } 
/* 223 */     if (this.next != null) {
/* 224 */       buf.setIntLE(nextOffsetSlot, buf.writerIndex() - varBlockStart);
/* 225 */       if (this.next.length > 4096000) throw ProtocolException.arrayTooLong("Next", this.next.length, 4096000);  VarInt.write(buf, this.next.length); for (int item : this.next) buf.writeIntLE(item); 
/*     */     } else {
/* 227 */       buf.setIntLE(nextOffsetSlot, -1);
/*     */     } 
...
/* 241 */     if (this.next != null) size += VarInt.size(this.next.length) + this.next.length * 4;
/*     */     
/* 243 */     return size;
/*     */   }
/*     */   
...
/* 351 */       int nextOffset = buffer.getIntLE(offset + 31);
/* 352 */       if (nextOffset < 0) {
/* 353 */         return ValidationResult.error("Invalid offset for Next");
/*     */       }
/* 355 */       int pos = offset + 35 + nextOffset;
/* 356 */       if (pos >= buffer.writerIndex()) {
/* 357 */         return ValidationResult.error("Offset out of bounds for Next");
/*     */       }
/* 359 */       int nextCount = VarInt.peek(buffer, pos);
/* 360 */       if (nextCount < 0) {
/* 361 */         return ValidationResult.error("Invalid array count for Next");
/*     */       }
/* 363 */       if (nextCount > 4096000) {
/* 364 */         return ValidationResult.error("Next exceeds max length 4096000");
/*     */       }
/* 366 */       pos += VarInt.length(buffer, pos);
/* 367 */       pos += nextCount * 4;
/* 368 */       if (pos > buffer.writerIndex()) {
/* 369 */         return ValidationResult.error("Buffer overflow reading Next");
/*     */       }
/*     */     } 
/* 372 */     return ValidationResult.OK;
/*     */   }
/*     */   
/*     */   public ParallelInteraction clone() {
/* 376 */     ParallelInteraction copy = new ParallelInteraction();
...
/* 390 */     copy.next = (this.next != null) ? Arrays.copyOf(this.next, this.next.length) : null;
/* 391 */     return copy;
/*     */   }
/*     */ 
/*     */   
/*     */   public boolean equals(Object obj) {
/*     */     ParallelInteraction other;
/* 397 */     if (this == obj) return true; 
/* 398 */     if (obj instanceof ParallelInteraction) { other = (ParallelInteraction)obj; } else { return false; }
...
/* 414 */     result = 31 * result + Arrays.hashCode(this.next);
/* 415 */     return result;
/*     */   }
/*     */ }


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\com\hypixel\hytale\protocol\ParallelInteraction.class
