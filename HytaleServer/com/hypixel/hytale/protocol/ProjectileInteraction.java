...
/*  52 */     this.configId = other.configId;
/*     */   }
/*     */   
/*     */   @Nonnull
/*     */   public static ProjectileInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
/*  57 */     ProjectileInteraction obj = new ProjectileInteraction();
...
/* 109 */       int configIdLen = VarInt.peek(buf, varPos5);
/* 110 */       if (configIdLen < 0) throw ProtocolException.negativeLength("ConfigId", configIdLen); 
/* 111 */       if (configIdLen > 4096000) throw ProtocolException.stringTooLong("ConfigId", configIdLen, 4096000); 
/* 112 */       obj.configId = PacketIO.readVarString(buf, varPos5, PacketIO.UTF8);
/*     */     } 
/*     */     
/* 115 */     return obj;
/*     */   }
...
/* 171 */     if (this.configId != null) nullBits = (byte)(nullBits | 0x20); 
...
/* 191 */     int configIdOffsetSlot = buf.writerIndex();
...
/*     */     } 
/* 225 */     if (this.configId != null) {
/* 226 */       buf.setIntLE(configIdOffsetSlot, buf.writerIndex() - varBlockStart);
/* 227 */       PacketIO.writeVarString(buf, this.configId, 4096000);
/*     */     } else {
/* 229 */       buf.setIntLE(configIdOffsetSlot, -1);
/*     */     } 
...
/* 243 */     if (this.configId != null) size += PacketIO.stringSize(this.configId);
/*     */     
/* 245 */     return size;
/*     */   }
/*     */   
...
/* 353 */       int configIdOffset = buffer.getIntLE(offset + 39);
/* 354 */       if (configIdOffset < 0) {
/* 355 */         return ValidationResult.error("Invalid offset for ConfigId");
/*     */       }
/* 357 */       int pos = offset + 43 + configIdOffset;
/* 358 */       if (pos >= buffer.writerIndex()) {
/* 359 */         return ValidationResult.error("Offset out of bounds for ConfigId");
/*     */       }
/* 361 */       int configIdLen = VarInt.peek(buffer, pos);
/* 362 */       if (configIdLen < 0) {
/* 363 */         return ValidationResult.error("Invalid string length for ConfigId");
/*     */       }
/* 365 */       if (configIdLen > 4096000) {
/* 366 */         return ValidationResult.error("ConfigId exceeds max length 4096000");
/*     */       }
/* 368 */       pos += VarInt.length(buffer, pos);
/* 369 */       pos += configIdLen;
/* 370 */       if (pos > buffer.writerIndex()) {
/* 371 */         return ValidationResult.error("Buffer overflow reading ConfigId");
/*     */       }
/*     */     } 
/* 374 */     return ValidationResult.OK;
/*     */   }
/*     */   
/*     */   public ProjectileInteraction clone() {
/* 378 */     ProjectileInteraction copy = new ProjectileInteraction();
...
/* 420 */     result = 31 * result + Objects.hashCode(this.configId);
/* 421 */     return result;
/*     */   }
/*     */ }


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\com\hypixel\hytale\protocol\ProjectileInteraction.class
