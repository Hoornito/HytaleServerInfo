...
/*  43 */     RequestAssets obj = new RequestAssets();
...
/*  51 */       if ((pos + assetsVarLen) + assetsCount * 64L > buf.readableBytes())
/*  52 */         throw ProtocolException.bufferTooSmall("Assets", pos + assetsVarLen + assetsCount * 64, buf.readableBytes()); 
/*  53 */       pos += assetsVarLen;
/*  54 */       obj.assets = new Asset[assetsCount];
/*  55 */       for (int i = 0; i < assetsCount; i++) {
/*  56 */         obj.assets[i] = Asset.deserialize(buf, pos);
...
/*     */ 
/*     */     
/*  79 */     if (this.assets != null) { if (this.assets.length > 4096000) throw ProtocolException.arrayTooLong("Assets", this.assets.length, 4096000);  VarInt.write(buf, this.assets.length); for (Asset item : this.assets) item.serialize(buf);  }
/*     */   
/*     */   }
/*     */   
...
/*  86 */       int assetsSize = 0;
/*  87 */       for (Asset elem : this.assets) assetsSize += elem.computeSize(); 
...
/* 115 */           return ValidationResult.error("Invalid Asset in Assets[" + i + "]: " + structResult.error());
/*     */         }
...
/* 124 */     RequestAssets copy = new RequestAssets();
/* 125 */     copy.assets = (this.assets != null) ? (Asset[])Arrays.<Asset>stream(this.assets).map(e -> e.clone()).toArray(x$0 -> new Asset[x$0]) : null;
/* 126 */     return copy;
/*     */   }
/*     */ 
/*     */   
...
/* 133 */     if (obj instanceof RequestAssets) { other = (RequestAssets)obj; } else { return false; }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\com\hypixel\hytale\protocol\packets\setup\RequestAssets.class
