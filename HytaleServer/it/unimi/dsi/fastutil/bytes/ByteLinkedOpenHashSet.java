...
/*  130 */     this.key = new byte[this.n + 1];
...
/*  200 */     this(16, f);
/*  201 */     for (; i.hasNext(); add(i.nextByte()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  221 */     this(ByteIterators.asByteIterator(i), f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  231 */     this(ByteIterators.asByteIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  244 */     ByteArrays.ensureOffsetLength(a, offset, length);
...
/*      */ 
/*      */   
/*      */   public static ByteLinkedOpenHashSet of() {
/*  286 */     return new ByteLinkedOpenHashSet();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*      */   public static ByteLinkedOpenHashSet of(byte e) {
/*  297 */     ByteLinkedOpenHashSet result = new ByteLinkedOpenHashSet(1, 0.75F);
/*  298 */     result.add(e);
/*  299 */     return result;
/*      */   }
/*      */ 
...
/*      */ 
/*      */   
/*      */   public static ByteLinkedOpenHashSet of(byte e0, byte e1) {
/*  313 */     ByteLinkedOpenHashSet result = new ByteLinkedOpenHashSet(2, 0.75F);
/*  314 */     result.add(e0);
...
/*      */ 
/*      */   
/*      */   public static ByteLinkedOpenHashSet of(byte e0, byte e1, byte e2) {
/*  333 */     ByteLinkedOpenHashSet result = new ByteLinkedOpenHashSet(3, 0.75F);
/*  334 */     result.add(e0);
...
/*      */ 
/*      */   
/*      */   public static ByteLinkedOpenHashSet of(byte... a) {
/*  355 */     ByteLinkedOpenHashSet result = new ByteLinkedOpenHashSet(a.length, 0.75F);
/*  356 */     for (byte element : a) {
...
/*  442 */     byte[] key = this.key; while (true) {
/*      */       byte curr; int last;
...
/*  700 */     Arrays.fill(this.key, (byte)0);
...
/*      */     
/*      */     SetIterator() {
/*  871 */       this.next = ByteLinkedOpenHashSet.this.first;
/*  872 */       this.index = 0;
/*      */     }
/*      */     
/*      */     SetIterator(byte from) {
/*  876 */       if (from == 0) {
/*  877 */         if (ByteLinkedOpenHashSet.this.containsNull) {
/*  878 */           this.next = (int)ByteLinkedOpenHashSet.this.link[ByteLinkedOpenHashSet.this.n];
/*  879 */           this.prev = ByteLinkedOpenHashSet.this.n; return;
/*      */         } 
/*  881 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  883 */       if (ByteLinkedOpenHashSet.this.key[ByteLinkedOpenHashSet.this.last] == from) {
/*  884 */         this.prev = ByteLinkedOpenHashSet.this.last;
/*  885 */         this.index = ByteLinkedOpenHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  889 */       byte[] key = ByteLinkedOpenHashSet.this.key;
/*  890 */       int pos = HashCommon.mix(from) & ByteLinkedOpenHashSet.this.mask;
/*      */       
/*  892 */       while (key[pos] != 0) {
/*  893 */         if (key[pos] == from) {
/*      */           
/*  895 */           this.next = (int)ByteLinkedOpenHashSet.this.link[pos];
/*  896 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  899 */         pos = pos + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */       } 
...
/*  918 */       this.next = (int)ByteLinkedOpenHashSet.this.link[this.curr];
/*  919 */       this.prev = this.curr;
/*  920 */       if (this.index >= 0) this.index++;
/*      */       
/*  922 */       return ByteLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
...
/*  929 */       this.prev = (int)(ByteLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*  930 */       this.next = this.curr;
/*  931 */       if (this.index >= 0) this.index--; 
/*  932 */       return ByteLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEachRemaining(ByteConsumer action) {
/*  937 */       byte[] key = ByteLinkedOpenHashSet.this.key;
/*  938 */       long[] link = ByteLinkedOpenHashSet.this.link;
...
/*      */       } 
/*  955 */       if (this.next == -1) {
/*  956 */         this.index = ByteLinkedOpenHashSet.this.size;
/*      */         return;
/*      */       } 
/*  959 */       int pos = ByteLinkedOpenHashSet.this.first;
/*  960 */       this.index = 1;
/*  961 */       while (pos != this.prev) {
/*  962 */         pos = (int)ByteLinkedOpenHashSet.this.link[pos];
/*  963 */         this.index++;
/*      */       } 
/*      */     }
/*      */ 
...
/*      */       { 
/*  986 */         this.index--;
/*  987 */         this.prev = (int)(ByteLinkedOpenHashSet.this.link[this.curr] >>> 32L); }
/*  988 */       else { this.next = (int)ByteLinkedOpenHashSet.this.link[this.curr]; }
/*  989 */        ByteLinkedOpenHashSet.this.size--;
/*      */ 
/*      */       
/*  992 */       if (this.prev == -1) { ByteLinkedOpenHashSet.this.first = this.next; }
/*  993 */       else { ByteLinkedOpenHashSet.this.link[this.prev] = ByteLinkedOpenHashSet.this.link[this.prev] ^ (ByteLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL; }
/*  994 */        if (this.next == -1) { ByteLinkedOpenHashSet.this.last = this.prev; }
/*  995 */       else { ByteLinkedOpenHashSet.this.link[this.next] = ByteLinkedOpenHashSet.this.link[this.next] ^ (ByteLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L; }
/*  996 */        int pos = this.curr;
/*  997 */       this.curr = -1;
/*  998 */       if (pos == ByteLinkedOpenHashSet.this.n) {
/*  999 */         ByteLinkedOpenHashSet.this.containsNull = false;
/* 1000 */         ByteLinkedOpenHashSet.this.key[ByteLinkedOpenHashSet.this.n] = 0;
/*      */       } else {
/*      */         
/* 1003 */         byte[] key = ByteLinkedOpenHashSet.this.key; while (true) {
/*      */           byte curr;
/*      */           int last;
/* 1006 */           pos = (last = pos) + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */           while (true) {
/* 1008 */             if ((curr = key[pos]) == 0) {
/* 1009 */               key[last] = 0;
/*      */               return;
/*      */             } 
/* 1012 */             int slot = HashCommon.mix(curr) & ByteLinkedOpenHashSet.this.mask;
/* 1013 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/* 1014 */               break;  pos = pos + 1 & ByteLinkedOpenHashSet.this.mask;
/*      */           } 
/* 1016 */           key[last] = curr;
/* 1017 */           if (this.next == pos) this.next = last; 
/* 1018 */           if (this.prev == pos) this.prev = last; 
/* 1019 */           ByteLinkedOpenHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1066 */     return ByteSpliterators.asSpliterator(iterator(), Size64.sizeOf(this), 337);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1139 */     byte[] key = this.key;
/* 1140 */     int mask = newN - 1;
/* 1141 */     byte[] newKey = new byte[newN + 1];
...
/* 1223 */     ByteIterator i = iterator();
/* 1224 */     s.defaultWriteObject();
/* 1225 */     for (int j = this.size; j-- != 0; s.writeByte(i.nextByte()));
/*      */   }
/*      */   
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\bytes\ByteLinkedOpenHashSet.class
