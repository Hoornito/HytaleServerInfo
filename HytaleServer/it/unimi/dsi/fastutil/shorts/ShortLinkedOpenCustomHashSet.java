...
/*  134 */     this.key = new short[this.n + 1];
...
/*  213 */     for (; i.hasNext(); add(i.nextShort()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  235 */     this(ShortIterators.asShortIterator(i), f, strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  246 */     this(ShortIterators.asShortIterator(i), strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  260 */     ShortArrays.ensureOffsetLength(a, offset, length);
...
/*      */     int pos;
/*  348 */     if (this.strategy.equals(k, (short)0)) {
...
/*  359 */         while ((curr = key[pos = pos + 1 & this.mask]) != 0) { if (this.strategy.equals(curr, k)) return false;  }
/*      */       
/*  361 */       }  key[pos] = k;
/*      */     } 
/*  363 */     if (this.size == 0) {
...
/*  423 */     if (this.strategy.equals(k, (short)0)) {
...
/*      */     short curr;
/*      */     int pos;
/*  431 */     if ((curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0) return false; 
/*  432 */     if (this.strategy.equals(k, curr)) return removeEntry(pos); 
/*      */     while (true) {
/*  434 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0) return false; 
...
/*  441 */     if (this.strategy.equals(k, (short)0)) return this.containsNull;
/*      */     
/*  443 */     short[] key = this.key;
/*      */     short curr;
/*      */     int pos;
/*  446 */     if ((curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0) return false; 
/*  447 */     if (this.strategy.equals(k, curr)) return true; 
/*      */     while (true) {
/*  449 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0) return false; 
...
/*      */       } }
/*      */     
/*  472 */     short k = this.key[pos];
/*  473 */     this.size--;
/*  474 */     if (this.strategy.equals(k, (short)0))
/*  475 */     { this.containsNull = false;
/*  476 */       this.key[this.n] = 0; }
...
/*      */       } }
/*      */     
/*  500 */     short k = this.key[pos];
/*  501 */     this.size--;
/*  502 */     if (this.strategy.equals(k, (short)0))
/*  503 */     { this.containsNull = false;
/*  504 */       this.key[this.n] = 0; }
...
/*      */     int pos;
/*  555 */     if (this.strategy.equals(k, (short)0)) {
...
/*      */     int pos;
/*  599 */     if (this.strategy.equals(k, (short)0)) {
...
/*  645 */     Arrays.fill(this.key, (short)0);
...
/*      */     
/*      */     SetIterator() {
/*  816 */       this.next = ShortLinkedOpenCustomHashSet.this.first;
/*  817 */       this.index = 0;
/*      */     }
/*      */     
/*      */     SetIterator(short from) {
/*  821 */       if (ShortLinkedOpenCustomHashSet.this.strategy.equals(from, (short)0)) {
/*  822 */         if (ShortLinkedOpenCustomHashSet.this.containsNull) {
/*  823 */           this.next = (int)ShortLinkedOpenCustomHashSet.this.link[ShortLinkedOpenCustomHashSet.this.n];
/*  824 */           this.prev = ShortLinkedOpenCustomHashSet.this.n; return;
/*      */         } 
/*  826 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  828 */       if (ShortLinkedOpenCustomHashSet.this.strategy.equals(ShortLinkedOpenCustomHashSet.this.key[ShortLinkedOpenCustomHashSet.this.last], from)) {
/*  829 */         this.prev = ShortLinkedOpenCustomHashSet.this.last;
/*  830 */         this.index = ShortLinkedOpenCustomHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  834 */       short[] key = ShortLinkedOpenCustomHashSet.this.key;
/*  835 */       int pos = HashCommon.mix(ShortLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & ShortLinkedOpenCustomHashSet.this.mask;
/*      */       
/*  837 */       while (key[pos] != 0) {
/*  838 */         if (ShortLinkedOpenCustomHashSet.this.strategy.equals(key[pos], from)) {
/*      */           
/*  840 */           this.next = (int)ShortLinkedOpenCustomHashSet.this.link[pos];
/*  841 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  844 */         pos = pos + 1 & ShortLinkedOpenCustomHashSet.this.mask;
/*      */       } 
...
/*  863 */       this.next = (int)ShortLinkedOpenCustomHashSet.this.link[this.curr];
/*  864 */       this.prev = this.curr;
/*  865 */       if (this.index >= 0) this.index++;
/*      */       
/*  867 */       return ShortLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
...
/*  874 */       this.prev = (int)(ShortLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L);
/*  875 */       this.next = this.curr;
/*  876 */       if (this.index >= 0) this.index--; 
/*  877 */       return ShortLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEachRemaining(ShortConsumer action) {
/*  882 */       short[] key = ShortLinkedOpenCustomHashSet.this.key;
/*  883 */       long[] link = ShortLinkedOpenCustomHashSet.this.link;
...
/*      */       } 
/*  900 */       if (this.next == -1) {
/*  901 */         this.index = ShortLinkedOpenCustomHashSet.this.size;
/*      */         return;
/*      */       } 
/*  904 */       int pos = ShortLinkedOpenCustomHashSet.this.first;
/*  905 */       this.index = 1;
/*  906 */       while (pos != this.prev) {
/*  907 */         pos = (int)ShortLinkedOpenCustomHashSet.this.link[pos];
/*  908 */         this.index++;
/*      */       } 
/*      */     }
/*      */ 
...
/*      */       { 
/*  931 */         this.index--;
/*  932 */         this.prev = (int)(ShortLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L); }
/*  933 */       else { this.next = (int)ShortLinkedOpenCustomHashSet.this.link[this.curr]; }
/*  934 */        ShortLinkedOpenCustomHashSet.this.size--;
/*      */ 
/*      */       
/*  937 */       if (this.prev == -1) { ShortLinkedOpenCustomHashSet.this.first = this.next; }
/*  938 */       else { ShortLinkedOpenCustomHashSet.this.link[this.prev] = ShortLinkedOpenCustomHashSet.this.link[this.prev] ^ (ShortLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL; }
/*  939 */        if (this.next == -1) { ShortLinkedOpenCustomHashSet.this.last = this.prev; }
/*  940 */       else { ShortLinkedOpenCustomHashSet.this.link[this.next] = ShortLinkedOpenCustomHashSet.this.link[this.next] ^ (ShortLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L; }
/*  941 */        int pos = this.curr;
/*  942 */       this.curr = -1;
/*  943 */       if (pos == ShortLinkedOpenCustomHashSet.this.n) {
/*  944 */         ShortLinkedOpenCustomHashSet.this.containsNull = false;
/*  945 */         ShortLinkedOpenCustomHashSet.this.key[ShortLinkedOpenCustomHashSet.this.n] = 0;
/*      */       } else {
/*      */         
/*  948 */         short[] key = ShortLinkedOpenCustomHashSet.this.key; while (true) {
/*      */           short curr;
/*      */           int last;
/*  951 */           pos = (last = pos) + 1 & ShortLinkedOpenCustomHashSet.this.mask;
/*      */           while (true) {
/*  953 */             if ((curr = key[pos]) == 0) {
/*  954 */               key[last] = 0;
/*      */               return;
/*      */             } 
/*  957 */             int slot = HashCommon.mix(ShortLinkedOpenCustomHashSet.this.strategy.hashCode(curr)) & ShortLinkedOpenCustomHashSet.this.mask;
/*  958 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*  959 */               break;  pos = pos + 1 & ShortLinkedOpenCustomHashSet.this.mask;
/*      */           } 
/*  961 */           key[last] = curr;
/*  962 */           if (this.next == pos) this.next = last; 
/*  963 */           if (this.prev == pos) this.prev = last; 
/*  964 */           ShortLinkedOpenCustomHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1011 */     return ShortSpliterators.asSpliterator(iterator(), Size64.sizeOf(this), 337);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1084 */     short[] key = this.key;
/* 1085 */     int mask = newN - 1;
/* 1086 */     short[] newKey = new short[newN + 1];
...
/* 1092 */       int pos; if (this.strategy.equals(key[i], (short)0)) { pos = newN; }
/*      */       else
/* 1094 */       { pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask;
/* 1095 */         for (; newKey[pos] != 0; pos = pos + 1 & mask); }
/*      */       
...
/*      */     try {
/* 1136 */       c = (ShortLinkedOpenCustomHashSet)super.clone();
/* 1137 */     } catch (CloneNotSupportedException cantHappen) {
/* 1138 */       throw new InternalError();
/*      */     } 
/* 1140 */     c.key = (short[])this.key.clone();
...
/* 1169 */     ShortIterator i = iterator();
/* 1170 */     s.defaultWriteObject();
/* 1171 */     for (int j = this.size; j-- != 0; s.writeShort(i.nextShort()));
/*      */   }
/*      */   
...
/* 1179 */     short[] key = this.key = new short[this.n + 1];
/* 1180 */     long[] link = this.link = new long[this.n + 1];
/* 1181 */     int prev = -1;
/* 1182 */     this.first = this.last = -1;
/*      */     
/* 1184 */     for (int i = this.size; i-- != 0; ) {
/* 1185 */       int pos; short k = s.readShort();
/* 1186 */       if (this.strategy.equals(k, (short)0))
/* 1187 */       { pos = this.n;
/* 1188 */         this.containsNull = true; }
/*      */       
/* 1190 */       else if (key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask] != 0) { while (key[pos = pos + 1 & this.mask] != 0); }
/*      */       
/* 1192 */       key[pos] = k;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\shorts\ShortLinkedOpenCustomHashSet.class
