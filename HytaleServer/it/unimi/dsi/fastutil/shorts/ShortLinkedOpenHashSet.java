...
/*  130 */     this.key = new short[this.n + 1];
...
/*  200 */     this(16, f);
/*  201 */     for (; i.hasNext(); add(i.nextShort()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  221 */     this(ShortIterators.asShortIterator(i), f);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  231 */     this(ShortIterators.asShortIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  244 */     ShortArrays.ensureOffsetLength(a, offset, length);
...
/*      */ 
/*      */   
/*      */   public static ShortLinkedOpenHashSet of() {
/*  286 */     return new ShortLinkedOpenHashSet();
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*      */   public static ShortLinkedOpenHashSet of(short e) {
/*  297 */     ShortLinkedOpenHashSet result = new ShortLinkedOpenHashSet(1, 0.75F);
/*  298 */     result.add(e);
/*  299 */     return result;
/*      */   }
/*      */ 
...
/*      */ 
/*      */   
/*      */   public static ShortLinkedOpenHashSet of(short e0, short e1) {
/*  313 */     ShortLinkedOpenHashSet result = new ShortLinkedOpenHashSet(2, 0.75F);
/*  314 */     result.add(e0);
...
/*      */ 
/*      */   
/*      */   public static ShortLinkedOpenHashSet of(short e0, short e1, short e2) {
/*  333 */     ShortLinkedOpenHashSet result = new ShortLinkedOpenHashSet(3, 0.75F);
/*  334 */     result.add(e0);
...
/*      */ 
/*      */   
/*      */   public static ShortLinkedOpenHashSet of(short... a) {
/*  355 */     ShortLinkedOpenHashSet result = new ShortLinkedOpenHashSet(a.length, 0.75F);
/*  356 */     for (short element : a) {
...
/*      */     } else {
/*      */       
/*  410 */       short[] key = this.key;
/*      */       short curr;
/*  412 */       if ((curr = key[pos = HashCommon.mix(k) & this.mask]) != 0) {
/*  413 */         if (curr == k) return false; 
/*  414 */         while ((curr = key[pos = pos + 1 & this.mask]) != 0) { if (curr == k) return false;  }
/*      */       
/*  416 */       }  key[pos] = k;
/*      */     } 
/*  418 */     if (this.size == 0) {
...
/*  442 */     short[] key = this.key; while (true) {
/*      */       short curr; int last;
/*  444 */       pos = (last = pos) + 1 & this.mask;
/*      */       while (true) {
/*  446 */         if ((curr = key[pos]) == 0) {
/*  447 */           key[last] = 0;
/*      */           return;
/*      */         } 
...
/*      */     } 
/*      */     
/*  483 */     short[] key = this.key;
/*      */     short curr;
/*      */     int pos;
/*  486 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0) return false; 
/*  487 */     if (k == curr) return removeEntry(pos); 
/*      */     while (true) {
/*  489 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0) return false; 
...
/*  496 */     if (k == 0) return this.containsNull;
/*      */     
/*  498 */     short[] key = this.key;
/*      */     short curr;
/*      */     int pos;
/*  501 */     if ((curr = key[pos = HashCommon.mix(k) & this.mask]) == 0) return false; 
/*  502 */     if (k == curr) return true; 
/*      */     while (true) {
/*  504 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0) return false; 
...
/*      */       } }
/*      */     
/*  527 */     short k = this.key[pos];
/*  528 */     this.size--;
/*  529 */     if (k == 0)
/*  530 */     { this.containsNull = false;
/*  531 */       this.key[this.n] = 0; }
...
/*      */       } }
/*      */     
/*  555 */     short k = this.key[pos];
/*  556 */     this.size--;
/*  557 */     if (k == 0)
/*  558 */     { this.containsNull = false;
/*  559 */       this.key[this.n] = 0; }
...
/*  700 */     Arrays.fill(this.key, (short)0);
...
/*      */     
/*      */     SetIterator() {
/*  871 */       this.next = ShortLinkedOpenHashSet.this.first;
/*  872 */       this.index = 0;
/*      */     }
/*      */     
/*      */     SetIterator(short from) {
/*  876 */       if (from == 0) {
/*  877 */         if (ShortLinkedOpenHashSet.this.containsNull) {
/*  878 */           this.next = (int)ShortLinkedOpenHashSet.this.link[ShortLinkedOpenHashSet.this.n];
/*  879 */           this.prev = ShortLinkedOpenHashSet.this.n; return;
/*      */         } 
/*  881 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  883 */       if (ShortLinkedOpenHashSet.this.key[ShortLinkedOpenHashSet.this.last] == from) {
/*  884 */         this.prev = ShortLinkedOpenHashSet.this.last;
/*  885 */         this.index = ShortLinkedOpenHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  889 */       short[] key = ShortLinkedOpenHashSet.this.key;
/*  890 */       int pos = HashCommon.mix(from) & ShortLinkedOpenHashSet.this.mask;
/*      */       
/*  892 */       while (key[pos] != 0) {
/*  893 */         if (key[pos] == from) {
/*      */           
/*  895 */           this.next = (int)ShortLinkedOpenHashSet.this.link[pos];
/*  896 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  899 */         pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */       } 
...
/*  918 */       this.next = (int)ShortLinkedOpenHashSet.this.link[this.curr];
/*  919 */       this.prev = this.curr;
/*  920 */       if (this.index >= 0) this.index++;
/*      */       
/*  922 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
...
/*  929 */       this.prev = (int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32L);
/*  930 */       this.next = this.curr;
/*  931 */       if (this.index >= 0) this.index--; 
/*  932 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEachRemaining(ShortConsumer action) {
/*  937 */       short[] key = ShortLinkedOpenHashSet.this.key;
/*  938 */       long[] link = ShortLinkedOpenHashSet.this.link;
...
/*      */       } 
/*  955 */       if (this.next == -1) {
/*  956 */         this.index = ShortLinkedOpenHashSet.this.size;
/*      */         return;
/*      */       } 
/*  959 */       int pos = ShortLinkedOpenHashSet.this.first;
/*  960 */       this.index = 1;
/*  961 */       while (pos != this.prev) {
/*  962 */         pos = (int)ShortLinkedOpenHashSet.this.link[pos];
/*  963 */         this.index++;
/*      */       } 
/*      */     }
/*      */ 
...
/*      */       { 
/*  986 */         this.index--;
/*  987 */         this.prev = (int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32L); }
/*  988 */       else { this.next = (int)ShortLinkedOpenHashSet.this.link[this.curr]; }
/*  989 */        ShortLinkedOpenHashSet.this.size--;
/*      */ 
/*      */       
/*  992 */       if (this.prev == -1) { ShortLinkedOpenHashSet.this.first = this.next; }
/*  993 */       else { ShortLinkedOpenHashSet.this.link[this.prev] = ShortLinkedOpenHashSet.this.link[this.prev] ^ (ShortLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL; }
/*  994 */        if (this.next == -1) { ShortLinkedOpenHashSet.this.last = this.prev; }
/*  995 */       else { ShortLinkedOpenHashSet.this.link[this.next] = ShortLinkedOpenHashSet.this.link[this.next] ^ (ShortLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L; }
/*  996 */        int pos = this.curr;
/*  997 */       this.curr = -1;
/*  998 */       if (pos == ShortLinkedOpenHashSet.this.n) {
/*  999 */         ShortLinkedOpenHashSet.this.containsNull = false;
/* 1000 */         ShortLinkedOpenHashSet.this.key[ShortLinkedOpenHashSet.this.n] = 0;
/*      */       } else {
/*      */         
/* 1003 */         short[] key = ShortLinkedOpenHashSet.this.key; while (true) {
/*      */           short curr;
/*      */           int last;
/* 1006 */           pos = (last = pos) + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */           while (true) {
/* 1008 */             if ((curr = key[pos]) == 0) {
/* 1009 */               key[last] = 0;
/*      */               return;
/*      */             } 
/* 1012 */             int slot = HashCommon.mix(curr) & ShortLinkedOpenHashSet.this.mask;
/* 1013 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/* 1014 */               break;  pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*      */           } 
/* 1016 */           key[last] = curr;
/* 1017 */           if (this.next == pos) this.next = last; 
/* 1018 */           if (this.prev == pos) this.prev = last; 
/* 1019 */           ShortLinkedOpenHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1066 */     return ShortSpliterators.asSpliterator(iterator(), Size64.sizeOf(this), 337);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1139 */     short[] key = this.key;
/* 1140 */     int mask = newN - 1;
/* 1141 */     short[] newKey = new short[newN + 1];
...
/* 1147 */       int pos; if (key[i] == 0) { pos = newN; }
/*      */       else
/* 1149 */       { pos = HashCommon.mix(key[i]) & mask;
/* 1150 */         for (; newKey[pos] != 0; pos = pos + 1 & mask); }
/*      */       
...
/* 1223 */     ShortIterator i = iterator();
/* 1224 */     s.defaultWriteObject();
/* 1225 */     for (int j = this.size; j-- != 0; s.writeShort(i.nextShort()));
/*      */   }
/*      */   
...
/* 1233 */     short[] key = this.key = new short[this.n + 1];
/* 1234 */     long[] link = this.link = new long[this.n + 1];
/* 1235 */     int prev = -1;
/* 1236 */     this.first = this.last = -1;
/*      */     
/* 1238 */     for (int i = this.size; i-- != 0; ) {
/* 1239 */       int pos; short k = s.readShort();
/* 1240 */       if (k == 0)
/* 1241 */       { pos = this.n;
/* 1242 */         this.containsNull = true; }
/*      */       
/* 1244 */       else if (key[pos = HashCommon.mix(k) & this.mask] != 0) { while (key[pos = pos + 1 & this.mask] != 0); }
/*      */       
/* 1246 */       key[pos] = k;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\shorts\ShortLinkedOpenHashSet.class
