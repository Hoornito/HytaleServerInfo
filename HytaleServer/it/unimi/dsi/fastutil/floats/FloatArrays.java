...
/*      */ {
/*  105 */   public static final float[] EMPTY_ARRAY = new float[0];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  113 */   public static final float[] DEFAULT_EMPTY_ARRAY = new float[0]; private static final int QUICKSORT_NO_REC = 16;
...
/*  202 */       float[] t = new float[newLength];
...
/*  221 */     float[] t = (length == 0) ? EMPTY_ARRAY : new float[length];
...
/*  255 */     float[] a = (length == 0) ? EMPTY_ARRAY : new float[length];
...
/*  312 */     while (i-- != 0) { if (Float.floatToIntBits(a1[i]) != Float.floatToIntBits(a2[i])) return false;  }
/*  313 */      return true;
/*      */   }
/*      */ 
/*      */ 
...
/*  543 */       if (len < 8192) {
/*  544 */         FloatArrays.quickSort(x, this.from, this.to, this.comp);
/*      */         
/*      */         return;
/*      */       } 
...
/*  550 */       int n = this.to - 1;
/*  551 */       int s = len / 8;
/*  552 */       l = FloatArrays.med3(x, l, l + s, l + 2 * s, this.comp);
/*  553 */       m = FloatArrays.med3(x, m - s, m, m + s, this.comp);
/*  554 */       n = FloatArrays.med3(x, n - 2 * s, n - s, n, this.comp);
/*  555 */       m = FloatArrays.med3(x, l, m, n, this.comp);
/*  556 */       float v = x[m];
/*      */       
...
/*  562 */           if (comparison == 0) FloatArrays.swap(x, a++, b); 
/*  563 */           b++; continue;
/*      */         } 
/*  565 */         while (c >= b && (comparison = this.comp.compare(x[c], v)) >= 0) {
/*  566 */           if (comparison == 0) FloatArrays.swap(x, c, d--); 
/*  567 */           c--;
/*      */         } 
/*  569 */         if (b > c)
/*  570 */           break;  FloatArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  574 */       s = Math.min(a - this.from, b - a);
/*  575 */       FloatArrays.swap(x, this.from, b - s, s);
/*  576 */       s = Math.min(d - c, this.to - d - 1);
/*  577 */       FloatArrays.swap(x, b, this.to - s, s);
/*      */       
/*  579 */       s = b - a;
/*  580 */       int t = d - c;
...
/*  626 */     int ab = Float.compare(x[a], x[b]);
/*  627 */     int ac = Float.compare(x[a], x[c]);
/*  628 */     int bc = Float.compare(x[b], x[c]);
...
/*  634 */       int m = i;
/*  635 */       for (int j = i + 1; j < to; ) { if (Float.compare(a[j], a[m]) < 0) m = j;  j++; }
/*  636 */        if (m != i) {
/*  637 */         float u = a[i];
/*  638 */         a[i] = a[m];
/*  639 */         a[m] = u;
...
/*  646 */       float t = a[i];
/*  647 */       int j = i; float u;
/*  648 */       for (u = a[j - 1]; Float.compare(t, u) < 0; u = a[--j - 1]) {
/*  649 */         a[j] = u;
/*  650 */         if (from == j - 1) {
/*  651 */           j--;
/*      */           break;
...
/*      */     while (true) {
/*      */       int comparison;
/*  699 */       if (b <= c && (comparison = Float.compare(x[b], v)) <= 0) {
/*  700 */         if (comparison == 0) swap(x, a++, b); 
/*  701 */         b++; continue;
/*      */       } 
/*  703 */       while (c >= b && (comparison = Float.compare(x[c], v)) >= 0) {
...
/*  757 */       if (len < 8192) {
/*  758 */         FloatArrays.quickSort(x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/*  764 */       int n = this.to - 1;
/*  765 */       int s = len / 8;
/*  766 */       l = FloatArrays.med3(x, l, l + s, l + 2 * s);
/*  767 */       m = FloatArrays.med3(x, m - s, m, m + s);
/*  768 */       n = FloatArrays.med3(x, n - 2 * s, n - s, n);
/*  769 */       m = FloatArrays.med3(x, l, m, n);
/*  770 */       float v = x[m];
/*      */       
/*  772 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/*  775 */         if (b <= c && (comparison = Float.compare(x[b], v)) <= 0) {
/*  776 */           if (comparison == 0) FloatArrays.swap(x, a++, b); 
/*  777 */           b++; continue;
/*      */         } 
/*  779 */         while (c >= b && (comparison = Float.compare(x[c], v)) >= 0) {
/*  780 */           if (comparison == 0) FloatArrays.swap(x, c, d--); 
/*  781 */           c--;
/*      */         } 
/*  783 */         if (b > c)
/*  784 */           break;  FloatArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  788 */       s = Math.min(a - this.from, b - a);
/*  789 */       FloatArrays.swap(x, this.from, b - s, s);
/*  790 */       s = Math.min(d - c, this.to - d - 1);
/*  791 */       FloatArrays.swap(x, b, this.to - s, s);
/*      */       
/*  793 */       s = b - a;
/*  794 */       int t = d - c;
...
/*  838 */     float aa = x[perm[a]];
/*  839 */     float bb = x[perm[b]];
/*  840 */     float cc = x[perm[c]];
/*  841 */     int ab = Float.compare(aa, bb);
/*  842 */     int ac = Float.compare(aa, cc);
/*  843 */     int bc = Float.compare(bb, cc);
...
/*  849 */       int t = perm[i];
/*  850 */       int j = i; int u;
/*  851 */       for (u = perm[j - 1]; Float.compare(a[t], a[u]) < 0; u = perm[--j - 1]) {
/*  852 */         perm[j] = u;
/*  853 */         if (from == j - 1) {
/*  854 */           j--;
/*      */           break;
...
/*  904 */     float v = x[perm[m]];
/*      */     
/*  906 */     int a = from, b = a, c = to - 1, d = c;
/*      */     while (true) {
/*      */       int comparison;
/*  909 */       if (b <= c && (comparison = Float.compare(x[perm[b]], v)) <= 0) {
/*  910 */         if (comparison == 0) IntArrays.swap(perm, a++, b); 
/*  911 */         b++; continue;
/*      */       } 
/*  913 */       while (c >= b && (comparison = Float.compare(x[perm[c]], v)) >= 0) {
...
/*  974 */       if (len < 8192) {
/*  975 */         FloatArrays.quickSortIndirect(this.perm, x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/*  981 */       int n = this.to - 1;
/*  982 */       int s = len / 8;
/*  983 */       l = FloatArrays.med3Indirect(this.perm, x, l, l + s, l + 2 * s);
/*  984 */       m = FloatArrays.med3Indirect(this.perm, x, m - s, m, m + s);
/*  985 */       n = FloatArrays.med3Indirect(this.perm, x, n - 2 * s, n - s, n);
/*  986 */       m = FloatArrays.med3Indirect(this.perm, x, l, m, n);
/*  987 */       float v = x[this.perm[m]];
/*      */       
/*  989 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/*  992 */         if (b <= c && (comparison = Float.compare(x[this.perm[b]], v)) <= 0) {
/*  993 */           if (comparison == 0) IntArrays.swap(this.perm, a++, b); 
/*  994 */           b++; continue;
/*      */         } 
/*  996 */         while (c >= b && (comparison = Float.compare(x[this.perm[c]], v)) >= 0) {
...
/* 1127 */     int t, ab = ((t = Float.compare(x[a], x[b])) == 0) ? Float.compare(y[a], y[b]) : t;
/* 1128 */     int ac = ((t = Float.compare(x[a], x[c])) == 0) ? Float.compare(y[a], y[c]) : t;
/* 1129 */     int bc = ((t = Float.compare(x[b], x[c])) == 0) ? Float.compare(y[b], y[c]) : t;
...
/* 1148 */       int m = i;
/* 1149 */       for (int j = i + 1; j < to; ) { int u; if ((u = Float.compare(a[j], a[m])) < 0 || (u == 0 && Float.compare(b[j], b[m]) < 0)) m = j;  j++; }
/* 1150 */        if (m != i) {
/* 1151 */         float t = a[i];
/* 1152 */         a[i] = a[m];
/* 1153 */         a[m] = t;
...
/* 1199 */     float v = x[m], w = y[m];
/*      */     
/* 1201 */     int a = from, b = a, c = to - 1, d = c;
/*      */     
/*      */     while (true) {
/* 1204 */       if (b <= c) { int comparison; int t; if ((comparison = ((t = Float.compare(x[b], v)) == 0) ? Float.compare(y[b], w) : t) <= 0) {
/* 1205 */           if (comparison == 0) swap(x, y, a++, b); 
/* 1206 */           b++; continue;
/*      */         }  }
/* 1208 */        while (c >= b) { int comparison; int t; if ((comparison = ((t = Float.compare(x[c], v)) == 0) ? Float.compare(y[c], w) : t) >= 0) {
...
/* 1267 */       if (len < 8192) {
/* 1268 */         FloatArrays.quickSort(x, y, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/* 1275 */       int s = len / 8;
/* 1276 */       l = FloatArrays.med3(x, y, l, l + s, l + 2 * s);
/* 1277 */       m = FloatArrays.med3(x, y, m - s, m, m + s);
/* 1278 */       n = FloatArrays.med3(x, y, n - 2 * s, n - s, n);
/* 1279 */       m = FloatArrays.med3(x, y, l, m, n);
/* 1280 */       float v = x[m], w = y[m];
/*      */       
/* 1282 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       
/*      */       while (true) {
/* 1285 */         if (b <= c) { int comparison; int i; if ((comparison = ((i = Float.compare(x[b], v)) == 0) ? Float.compare(y[b], w) : i) <= 0) {
/* 1286 */             if (comparison == 0) FloatArrays.swap(x, y, a++, b); 
/* 1287 */             b++; continue;
/*      */           }  }
/* 1289 */          while (c >= b) { int comparison; int i; if ((comparison = ((i = Float.compare(x[c], v)) == 0) ? Float.compare(y[c], w) : i) >= 0) {
/* 1290 */             if (comparison == 0) FloatArrays.swap(x, y, c, d--); 
/* 1291 */             c--;
/*      */           }  }
/* 1293 */          if (b > c)
/* 1294 */           break;  FloatArrays.swap(x, y, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/* 1298 */       s = Math.min(a - this.from, b - a);
/* 1299 */       FloatArrays.swap(x, y, this.from, b - s, s);
/* 1300 */       s = Math.min(d - c, this.to - d - 1);
/* 1301 */       FloatArrays.swap(x, y, b, this.to - s, s);
/* 1302 */       s = b - a;
/* 1303 */       int t = d - c;
/*      */       
...
/*      */ 
/*      */     
/* 1452 */     if (Float.compare(supp[mid - 1], supp[mid]) <= 0) {
/* 1453 */       System.arraycopy(supp, from, a, from, len);
/*      */       
/*      */       return;
/*      */     } 
/* 1457 */     for (int i = from, p = from, q = mid; i < to; i++) {
/* 1458 */       if (q >= to || (p < mid && Float.compare(supp[p], supp[q]) <= 0)) { a[i] = supp[p++]; }
...
/* 1475 */     mergeSort(a, from, to, (float[])null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1546 */     mergeSort(a, from, to, comp, (float[])null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1762 */     int i = Float.floatToIntBits(f);
/* 1763 */     return (i >= 0) ? i : (i ^ Integer.MAX_VALUE);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     } 
/* 1803 */     int maxLevel = 3;
/* 1804 */     int stackSize = 766;
/* 1805 */     int stackPos = 0;
/* 1806 */     int[] offsetStack = new int[766];
/* 1807 */     int[] lengthStack = new int[766];
/* 1808 */     int[] levelStack = new int[766];
...
/* 1818 */       int signMask = (level % 4 == 0) ? 128 : 0;
/* 1819 */       int shift = (3 - level % 4) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1824 */       for (int i = first + length; i-- != first; count[fixFloat(a[i]) >>> shift & 0xFF ^ signMask] = count[fixFloat(a[i]) >>> shift & 0xFF ^ signMask] + 1);
/*      */       
/* 1826 */       int lastUsed = -1;
...
/* 1834 */         float t = a[k];
/* 1835 */         c = fixFloat(t) >>> shift & 0xFF ^ signMask;
/* 1836 */         if (k < end) {
/* 1837 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 1838 */             float z = t;
/* 1839 */             t = a[d];
/* 1840 */             a[d] = z;
/* 1841 */             c = fixFloat(t) >>> shift & 0xFF ^ signMask;
/*      */           } 
/* 1843 */           a[k] = t;
/*      */         } 
/* 1845 */         if (level < 3 && count[c] > 1)
...
/*      */     } 
/* 2022 */     int maxLevel = 3;
/* 2023 */     int stackSize = 766;
/* 2024 */     int stackPos = 0;
/* 2025 */     int[] offsetStack = new int[766];
/* 2026 */     int[] lengthStack = new int[766];
/* 2027 */     int[] levelStack = new int[766];
...
/* 2038 */       int signMask = (level % 4 == 0) ? 128 : 0;
/* 2039 */       int shift = (3 - level % 4) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2044 */       for (int i = first + length; i-- != first; count[fixFloat(a[perm[i]]) >>> shift & 0xFF ^ signMask] = count[fixFloat(a[perm[i]]) >>> shift & 0xFF ^ signMask] + 1);
/*      */       
...
/*      */       } 
/* 2051 */       if (stable) {
/* 2052 */         for (j = first + length; j-- != first; ) { pos[fixFloat(a[perm[j]]) >>> shift & 0xFF ^ signMask] = pos[fixFloat(a[perm[j]]) >>> shift & 0xFF ^ signMask] - 1; support[pos[fixFloat(a[perm[j]]) >>> shift & 0xFF ^ signMask] - 1] = perm[j]; }
/* 2053 */          System.arraycopy(support, 0, perm, first, length);
/* 2054 */         for (j = 0, p = first; j <= lastUsed; j++) {
/* 2055 */           if (level < 3 && count[j] > 1) {
...
/* 2072 */         int t = perm[k];
/* 2073 */         c = fixFloat(a[t]) >>> shift & 0xFF ^ signMask;
/* 2074 */         if (k < end) {
/* 2075 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 2076 */             int z = t;
/* 2077 */             t = perm[d];
/* 2078 */             perm[d] = z;
/* 2079 */             c = fixFloat(a[t]) >>> shift & 0xFF ^ signMask;
/*      */           } 
/* 2081 */           perm[k] = t;
/*      */         } 
/* 2083 */         if (level < 3 && count[c] > 1) {
...
/*      */               count[fixFloat(a[perm[i]]) >>> shift & 0xFF ^ signMask] = count[fixFloat(a[perm[i]]) >>> shift & 0xFF ^ signMask] + 1;  int lastUsed = -1; int j = 0; int p = first; while (j < 256) {
/*      */               if (count[j] != 0)
...
/* 2270 */     int layers = 2;
/* 2271 */     int maxLevel = 7;
/* 2272 */     int stackSize = 1786;
/* 2273 */     int stackPos = 0;
/* 2274 */     int[] offsetStack = new int[1786];
/* 2275 */     int[] lengthStack = new int[1786];
/* 2276 */     int[] levelStack = new int[1786];
...
/* 2286 */       int signMask = (level % 4 == 0) ? 128 : 0;
/* 2287 */       float[] k = (level < 4) ? a : b;
/* 2288 */       int shift = (3 - level % 4) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2293 */       for (int i = first + length; i-- != first; count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] = count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] + 1);
/*      */       
/* 2295 */       int lastUsed = -1;
...
/* 2303 */         float t = a[m];
/* 2304 */         float u = b[m];
/* 2305 */         c = fixFloat(k[m]) >>> shift & 0xFF ^ signMask;
/* 2306 */         if (m < end) {
/* 2307 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2308 */             c = fixFloat(k[d]) >>> shift & 0xFF ^ signMask;
/* 2309 */             float z = t;
/* 2310 */             t = a[d];
/* 2311 */             a[d] = z;
/* 2312 */             z = u;
...
/*      */                 queue.add(POISON_PILL); 
/*      */             }  Segment segment = queue.take(); if (segment == POISON_PILL)
/*      */               return null;  int first = segment.offset; int length = segment.length; int level = segment.level; int signMask = (level % 4 == 0) ? 128 : 0; float[] k = (level < 4) ? a : b; int shift = (3 - level % 4) * 8; int i = first + length; while (i-- != first)
/*      */               count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] = count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] + 1;  int lastUsed = -1; int j = 0;
/*      */             int p = first;
/*      */             while (j < 256) {
/*      */               if (count[j] != 0)
/*      */                 lastUsed = j; 
...
/* 2447 */       int t = perm[i];
/* 2448 */       int j = i; int u;
/* 2449 */       for (u = perm[j - 1]; Float.compare(a[t], a[u]) < 0 || (Float.compare(a[t], a[u]) == 0 && Float.compare(b[t], b[u]) < 0); u = perm[--j - 1]) {
/* 2450 */         perm[j] = u;
/* 2451 */         if (from == j - 1) {
/* 2452 */           j--;
/*      */           break;
...
/* 2516 */     int layers = 2;
/* 2517 */     int maxLevel = 7;
/* 2518 */     int stackSize = 1786;
/* 2519 */     int stackPos = 0;
/* 2520 */     int[] offsetStack = new int[1786];
/* 2521 */     int[] lengthStack = new int[1786];
/* 2522 */     int[] levelStack = new int[1786];
...
/* 2533 */       int signMask = (level % 4 == 0) ? 128 : 0;
/* 2534 */       float[] k = (level < 4) ? a : b;
/* 2535 */       int shift = (3 - level % 4) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2540 */       for (int i = first + length; i-- != first; count[fixFloat(k[perm[i]]) >>> shift & 0xFF ^ signMask] = count[fixFloat(k[perm[i]]) >>> shift & 0xFF ^ signMask] + 1);
/*      */       
...
/*      */       } 
/* 2547 */       if (stable) {
/* 2548 */         for (j = first + length; j-- != first; ) { pos[fixFloat(k[perm[j]]) >>> shift & 0xFF ^ signMask] = pos[fixFloat(k[perm[j]]) >>> shift & 0xFF ^ signMask] - 1; support[pos[fixFloat(k[perm[j]]) >>> shift & 0xFF ^ signMask] - 1] = perm[j]; }
/* 2549 */          System.arraycopy(support, 0, perm, first, length);
/* 2550 */         for (j = 0, p = first; j < 256; j++) {
/* 2551 */           if (level < 7 && count[j] > 1) {
...
/* 2566 */         int t = perm[m];
/* 2567 */         c = fixFloat(k[t]) >>> shift & 0xFF ^ signMask;
/* 2568 */         if (m < end) {
/* 2569 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2570 */             int z = t;
/* 2571 */             t = perm[d];
/* 2572 */             perm[d] = z;
/* 2573 */             c = fixFloat(k[t]) >>> shift & 0xFF ^ signMask;
/*      */           } 
/* 2575 */           perm[m] = t;
/*      */         } 
/* 2577 */         if (level < 7 && count[c] > 1) {
...
/* 2656 */      int stackSize = 255 * (layers * 4 - 1) + 1;
/* 2657 */     int stackPos = 0;
...
/* 2671 */       int signMask = (level % 4 == 0) ? 128 : 0;
/* 2672 */       float[] k = a[level / 4];
/* 2673 */       int shift = (3 - level % 4) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2678 */       for (int i = first + length; i-- != first; count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] = count[fixFloat(k[i]) >>> shift & 0xFF ^ signMask] + 1);
/*      */       
/* 2680 */       int lastUsed = -1;
...
/* 2689 */         c = fixFloat(k[m]) >>> shift & 0xFF ^ signMask;
/* 2690 */         if (m < end) {
/* 2691 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2692 */             c = fixFloat(k[d]) >>> shift & 0xFF ^ signMask;
...
/* 2776 */       float t = a[from + length - i - 1];
...
/*      */ 
/*      */   
/* 2806 */   public static final Hash.Strategy<float[]> HASH_STRATEGY = new ArrayHashStrategy();
/*      */ }


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\floats\FloatArrays.class
