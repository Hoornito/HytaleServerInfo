...
/*  134 */     this.key = new double[this.n + 1];
...
/*  213 */     for (; i.hasNext(); add(i.nextDouble()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  235 */     this(DoubleIterators.asDoubleIterator(i), f, strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  246 */     this(DoubleIterators.asDoubleIterator(i), strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  260 */     DoubleArrays.ensureOffsetLength(a, offset, length);
...
/*      */     int pos;
/*  348 */     if (this.strategy.equals(k, 0.0D)) {
...
/*      */       double curr;
/*  357 */       if (Double.doubleToLongBits(curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) != 0L) {
/*  358 */         if (this.strategy.equals(curr, k)) return false; 
/*  359 */         while (Double.doubleToLongBits(curr = key[pos = pos + 1 & this.mask]) != 0L) { if (this.strategy.equals(curr, k)) return false;  }
/*      */       
/*  361 */       }  key[pos] = k;
/*      */     } 
/*  363 */     if (this.size == 0) {
...
/*  423 */     if (this.strategy.equals(k, 0.0D)) {
...
/*      */     double curr;
/*      */     int pos;
/*  431 */     if (Double.doubleToLongBits(curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0L) return false; 
/*  432 */     if (this.strategy.equals(k, curr)) return removeEntry(pos); 
/*      */     while (true) {
/*  434 */       if (Double.doubleToLongBits(curr = key[pos = pos + 1 & this.mask]) == 0L) return false; 
...
/*  441 */     if (this.strategy.equals(k, 0.0D)) return this.containsNull;
/*      */     
/*  443 */     double[] key = this.key;
/*      */     double curr;
/*      */     int pos;
/*  446 */     if (Double.doubleToLongBits(curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0L) return false; 
/*  447 */     if (this.strategy.equals(k, curr)) return true; 
/*      */     while (true) {
/*  449 */       if (Double.doubleToLongBits(curr = key[pos = pos + 1 & this.mask]) == 0L) return false; 
...
/*      */       } }
/*      */     
/*  472 */     double k = this.key[pos];
/*  473 */     this.size--;
/*  474 */     if (this.strategy.equals(k, 0.0D))
/*  475 */     { this.containsNull = false;
/*  476 */       this.key[this.n] = 0.0D; }
...
/*      */       } }
/*      */     
/*  500 */     double k = this.key[pos];
/*  501 */     this.size--;
/*  502 */     if (this.strategy.equals(k, 0.0D))
/*  503 */     { this.containsNull = false;
/*  504 */       this.key[this.n] = 0.0D; }
...
/*      */     int pos;
/*  555 */     if (this.strategy.equals(k, 0.0D)) {
...
/*      */       
/*  567 */       while (Double.doubleToLongBits(key[pos]) != 0L) {
...
/*      */     int pos;
/*  599 */     if (this.strategy.equals(k, 0.0D)) {
...
/*      */     } else {
/*      */       
/*  608 */       double[] key = this.key;
/*  609 */       pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask;
/*      */       
/*  611 */       while (Double.doubleToLongBits(key[pos]) != 0L) {
...
/*      */     
/*      */     SetIterator() {
/*  816 */       this.next = DoubleLinkedOpenCustomHashSet.this.first;
/*  817 */       this.index = 0;
/*      */     }
/*      */     
/*      */     SetIterator(double from) {
/*  821 */       if (DoubleLinkedOpenCustomHashSet.this.strategy.equals(from, 0.0D)) {
/*  822 */         if (DoubleLinkedOpenCustomHashSet.this.containsNull) {
/*  823 */           this.next = (int)DoubleLinkedOpenCustomHashSet.this.link[DoubleLinkedOpenCustomHashSet.this.n];
/*  824 */           this.prev = DoubleLinkedOpenCustomHashSet.this.n; return;
/*      */         } 
/*  826 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  828 */       if (DoubleLinkedOpenCustomHashSet.this.strategy.equals(DoubleLinkedOpenCustomHashSet.this.key[DoubleLinkedOpenCustomHashSet.this.last], from)) {
/*  829 */         this.prev = DoubleLinkedOpenCustomHashSet.this.last;
/*  830 */         this.index = DoubleLinkedOpenCustomHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  834 */       double[] key = DoubleLinkedOpenCustomHashSet.this.key;
/*  835 */       int pos = HashCommon.mix(DoubleLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & DoubleLinkedOpenCustomHashSet.this.mask;
/*      */       
/*  837 */       while (Double.doubleToLongBits(key[pos]) != 0L) {
/*  838 */         if (DoubleLinkedOpenCustomHashSet.this.strategy.equals(key[pos], from)) {
/*      */           
/*  840 */           this.next = (int)DoubleLinkedOpenCustomHashSet.this.link[pos];
/*  841 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  844 */         pos = pos + 1 & DoubleLinkedOpenCustomHashSet.this.mask;
/*      */       } 
...
/*  863 */       this.next = (int)DoubleLinkedOpenCustomHashSet.this.link[this.curr];
/*  864 */       this.prev = this.curr;
/*  865 */       if (this.index >= 0) this.index++;
/*      */       
/*  867 */       return DoubleLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
...
/*  874 */       this.prev = (int)(DoubleLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L);
/*  875 */       this.next = this.curr;
/*  876 */       if (this.index >= 0) this.index--; 
/*  877 */       return DoubleLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEachRemaining(DoubleConsumer action) {
/*  882 */       double[] key = DoubleLinkedOpenCustomHashSet.this.key;
/*  883 */       long[] link = DoubleLinkedOpenCustomHashSet.this.link;
...
/*      */       } 
/*  900 */       if (this.next == -1) {
/*  901 */         this.index = DoubleLinkedOpenCustomHashSet.this.size;
/*      */         return;
/*      */       } 
/*  904 */       int pos = DoubleLinkedOpenCustomHashSet.this.first;
/*  905 */       this.index = 1;
/*  906 */       while (pos != this.prev) {
/*  907 */         pos = (int)DoubleLinkedOpenCustomHashSet.this.link[pos];
/*  908 */         this.index++;
/*      */       } 
/*      */     }
/*      */ 
...
/*      */       { 
/*  931 */         this.index--;
/*  932 */         this.prev = (int)(DoubleLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L); }
/*  933 */       else { this.next = (int)DoubleLinkedOpenCustomHashSet.this.link[this.curr]; }
/*  934 */        DoubleLinkedOpenCustomHashSet.this.size--;
/*      */ 
/*      */       
/*  937 */       if (this.prev == -1) { DoubleLinkedOpenCustomHashSet.this.first = this.next; }
/*  938 */       else { DoubleLinkedOpenCustomHashSet.this.link[this.prev] = DoubleLinkedOpenCustomHashSet.this.link[this.prev] ^ (DoubleLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL; }
/*  939 */        if (this.next == -1) { DoubleLinkedOpenCustomHashSet.this.last = this.prev; }
/*  940 */       else { DoubleLinkedOpenCustomHashSet.this.link[this.next] = DoubleLinkedOpenCustomHashSet.this.link[this.next] ^ (DoubleLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L; }
/*  941 */        int pos = this.curr;
/*  942 */       this.curr = -1;
/*  943 */       if (pos == DoubleLinkedOpenCustomHashSet.this.n) {
/*  944 */         DoubleLinkedOpenCustomHashSet.this.containsNull = false;
/*  945 */         DoubleLinkedOpenCustomHashSet.this.key[DoubleLinkedOpenCustomHashSet.this.n] = 0.0D;
/*      */       } else {
/*      */         
/*  948 */         double[] key = DoubleLinkedOpenCustomHashSet.this.key; while (true) {
/*      */           double curr;
/*      */           int last;
/*  951 */           pos = (last = pos) + 1 & DoubleLinkedOpenCustomHashSet.this.mask;
/*      */           while (true) {
/*  953 */             if (Double.doubleToLongBits(curr = key[pos]) == 0L) {
/*  954 */               key[last] = 0.0D;
/*      */               return;
/*      */             } 
/*  957 */             int slot = HashCommon.mix(DoubleLinkedOpenCustomHashSet.this.strategy.hashCode(curr)) & DoubleLinkedOpenCustomHashSet.this.mask;
/*  958 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*  959 */               break;  pos = pos + 1 & DoubleLinkedOpenCustomHashSet.this.mask;
/*      */           } 
/*  961 */           key[last] = curr;
/*  962 */           if (this.next == pos) this.next = last; 
/*  963 */           if (this.prev == pos) this.prev = last; 
/*  964 */           DoubleLinkedOpenCustomHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1011 */     return DoubleSpliterators.asSpliterator(iterator(), Size64.sizeOf(this), 337);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1084 */     double[] key = this.key;
/* 1085 */     int mask = newN - 1;
/* 1086 */     double[] newKey = new double[newN + 1];
...
/* 1092 */       int pos; if (this.strategy.equals(key[i], 0.0D)) { pos = newN; }
/*      */       else
/* 1094 */       { pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask;
/* 1095 */         for (; Double.doubleToLongBits(newKey[pos]) != 0L; pos = pos + 1 & mask); }
/*      */       
...
/*      */     try {
/* 1136 */       c = (DoubleLinkedOpenCustomHashSet)super.clone();
...
/* 1160 */       for (; Double.doubleToLongBits(this.key[i]) == 0L; i++);
...
/* 1169 */     DoubleIterator i = iterator();
/* 1170 */     s.defaultWriteObject();
/* 1171 */     for (int j = this.size; j-- != 0; s.writeDouble(i.nextDouble()));
/*      */   }
/*      */   
...
/* 1179 */     double[] key = this.key = new double[this.n + 1];
/* 1180 */     long[] link = this.link = new long[this.n + 1];
/* 1181 */     int prev = -1;
/* 1182 */     this.first = this.last = -1;
/*      */     
/* 1184 */     for (int i = this.size; i-- != 0; ) {
/* 1185 */       int pos; double k = s.readDouble();
/* 1186 */       if (this.strategy.equals(k, 0.0D))
/* 1187 */       { pos = this.n;
/* 1188 */         this.containsNull = true; }
/*      */       
/* 1190 */       else if (Double.doubleToLongBits(key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) != 0L) { while (Double.doubleToLongBits(key[pos = pos + 1 & this.mask]) != 0L); }
/*      */       
/* 1192 */       key[pos] = k;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\doubles\DoubleLinkedOpenCustomHashSet.class
