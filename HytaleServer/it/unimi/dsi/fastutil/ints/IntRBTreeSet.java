...
/*   71 */     this.actualComparator = IntComparators.asIntComparator(this.storedComparator);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     allocatePaths();
/*  131 */     for (; i.hasNext(); add(i.nextInt()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  141 */     this(IntIterators.asIntIterator(i));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  154 */     this(c);
/*  155 */     IntArrays.ensureOffsetLength(a, offset, length);
...
/*  167 */     this(a, offset, length, (Comparator<? super Integer>)null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  216 */     return (this.actualComparator == null) ? Integer.compare(k1, k2) : this.actualComparator.compare(k1, k2);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     
/*      */     SetIterator() {
/*  897 */       this.next = IntRBTreeSet.this.firstEntry;
/*      */     }
/*      */     
/*      */     SetIterator(int k) {
/*  901 */       if ((this.next = IntRBTreeSet.this.locateKey(k)) != null) {
/*  902 */         if (IntRBTreeSet.this.compare(this.next.key, k) <= 0)
...
/*  970 */       updatePrevious();
/*  971 */       updateNext();
/*  972 */       IntRBTreeSet.this.remove(this.curr.key);
/*  973 */       this.curr = null;
/*      */     }
/*      */   }
/*      */ 
...
/*  994 */     return new Subset(0, true, to, false);
/*      */   }
/*      */ 
/*      */   
/*      */   public IntSortedSet tailSet(int from) {
/*  999 */     return new Subset(from, false, 0, true);
/*      */   }
/*      */ 
/*      */   
...
/* 1037 */       if (!bottom && !top && IntRBTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")"); 
/* 1038 */       this.from = from;
...
/* 1060 */       return ((this.bottom || IntRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || IntRBTreeSet.this.compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */ 
/*      */     
/*      */     public boolean contains(int k) {
/* 1066 */       return (in(k) && IntRBTreeSet.this.contains(k));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean add(int k) {
...
/* 1072 */       return IntRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */ 
/*      */     
...
/* 1079 */       return IntRBTreeSet.this.remove(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public IntComparator comparator() {
/* 1100 */       return IntRBTreeSet.this.actualComparator;
/*      */     }
/*      */ 
/*      */     
/*      */     public IntBidirectionalIterator iterator() {
...
/* 1116 */       return (IntRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */ 
/*      */     
/*      */     public IntSortedSet tailSet(int from) {
/* 1121 */       if (this.bottom) return new Subset(from, false, this.to, this.top); 
/* 1122 */       return (IntRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */ 
/*      */     
/*      */     public IntSortedSet subSet(int from, int to) {
/* 1127 */       if (this.top && this.bottom) return new Subset(from, false, to, false); 
/* 1128 */       if (!this.top) to = (IntRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1129 */       if (!this.bottom) from = (IntRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1140 */       if (IntRBTreeSet.this.tree == null) return null;
/*      */ 
/*      */ 
/*      */       
/* 1144 */       if (this.bottom) { e = IntRBTreeSet.this.firstEntry; }
/*      */       else
/* 1146 */       { e = IntRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1148 */         if (IntRBTreeSet.this.compare(e.key, this.from) < 0) e = e.next();
/*      */          }
/*      */ 
/*      */       
/* 1152 */       if (e == null || (!this.top && IntRBTreeSet.this.compare(e.key, this.to) >= 0)) return null; 
/* 1153 */       return e;
/*      */     }
/*      */ 
/*      */ 
...
/*      */       IntRBTreeSet.Entry e;
/* 1162 */       if (IntRBTreeSet.this.tree == null) return null;
/*      */ 
/*      */ 
/*      */       
/* 1166 */       if (this.top) { e = IntRBTreeSet.this.lastEntry; }
/*      */       else
/* 1168 */       { e = IntRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1170 */         if (IntRBTreeSet.this.compare(e.key, this.to) >= 0) e = e.prev();
/*      */          }
/*      */ 
/*      */       
/* 1174 */       if (e == null || (!this.bottom && IntRBTreeSet.this.compare(e.key, this.from) < 0)) return null; 
/* 1175 */       return e;
/*      */     }
/*      */ 
/*      */     
/*      */     public int firstInt() {
/* 1180 */       IntRBTreeSet.Entry e = firstEntry();
...
/*      */     
/*      */     public int lastInt() {
/* 1187 */       IntRBTreeSet.Entry e = lastEntry();
...
/*      */     {
/*      */       SubsetIterator() {
/* 1203 */         this.next = IntRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       
/*      */       SubsetIterator(int k) {
/* 1207 */         this();
/* 1208 */         if (this.next != null) {
/* 1209 */           if (!IntRBTreeSet.Subset.this.bottom && IntRBTreeSet.this.compare(k, this.next.key) < 0) { this.prev = null; }
/* 1210 */           else if (!IntRBTreeSet.Subset.this.top && IntRBTreeSet.this.compare(k, (this.prev = IntRBTreeSet.Subset.this.lastEntry()).key) >= 0) { this.next = null; }
/*      */           else
/* 1212 */           { this.next = IntRBTreeSet.this.locateKey(k);
/* 1213 */             if (IntRBTreeSet.this.compare(this.next.key, k) <= 0)
...
/* 1224 */         if (!IntRBTreeSet.Subset.this.bottom && this.prev != null && IntRBTreeSet.this.compare(this.prev.key, IntRBTreeSet.Subset.this.from) < 0) this.prev = null;
/*      */       
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1229 */         this.next = this.next.next();
/* 1230 */         if (!IntRBTreeSet.Subset.this.top && this.next != null && IntRBTreeSet.this.compare(this.next.key, IntRBTreeSet.Subset.this.to) >= 0) this.next = null;
/*      */       
/*      */       }
/*      */     }
/*      */   }
...
/*      */     IntRBTreeSet c;
/*      */     try {
/* 1249 */       c = (IntRBTreeSet)super.clone();
...
/* 1301 */     for (; n-- != 0; s.writeInt(i.nextInt()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1314 */     if (n == 1) {
/* 1315 */       Entry entry = new Entry(s.readInt());
...
/*      */ 
/*      */       
/* 1324 */       Entry entry = new Entry(s.readInt());
/* 1325 */       entry.black(true);
/* 1326 */       entry.right(new Entry(s.readInt()));
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\ints\IntRBTreeSet.class
