...
/*      */ {
/*  105 */   public static final long[] EMPTY_ARRAY = new long[0];
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */ 
/*      */   
/*  113 */   public static final long[] DEFAULT_EMPTY_ARRAY = new long[0]; private static final int QUICKSORT_NO_REC = 16;
...
/*  221 */     long[] t = (length == 0) ? EMPTY_ARRAY : new long[length];
...
/*  255 */     long[] a = (length == 0) ? EMPTY_ARRAY : new long[length];
...
/*  543 */       if (len < 8192) {
/*  544 */         LongArrays.quickSort(x, this.from, this.to, this.comp);
/*      */         
/*      */         return;
/*      */       } 
...
/*  550 */       int n = this.to - 1;
/*  551 */       int s = len / 8;
/*  552 */       l = LongArrays.med3(x, l, l + s, l + 2 * s, this.comp);
/*  553 */       m = LongArrays.med3(x, m - s, m, m + s, this.comp);
/*  554 */       n = LongArrays.med3(x, n - 2 * s, n - s, n, this.comp);
/*  555 */       m = LongArrays.med3(x, l, m, n, this.comp);
/*  556 */       long v = x[m];
/*      */       
...
/*  562 */           if (comparison == 0) LongArrays.swap(x, a++, b); 
/*  563 */           b++; continue;
/*      */         } 
/*  565 */         while (c >= b && (comparison = this.comp.compare(x[c], v)) >= 0) {
/*  566 */           if (comparison == 0) LongArrays.swap(x, c, d--); 
/*  567 */           c--;
/*      */         } 
/*  569 */         if (b > c)
/*  570 */           break;  LongArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  574 */       s = Math.min(a - this.from, b - a);
/*  575 */       LongArrays.swap(x, this.from, b - s, s);
/*  576 */       s = Math.min(d - c, this.to - d - 1);
/*  577 */       LongArrays.swap(x, b, this.to - s, s);
/*      */       
/*  579 */       s = b - a;
/*  580 */       int t = d - c;
...
/*  626 */     int ab = Long.compare(x[a], x[b]);
/*  627 */     int ac = Long.compare(x[a], x[c]);
/*  628 */     int bc = Long.compare(x[b], x[c]);
...
/*      */     while (true) {
/*      */       int comparison;
/*  699 */       if (b <= c && (comparison = Long.compare(x[b], v)) <= 0) {
/*  700 */         if (comparison == 0) swap(x, a++, b); 
/*  701 */         b++; continue;
/*      */       } 
/*  703 */       while (c >= b && (comparison = Long.compare(x[c], v)) >= 0) {
...
/*  757 */       if (len < 8192) {
/*  758 */         LongArrays.quickSort(x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/*  764 */       int n = this.to - 1;
/*  765 */       int s = len / 8;
/*  766 */       l = LongArrays.med3(x, l, l + s, l + 2 * s);
/*  767 */       m = LongArrays.med3(x, m - s, m, m + s);
/*  768 */       n = LongArrays.med3(x, n - 2 * s, n - s, n);
/*  769 */       m = LongArrays.med3(x, l, m, n);
/*  770 */       long v = x[m];
/*      */       
/*  772 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/*  775 */         if (b <= c && (comparison = Long.compare(x[b], v)) <= 0) {
/*  776 */           if (comparison == 0) LongArrays.swap(x, a++, b); 
/*  777 */           b++; continue;
/*      */         } 
/*  779 */         while (c >= b && (comparison = Long.compare(x[c], v)) >= 0) {
/*  780 */           if (comparison == 0) LongArrays.swap(x, c, d--); 
/*  781 */           c--;
/*      */         } 
/*  783 */         if (b > c)
/*  784 */           break;  LongArrays.swap(x, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/*  788 */       s = Math.min(a - this.from, b - a);
/*  789 */       LongArrays.swap(x, this.from, b - s, s);
/*  790 */       s = Math.min(d - c, this.to - d - 1);
/*  791 */       LongArrays.swap(x, b, this.to - s, s);
/*      */       
/*  793 */       s = b - a;
/*  794 */       int t = d - c;
...
/*  838 */     long aa = x[perm[a]];
/*  839 */     long bb = x[perm[b]];
/*  840 */     long cc = x[perm[c]];
/*  841 */     int ab = Long.compare(aa, bb);
/*  842 */     int ac = Long.compare(aa, cc);
/*  843 */     int bc = Long.compare(bb, cc);
...
/*      */     while (true) {
/*      */       int comparison;
/*  909 */       if (b <= c && (comparison = Long.compare(x[perm[b]], v)) <= 0) {
/*  910 */         if (comparison == 0) IntArrays.swap(perm, a++, b); 
/*  911 */         b++; continue;
/*      */       } 
/*  913 */       while (c >= b && (comparison = Long.compare(x[perm[c]], v)) >= 0) {
...
/*  974 */       if (len < 8192) {
/*  975 */         LongArrays.quickSortIndirect(this.perm, x, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/*  981 */       int n = this.to - 1;
/*  982 */       int s = len / 8;
/*  983 */       l = LongArrays.med3Indirect(this.perm, x, l, l + s, l + 2 * s);
/*  984 */       m = LongArrays.med3Indirect(this.perm, x, m - s, m, m + s);
/*  985 */       n = LongArrays.med3Indirect(this.perm, x, n - 2 * s, n - s, n);
/*  986 */       m = LongArrays.med3Indirect(this.perm, x, l, m, n);
/*  987 */       long v = x[this.perm[m]];
/*      */       
/*  989 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       while (true) {
/*      */         int comparison;
/*  992 */         if (b <= c && (comparison = Long.compare(x[this.perm[b]], v)) <= 0) {
/*  993 */           if (comparison == 0) IntArrays.swap(this.perm, a++, b); 
/*  994 */           b++; continue;
/*      */         } 
/*  996 */         while (c >= b && (comparison = Long.compare(x[this.perm[c]], v)) >= 0) {
...
/* 1127 */     int t, ab = ((t = Long.compare(x[a], x[b])) == 0) ? Long.compare(y[a], y[b]) : t;
/* 1128 */     int ac = ((t = Long.compare(x[a], x[c])) == 0) ? Long.compare(y[a], y[c]) : t;
/* 1129 */     int bc = ((t = Long.compare(x[b], x[c])) == 0) ? Long.compare(y[b], y[c]) : t;
...
/* 1148 */       int m = i;
/* 1149 */       for (int j = i + 1; j < to; ) { int u; if ((u = Long.compare(a[j], a[m])) < 0 || (u == 0 && b[j] < b[m])) m = j;  j++; }
/* 1150 */        if (m != i) {
/* 1151 */         long t = a[i];
/* 1152 */         a[i] = a[m];
/* 1153 */         a[m] = t;
...
/* 1199 */     long v = x[m], w = y[m];
/*      */     
/* 1201 */     int a = from, b = a, c = to - 1, d = c;
/*      */     
/*      */     while (true) {
/* 1204 */       if (b <= c) { int comparison; int t; if ((comparison = ((t = Long.compare(x[b], v)) == 0) ? Long.compare(y[b], w) : t) <= 0) {
/* 1205 */           if (comparison == 0) swap(x, y, a++, b); 
/* 1206 */           b++; continue;
/*      */         }  }
/* 1208 */        while (c >= b) { int comparison; int t; if ((comparison = ((t = Long.compare(x[c], v)) == 0) ? Long.compare(y[c], w) : t) >= 0) {
...
/* 1267 */       if (len < 8192) {
/* 1268 */         LongArrays.quickSort(x, y, this.from, this.to);
/*      */         
/*      */         return;
/*      */       } 
...
/* 1275 */       int s = len / 8;
/* 1276 */       l = LongArrays.med3(x, y, l, l + s, l + 2 * s);
/* 1277 */       m = LongArrays.med3(x, y, m - s, m, m + s);
/* 1278 */       n = LongArrays.med3(x, y, n - 2 * s, n - s, n);
/* 1279 */       m = LongArrays.med3(x, y, l, m, n);
/* 1280 */       long v = x[m], w = y[m];
/*      */       
/* 1282 */       int a = this.from, b = a, c = this.to - 1, d = c;
/*      */       
/*      */       while (true) {
/* 1285 */         if (b <= c) { int comparison; int i; if ((comparison = ((i = Long.compare(x[b], v)) == 0) ? Long.compare(y[b], w) : i) <= 0) {
/* 1286 */             if (comparison == 0) LongArrays.swap(x, y, a++, b); 
/* 1287 */             b++; continue;
/*      */           }  }
/* 1289 */          while (c >= b) { int comparison; int i; if ((comparison = ((i = Long.compare(x[c], v)) == 0) ? Long.compare(y[c], w) : i) >= 0) {
/* 1290 */             if (comparison == 0) LongArrays.swap(x, y, c, d--); 
/* 1291 */             c--;
/*      */           }  }
/* 1293 */          if (b > c)
/* 1294 */           break;  LongArrays.swap(x, y, b++, c--);
/*      */       } 
/*      */ 
/*      */       
/* 1298 */       s = Math.min(a - this.from, b - a);
/* 1299 */       LongArrays.swap(x, y, this.from, b - s, s);
/* 1300 */       s = Math.min(d - c, this.to - d - 1);
/* 1301 */       LongArrays.swap(x, y, b, this.to - s, s);
/* 1302 */       s = b - a;
/* 1303 */       int t = d - c;
/*      */       
...
/* 1475 */     mergeSort(a, from, to, (long[])null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1546 */     mergeSort(a, from, to, comp, (long[])null);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     } 
/* 1799 */     int maxLevel = 7;
/* 1800 */     int stackSize = 1786;
/* 1801 */     int stackPos = 0;
/* 1802 */     int[] offsetStack = new int[1786];
/* 1803 */     int[] lengthStack = new int[1786];
/* 1804 */     int[] levelStack = new int[1786];
...
/* 1814 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 1815 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 1820 */       for (int i = first + length; i-- != first; count[(int)(a[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(a[i] >>> shift & 0xFFL ^ signMask)] + 1);
/*      */       
/* 1822 */       int lastUsed = -1;
...
/* 1830 */         long t = a[k];
/* 1831 */         c = (int)(t >>> shift & 0xFFL ^ signMask);
/* 1832 */         if (k < end) {
/* 1833 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 1834 */             long z = t;
/* 1835 */             t = a[d];
/* 1836 */             a[d] = z;
/* 1837 */             c = (int)(t >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 1839 */           a[k] = t;
/*      */         } 
/* 1841 */         if (level < 7 && count[c] > 1)
...
/*      */     } 
/* 2018 */     int maxLevel = 7;
/* 2019 */     int stackSize = 1786;
/* 2020 */     int stackPos = 0;
/* 2021 */     int[] offsetStack = new int[1786];
/* 2022 */     int[] lengthStack = new int[1786];
/* 2023 */     int[] levelStack = new int[1786];
...
/* 2034 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2035 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2040 */       for (int i = first + length; i-- != first; count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] = count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] + 1);
/*      */       
...
/*      */       } 
/* 2047 */       if (stable) {
/* 2048 */         for (j = first + length; j-- != first; ) { pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] = pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1; support[pos[(int)(a[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1] = perm[j]; }
/* 2049 */          System.arraycopy(support, 0, perm, first, length);
/* 2050 */         for (j = 0, p = first; j <= lastUsed; j++) {
/* 2051 */           if (level < 7 && count[j] > 1) {
...
/* 2068 */         int t = perm[k];
/* 2069 */         c = (int)(a[t] >>> shift & 0xFFL ^ signMask);
/* 2070 */         if (k < end) {
/* 2071 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > k; ) {
/* 2072 */             int z = t;
/* 2073 */             t = perm[d];
/* 2074 */             perm[d] = z;
/* 2075 */             c = (int)(a[t] >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 2077 */           perm[k] = t;
/*      */         } 
/* 2079 */         if (level < 7 && count[c] > 1) {
...
/*      */               count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] = count[(int)(a[perm[i]] >>> shift & 0xFFL ^ signMask)] + 1;  int lastUsed = -1; int j = 0; int p = first; while (j < 256) {
/*      */               if (count[j] != 0)
...
/*      */     } 
/* 2266 */     int layers = 2;
/* 2267 */     int maxLevel = 15;
/* 2268 */     int stackSize = 3826;
/* 2269 */     int stackPos = 0;
/* 2270 */     int[] offsetStack = new int[3826];
/* 2271 */     int[] lengthStack = new int[3826];
/* 2272 */     int[] levelStack = new int[3826];
...
/* 2282 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2283 */       long[] k = (level < 8) ? a : b;
/* 2284 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2289 */       for (int i = first + length; i-- != first; count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] + 1);
/*      */       
/* 2291 */       int lastUsed = -1;
...
/* 2299 */         long t = a[m];
/* 2300 */         long u = b[m];
/* 2301 */         c = (int)(k[m] >>> shift & 0xFFL ^ signMask);
/* 2302 */         if (m < end) {
/* 2303 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2304 */             c = (int)(k[d] >>> shift & 0xFFL ^ signMask);
/* 2305 */             long z = t;
/* 2306 */             t = a[d];
/* 2307 */             a[d] = z;
/* 2308 */             z = u;
...
/*      */                 queue.add(POISON_PILL); 
/*      */             }  Segment segment = queue.take(); if (segment == POISON_PILL)
/*      */               return null;  int first = segment.offset; int length = segment.length; int level = segment.level; int signMask = (level % 8 == 0) ? 128 : 0; long[] k = (level < 8) ? a : b; int shift = (7 - level % 8) * 8; int i = first + length; while (i-- != first)
/*      */               count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] + 1;  int lastUsed = -1; int j = 0;
/*      */             int p = first;
/*      */             while (j < 256) {
/*      */               if (count[j] != 0)
/*      */                 lastUsed = j; 
...
/*      */     } 
/* 2512 */     int layers = 2;
/* 2513 */     int maxLevel = 15;
/* 2514 */     int stackSize = 3826;
/* 2515 */     int stackPos = 0;
/* 2516 */     int[] offsetStack = new int[3826];
/* 2517 */     int[] lengthStack = new int[3826];
/* 2518 */     int[] levelStack = new int[3826];
...
/* 2529 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2530 */       long[] k = (level < 8) ? a : b;
/* 2531 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2536 */       for (int i = first + length; i-- != first; count[(int)(k[perm[i]] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[perm[i]] >>> shift & 0xFFL ^ signMask)] + 1);
/*      */       
...
/*      */       } 
/* 2543 */       if (stable) {
/* 2544 */         for (j = first + length; j-- != first; ) { pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] = pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1; support[pos[(int)(k[perm[j]] >>> shift & 0xFFL ^ signMask)] - 1] = perm[j]; }
/* 2545 */          System.arraycopy(support, 0, perm, first, length);
/* 2546 */         for (j = 0, p = first; j < 256; j++) {
/* 2547 */           if (level < 15 && count[j] > 1) {
...
/* 2562 */         int t = perm[m];
/* 2563 */         c = (int)(k[t] >>> shift & 0xFFL ^ signMask);
/* 2564 */         if (m < end) {
/* 2565 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2566 */             int z = t;
/* 2567 */             t = perm[d];
/* 2568 */             perm[d] = z;
/* 2569 */             c = (int)(k[t] >>> shift & 0xFFL ^ signMask);
/*      */           } 
/* 2571 */           perm[m] = t;
/*      */         } 
/* 2573 */         if (level < 15 && count[c] > 1) {
...
/* 2652 */      int stackSize = 255 * (layers * 8 - 1) + 1;
/* 2653 */     int stackPos = 0;
...
/* 2667 */       int signMask = (level % 8 == 0) ? 128 : 0;
/* 2668 */       long[] k = a[level / 8];
/* 2669 */       int shift = (7 - level % 8) * 8;
/*      */ 
/*      */ 
/*      */ 
/*      */       
/* 2674 */       for (int i = first + length; i-- != first; count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] = count[(int)(k[i] >>> shift & 0xFFL ^ signMask)] + 1);
/*      */       
/* 2676 */       int lastUsed = -1;
...
/* 2685 */         c = (int)(k[m] >>> shift & 0xFFL ^ signMask);
/* 2686 */         if (m < end) {
/* 2687 */           int d; for (pos[c] = pos[c] - 1; (d = pos[c] - 1) > m; ) {
/* 2688 */             c = (int)(k[d] >>> shift & 0xFFL ^ signMask);
...
/*      */ 
/*      */   
/* 2802 */   public static final Hash.Strategy<long[]> HASH_STRATEGY = new ArrayHashStrategy();
/*      */ }


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\longs\LongArrays.class
