...
/*  800 */       Entry e = (Entry)o;
/*  801 */       return (this.key == e.key);
/*      */     }
/*      */ 
/*      */     
/*      */     public int hashCode() {
/*  806 */       return HashCommon.long2int(this.key);
/*      */     }
/*      */ 
/*      */     
...
/*      */     
/*      */     SetIterator() {
/*  897 */       this.next = LongRBTreeSet.this.firstEntry;
/*      */     }
/*      */     
/*      */     SetIterator(long k) {
/*  901 */       if ((this.next = LongRBTreeSet.this.locateKey(k)) != null) {
/*  902 */         if (LongRBTreeSet.this.compare(this.next.key, k) <= 0)
...
/*  970 */       updatePrevious();
/*  971 */       updateNext();
/*  972 */       LongRBTreeSet.this.remove(this.curr.key);
/*  973 */       this.curr = null;
/*      */     }
/*      */   }
/*      */ 
...
/*  994 */     return new Subset(0L, true, to, false);
/*      */   }
/*      */ 
/*      */   
/*      */   public LongSortedSet tailSet(long from) {
/*  999 */     return new Subset(from, false, 0L, true);
/*      */   }
/*      */ 
/*      */   
...
/* 1037 */       if (!bottom && !top && LongRBTreeSet.this.compare(from, to) > 0) throw new IllegalArgumentException("Start element (" + from + ") is larger than end element (" + to + ")"); 
/* 1038 */       this.from = from;
...
/* 1060 */       return ((this.bottom || LongRBTreeSet.this.compare(k, this.from) >= 0) && (this.top || LongRBTreeSet.this.compare(k, this.to) < 0));
/*      */     }
/*      */ 
/*      */ 
/*      */     
/*      */     public boolean contains(long k) {
/* 1066 */       return (in(k) && LongRBTreeSet.this.contains(k));
/*      */     }
/*      */ 
/*      */     
/*      */     public boolean add(long k) {
...
/* 1072 */       return LongRBTreeSet.this.add(k);
/*      */     }
/*      */ 
/*      */ 
/*      */     
...
/* 1079 */       return LongRBTreeSet.this.remove(k);
/*      */     }
/*      */ 
/*      */     
/*      */     public int size() {
...
/*      */     
/*      */     public LongComparator comparator() {
/* 1100 */       return LongRBTreeSet.this.actualComparator;
/*      */     }
/*      */ 
/*      */     
/*      */     public LongBidirectionalIterator iterator() {
...
/* 1116 */       return (LongRBTreeSet.this.compare(to, this.to) < 0) ? new Subset(this.from, this.bottom, to, false) : this;
/*      */     }
/*      */ 
/*      */     
/*      */     public LongSortedSet tailSet(long from) {
/* 1121 */       if (this.bottom) return new Subset(from, false, this.to, this.top); 
/* 1122 */       return (LongRBTreeSet.this.compare(from, this.from) > 0) ? new Subset(from, false, this.to, this.top) : this;
/*      */     }
/*      */ 
/*      */     
/*      */     public LongSortedSet subSet(long from, long to) {
/* 1127 */       if (this.top && this.bottom) return new Subset(from, false, to, false); 
/* 1128 */       if (!this.top) to = (LongRBTreeSet.this.compare(to, this.to) < 0) ? to : this.to; 
/* 1129 */       if (!this.bottom) from = (LongRBTreeSet.this.compare(from, this.from) > 0) ? from : this.from; 
...
/* 1140 */       if (LongRBTreeSet.this.tree == null) return null;
/*      */ 
/*      */ 
/*      */       
/* 1144 */       if (this.bottom) { e = LongRBTreeSet.this.firstEntry; }
/*      */       else
/* 1146 */       { e = LongRBTreeSet.this.locateKey(this.from);
/*      */         
/* 1148 */         if (LongRBTreeSet.this.compare(e.key, this.from) < 0) e = e.next();
/*      */          }
/*      */ 
/*      */       
/* 1152 */       if (e == null || (!this.top && LongRBTreeSet.this.compare(e.key, this.to) >= 0)) return null; 
/* 1153 */       return e;
/*      */     }
/*      */ 
/*      */ 
...
/*      */       LongRBTreeSet.Entry e;
/* 1162 */       if (LongRBTreeSet.this.tree == null) return null;
/*      */ 
/*      */ 
/*      */       
/* 1166 */       if (this.top) { e = LongRBTreeSet.this.lastEntry; }
/*      */       else
/* 1168 */       { e = LongRBTreeSet.this.locateKey(this.to);
/*      */         
/* 1170 */         if (LongRBTreeSet.this.compare(e.key, this.to) >= 0) e = e.prev();
/*      */          }
/*      */ 
/*      */       
/* 1174 */       if (e == null || (!this.bottom && LongRBTreeSet.this.compare(e.key, this.from) < 0)) return null; 
/* 1175 */       return e;
/*      */     }
/*      */ 
/*      */     
/*      */     public long firstLong() {
/* 1180 */       LongRBTreeSet.Entry e = firstEntry();
...
/*      */     
/*      */     public long lastLong() {
/* 1187 */       LongRBTreeSet.Entry e = lastEntry();
...
/*      */       extends LongRBTreeSet.SetIterator
/*      */     {
/*      */       SubsetIterator() {
/* 1203 */         this.next = LongRBTreeSet.Subset.this.firstEntry();
/*      */       }
/*      */       
/*      */       SubsetIterator(long k) {
/* 1207 */         this();
/* 1208 */         if (this.next != null) {
/* 1209 */           if (!LongRBTreeSet.Subset.this.bottom && LongRBTreeSet.this.compare(k, this.next.key) < 0) { this.prev = null; }
/* 1210 */           else if (!LongRBTreeSet.Subset.this.top && LongRBTreeSet.this.compare(k, (this.prev = LongRBTreeSet.Subset.this.lastEntry()).key) >= 0) { this.next = null; }
/*      */           else
/* 1212 */           { this.next = LongRBTreeSet.this.locateKey(k);
/* 1213 */             if (LongRBTreeSet.this.compare(this.next.key, k) <= 0)
...
/* 1224 */         if (!LongRBTreeSet.Subset.this.bottom && this.prev != null && LongRBTreeSet.this.compare(this.prev.key, LongRBTreeSet.Subset.this.from) < 0) this.prev = null;
/*      */       
/*      */       }
/*      */       
/*      */       void updateNext() {
/* 1229 */         this.next = this.next.next();
/* 1230 */         if (!LongRBTreeSet.Subset.this.top && this.next != null && LongRBTreeSet.this.compare(this.next.key, LongRBTreeSet.Subset.this.to) >= 0) this.next = null;
/*      */       
/*      */       }
/*      */     }
/*      */   }
...
/*      */     LongRBTreeSet c;
/*      */     try {
/* 1249 */       c = (LongRBTreeSet)super.clone();
...
/* 1301 */     for (; n-- != 0; s.writeLong(i.nextLong()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/* 1314 */     if (n == 1) {
/* 1315 */       Entry entry = new Entry(s.readLong());
...
/*      */ 
/*      */       
/* 1324 */       Entry entry = new Entry(s.readLong());
/* 1325 */       entry.black(true);
/* 1326 */       entry.right(new Entry(s.readLong()));
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\longs\LongRBTreeSet.class
