...
/* 106 */     this.key = LongBigArrays.newBigArray(this.n);
/* 107 */     initMasks();
/*     */   }
/*     */ 
/*     */ 
...
/* 176 */     this(16L, f);
/* 177 */     for (; i.hasNext(); add(i.nextLong()));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 197 */     this(LongIterators.asLongIterator(i), f);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 207 */     this(LongIterators.asLongIterator(i));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 220 */     LongArrays.ensureOffsetLength(a, offset, length);
...
/* 267 */     return stream.<LongOpenHashBigSet>collect(LongOpenHashBigSet::new, LongOpenHashBigSet::add, LongOpenHashBigSet::addAll);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 281 */     return stream.<LongOpenHashBigSet>collect(() -> new LongOpenHashBigSet(expectedSize), LongOpenHashBigSet::add, LongOpenHashBigSet::addAll);
/*     */   }
/*     */   
...
/* 325 */       long h = HashCommon.mix(k); int displ, base;
/*     */       long curr;
/* 327 */       if ((curr = key[base = (int)((h & this.mask) >>> 27L)][displ = (int)(h & this.segmentMask)]) != 0L) {
/* 328 */         if (curr == k) return false;  while (true) {
/* 329 */           if ((curr = key[base = base + (((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0) & this.baseMask][displ]) != 0L) { if (curr == k) return false;  continue; }  break;
/*     */         } 
...
/*     */     long curr;
/*     */     int displ, base;
/* 388 */     if ((curr = key[base = (int)((h & this.mask) >>> 27L)][displ = (int)(h & this.segmentMask)]) == 0L) return false; 
/* 389 */     if (curr == k) return removeEntry(base, displ); 
/*     */     while (true) {
/* 391 */       if ((curr = key[base = base + (((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0) & this.baseMask][displ]) == 0L) return false; 
...
/* 398 */     if (k == 0L) return this.containsNull;
/*     */     
/* 400 */     long[][] key = this.key;
/* 401 */     long h = HashCommon.mix(k);
/*     */     long curr;
/*     */     int displ, base;
/* 404 */     if ((curr = key[base = (int)((h & this.mask) >>> 27L)][displ = (int)(h & this.segmentMask)]) == 0L) return false; 
/* 405 */     if (curr == k) return true; 
/*     */     while (true) {
/* 407 */       if ((curr = key[base = base + (((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0) & this.baseMask][displ]) == 0L) return false; 
...
/* 427 */     BigArrays.fill(this.key, 0L);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */   {
/* 437 */     int base = LongOpenHashBigSet.this.key.length;
/*     */ 
/*     */ 
/*     */     
/*     */     int displ;
...
/* 445 */     long last = -1L;
/*     */     
/* 447 */     long c = LongOpenHashBigSet.this.size;
/*     */     
/* 449 */     boolean mustReturnNull = LongOpenHashBigSet.this.containsNull;
/*     */ 
/*     */ 
/*     */     
...
/* 467 */         this.last = LongOpenHashBigSet.this.n;
/* 468 */         return 0L;
/*     */       } 
/* 470 */       long[][] key = LongOpenHashBigSet.this.key;
/*     */       while (true) {
/* 472 */         if (this.displ == 0 && this.base <= 0) {
/*     */           
/* 474 */           this.last = Long.MIN_VALUE;
/* 475 */           return this.wrapped.getLong(---this.base - 1);
/*     */         } 
/* 477 */         if (this.displ-- == 0) this.displ = (key[--this.base]).length - 1; 
/* 478 */         long k = key[this.base][this.displ];
/* 479 */         if (k != 0L) {
...
/* 496 */       long[][] key = LongOpenHashBigSet.this.key; while (true) {
/*     */         long curr, last;
/* 498 */         pos = (last = pos) + 1L & LongOpenHashBigSet.this.mask;
/*     */         while (true) {
/* 500 */           if ((curr = BigArrays.get(key, pos)) == 0L) {
/* 501 */             BigArrays.set(key, last, 0L);
/*     */             return;
/*     */           } 
/* 504 */           long slot = HashCommon.mix(curr) & LongOpenHashBigSet.this.mask;
/* 505 */           if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/* 506 */             break;  pos = pos + 1L & LongOpenHashBigSet.this.mask;
/*     */         } 
/* 508 */         if (pos < last) {
/* 509 */           if (this.wrapped == null) this.wrapped = new LongArrayList(); 
...
/* 519 */       if (this.last == LongOpenHashBigSet.this.n) { LongOpenHashBigSet.this.containsNull = false; }
/* 520 */       else if (this.base >= 0) { shiftKeys(this.last); }
/*     */       else
/*     */       
/* 523 */       { LongOpenHashBigSet.this.remove(this.wrapped.getLong(-this.base - 1));
/* 524 */         this.last = -1L;
/*     */         return; }
/*     */       
/* 527 */       LongOpenHashBigSet.this.size--;
/* 528 */       this.last = -1L;
/*     */     }
/*     */     
/*     */     private SetIterator() {}
...
/* 549 */     long pos = 0L;
/*     */     
/* 551 */     long max = LongOpenHashBigSet.this.n;
/*     */     
/* 553 */     long c = 0L;
/*     */     
/* 555 */     boolean mustReturnNull = LongOpenHashBigSet.this.containsNull;
/*     */ 
/*     */     
...
/* 572 */         this.c++;
/* 573 */         action.accept(0L);
/* 574 */         return true;
/*     */       } 
/* 576 */       long[][] key = LongOpenHashBigSet.this.key;
/* 577 */       while (this.pos < this.max) {
/* 578 */         long gotten = BigArrays.get(key, this.pos);
/* 579 */         if (gotten != 0L) {
/* 580 */           this.c++;
/* 581 */           this.pos++;
...
/* 595 */         action.accept(0L);
/* 596 */         this.c++;
/*     */       } 
/* 598 */       long[][] key = LongOpenHashBigSet.this.key;
/* 599 */       while (this.pos < this.max) {
/* 600 */         long gotten = BigArrays.get(key, this.pos);
/* 601 */         if (gotten != 0L) {
...
/*     */       {
/* 618 */         return LongOpenHashBigSet.this.size - this.c;
/*     */       }
/*     */ 
/*     */ 
/*     */       
/* 623 */       return Math.min(LongOpenHashBigSet.this.size - this.c, (long)(LongOpenHashBigSet.this.realSize() / LongOpenHashBigSet.this.n * (this.max - this.pos)) + (this.mustReturnNull ? 1L : 0L));
/*     */     }
/*     */ 
/*     */ 
/*     */     
...
/* 656 */         n--;
/*     */       } 
/* 658 */       long[][] key = LongOpenHashBigSet.this.key;
/* 659 */       while (this.pos < this.max && n > 0L) {
/* 660 */         if (BigArrays.get(key, this.pos++) != 0L) {
/* 661 */           skipped++;
/* 662 */           n--;
/*     */         } 
/*     */       } 
...
/* 677 */       action.accept(0L);
/*     */     }
/* 679 */     long pos = 0L;
/* 680 */     long max = this.n;
/* 681 */     long[][] key = this.key;
/* 682 */     while (pos < max) {
/* 683 */       long gotten = BigArrays.get(key, pos++);
/* 684 */       if (gotten != 0L) {
...
/* 748 */     long[][] key = this.key;
/* 749 */     long[][] newKey = LongBigArrays.newBigArray(newN);
...
/* 757 */       for (; key[base][displ] == 0L; base += ((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0);
/* 758 */       long k = key[base][displ];
/* 759 */       long h = HashCommon.mix(k);
/*     */       int b, d;
/* 761 */       if (newKey[b = (int)((h & mask) >>> 27L)][d = (int)(h & newSegmentMask)] != 0L) while (true) { if (newKey[b = b + (((d = d + 1 & newSegmentMask) == 0) ? 1 : 0) & newBaseMask][d] != 0L)
...
/*     */     try {
/* 801 */       c = (LongOpenHashBigSet)super.clone();
...
/* 824 */       for (; key[base][displ] == 0L; base += ((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0);
/* 825 */       h += HashCommon.long2int(key[base][displ]);
...
/* 832 */     LongIterator i = iterator();
/* 833 */     s.defaultWriteObject();
/* 834 */     for (long j = this.size; j-- != 0L; s.writeLong(i.nextLong()));
/*     */   }
/*     */   
...
/* 841 */     long[][] key = this.key = LongBigArrays.newBigArray(this.n);
/* 842 */     initMasks();
/*     */ 
/*     */ 
/*     */     
/* 846 */     for (long i = this.size; i-- != 0L; ) {
/* 847 */       long k = s.readLong();
/* 848 */       if (k == 0L) { this.containsNull = true; continue; }
/*     */       
/* 850 */       long h = HashCommon.mix(k); int base, displ;
/* 851 */       if (key[base = (int)((h & this.mask) >>> 27L)][displ = (int)(h & this.segmentMask)] != 0L) while (true) { if (key[base = base + (((displ = displ + 1 & this.segmentMask) == 0) ? 1 : 0) & this.baseMask][displ] != 0L)
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\longs\LongOpenHashBigSet.class
