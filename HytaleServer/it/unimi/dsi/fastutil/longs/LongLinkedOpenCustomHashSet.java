...
/*  134 */     this.key = new long[this.n + 1];
...
/*  213 */     for (; i.hasNext(); add(i.nextLong()));
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  235 */     this(LongIterators.asLongIterator(i), f, strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  246 */     this(LongIterators.asLongIterator(i), strategy);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  260 */     LongArrays.ensureOffsetLength(a, offset, length);
...
/*      */     int pos;
/*  348 */     if (this.strategy.equals(k, 0L)) {
...
/*      */       long curr;
/*  357 */       if ((curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) != 0L) {
/*  358 */         if (this.strategy.equals(curr, k)) return false; 
/*  359 */         while ((curr = key[pos = pos + 1 & this.mask]) != 0L) { if (this.strategy.equals(curr, k)) return false;  }
/*      */       
/*  361 */       }  key[pos] = k;
/*      */     } 
/*  363 */     if (this.size == 0) {
...
/*  387 */     long[] key = this.key; while (true) {
/*      */       long curr; int last;
/*  389 */       pos = (last = pos) + 1 & this.mask;
/*      */       while (true) {
/*  391 */         if ((curr = key[pos]) == 0L) {
/*  392 */           key[last] = 0L;
/*      */           return;
/*      */         } 
...
/*      */     } 
/*      */     
/*  428 */     long[] key = this.key;
/*      */     long curr;
/*      */     int pos;
/*  431 */     if ((curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0L) return false; 
/*  432 */     if (this.strategy.equals(k, curr)) return removeEntry(pos); 
/*      */     while (true) {
/*  434 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0L) return false; 
...
/*  441 */     if (this.strategy.equals(k, 0L)) return this.containsNull;
/*      */     
/*  443 */     long[] key = this.key;
/*      */     long curr;
/*      */     int pos;
/*  446 */     if ((curr = key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask]) == 0L) return false; 
/*  447 */     if (this.strategy.equals(k, curr)) return true; 
/*      */     while (true) {
/*  449 */       if ((curr = key[pos = pos + 1 & this.mask]) == 0L) return false; 
...
/*      */       } }
/*      */     
/*  472 */     long k = this.key[pos];
/*  473 */     this.size--;
/*  474 */     if (this.strategy.equals(k, 0L))
/*  475 */     { this.containsNull = false;
/*  476 */       this.key[this.n] = 0L; }
...
/*      */       } }
/*      */     
/*  500 */     long k = this.key[pos];
/*  501 */     this.size--;
/*  502 */     if (this.strategy.equals(k, 0L))
/*  503 */     { this.containsNull = false;
/*  504 */       this.key[this.n] = 0L; }
...
/*      */     int pos;
/*  555 */     if (this.strategy.equals(k, 0L)) {
...
/*      */     int pos;
/*  599 */     if (this.strategy.equals(k, 0L)) {
...
/*      */     
/*      */     SetIterator() {
/*  816 */       this.next = LongLinkedOpenCustomHashSet.this.first;
/*  817 */       this.index = 0;
/*      */     }
/*      */     
/*      */     SetIterator(long from) {
/*  821 */       if (LongLinkedOpenCustomHashSet.this.strategy.equals(from, 0L)) {
/*  822 */         if (LongLinkedOpenCustomHashSet.this.containsNull) {
/*  823 */           this.next = (int)LongLinkedOpenCustomHashSet.this.link[LongLinkedOpenCustomHashSet.this.n];
/*  824 */           this.prev = LongLinkedOpenCustomHashSet.this.n; return;
/*      */         } 
/*  826 */         throw new NoSuchElementException("The key " + from + " does not belong to this set.");
/*      */       } 
/*  828 */       if (LongLinkedOpenCustomHashSet.this.strategy.equals(LongLinkedOpenCustomHashSet.this.key[LongLinkedOpenCustomHashSet.this.last], from)) {
/*  829 */         this.prev = LongLinkedOpenCustomHashSet.this.last;
/*  830 */         this.index = LongLinkedOpenCustomHashSet.this.size;
/*      */         
/*      */         return;
/*      */       } 
/*  834 */       long[] key = LongLinkedOpenCustomHashSet.this.key;
/*  835 */       int pos = HashCommon.mix(LongLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & LongLinkedOpenCustomHashSet.this.mask;
/*      */       
/*  837 */       while (key[pos] != 0L) {
/*  838 */         if (LongLinkedOpenCustomHashSet.this.strategy.equals(key[pos], from)) {
/*      */           
/*  840 */           this.next = (int)LongLinkedOpenCustomHashSet.this.link[pos];
/*  841 */           this.prev = pos;
/*      */           return;
/*      */         } 
/*  844 */         pos = pos + 1 & LongLinkedOpenCustomHashSet.this.mask;
/*      */       } 
...
/*  863 */       this.next = (int)LongLinkedOpenCustomHashSet.this.link[this.curr];
/*  864 */       this.prev = this.curr;
/*  865 */       if (this.index >= 0) this.index++;
/*      */       
/*  867 */       return LongLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
...
/*  874 */       this.prev = (int)(LongLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L);
/*  875 */       this.next = this.curr;
/*  876 */       if (this.index >= 0) this.index--; 
/*  877 */       return LongLinkedOpenCustomHashSet.this.key[this.curr];
/*      */     }
/*      */ 
/*      */     
/*      */     public void forEachRemaining(LongConsumer action) {
/*  882 */       long[] key = LongLinkedOpenCustomHashSet.this.key;
/*  883 */       long[] link = LongLinkedOpenCustomHashSet.this.link;
...
/*      */         return;
/*      */       } 
/*  904 */       int pos = LongLinkedOpenCustomHashSet.this.first;
/*  905 */       this.index = 1;
/*  906 */       while (pos != this.prev) {
/*  907 */         pos = (int)LongLinkedOpenCustomHashSet.this.link[pos];
/*  908 */         this.index++;
/*      */       } 
/*      */     }
/*      */ 
...
/*      */       { 
/*  931 */         this.index--;
/*  932 */         this.prev = (int)(LongLinkedOpenCustomHashSet.this.link[this.curr] >>> 32L); }
/*  933 */       else { this.next = (int)LongLinkedOpenCustomHashSet.this.link[this.curr]; }
/*  934 */        LongLinkedOpenCustomHashSet.this.size--;
/*      */ 
/*      */       
/*  937 */       if (this.prev == -1) { LongLinkedOpenCustomHashSet.this.first = this.next; }
/*  938 */       else { LongLinkedOpenCustomHashSet.this.link[this.prev] = LongLinkedOpenCustomHashSet.this.link[this.prev] ^ (LongLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFFL) & 0xFFFFFFFFL; }
/*  939 */        if (this.next == -1) { LongLinkedOpenCustomHashSet.this.last = this.prev; }
/*  940 */       else { LongLinkedOpenCustomHashSet.this.link[this.next] = LongLinkedOpenCustomHashSet.this.link[this.next] ^ (LongLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFFL) << 32L) & 0xFFFFFFFF00000000L; }
/*  941 */        int pos = this.curr;
/*  942 */       this.curr = -1;
/*  943 */       if (pos == LongLinkedOpenCustomHashSet.this.n) {
/*  944 */         LongLinkedOpenCustomHashSet.this.containsNull = false;
/*  945 */         LongLinkedOpenCustomHashSet.this.key[LongLinkedOpenCustomHashSet.this.n] = 0L;
/*      */       } else {
/*      */         
/*  948 */         long[] key = LongLinkedOpenCustomHashSet.this.key; while (true) {
/*      */           long curr;
/*      */           int last;
/*  951 */           pos = (last = pos) + 1 & LongLinkedOpenCustomHashSet.this.mask;
/*      */           while (true) {
/*  953 */             if ((curr = key[pos]) == 0L) {
/*  954 */               key[last] = 0L;
/*      */               return;
/*      */             } 
/*  957 */             int slot = HashCommon.mix(LongLinkedOpenCustomHashSet.this.strategy.hashCode(curr)) & LongLinkedOpenCustomHashSet.this.mask;
/*  958 */             if ((last <= pos) ? (last >= slot || slot > pos) : (last >= slot && slot > pos))
/*  959 */               break;  pos = pos + 1 & LongLinkedOpenCustomHashSet.this.mask;
/*      */           } 
/*  961 */           key[last] = curr;
/*  962 */           if (this.next == pos) this.next = last; 
/*  963 */           if (this.prev == pos) this.prev = last; 
/*  964 */           LongLinkedOpenCustomHashSet.this.fixPointers(pos, last);
/*      */         } 
/*      */       } 
/*      */     }
/*      */   }
...
/* 1011 */     return LongSpliterators.asSpliterator(iterator(), Size64.sizeOf(this), 337);
/*      */   }
/*      */ 
/*      */ 
/*      */   
...
/* 1084 */     long[] key = this.key;
/* 1085 */     int mask = newN - 1;
/* 1086 */     long[] newKey = new long[newN + 1];
...
/* 1092 */       int pos; if (this.strategy.equals(key[i], 0L)) { pos = newN; }
/*      */       else
/* 1094 */       { pos = HashCommon.mix(this.strategy.hashCode(key[i])) & mask;
/* 1095 */         for (; newKey[pos] != 0L; pos = pos + 1 & mask); }
/*      */       
...
/*      */     try {
/* 1136 */       c = (LongLinkedOpenCustomHashSet)super.clone();
/* 1137 */     } catch (CloneNotSupportedException cantHappen) {
/* 1138 */       throw new InternalError();
/*      */     } 
/* 1140 */     c.key = (long[])this.key.clone();
...
/* 1169 */     LongIterator i = iterator();
/* 1170 */     s.defaultWriteObject();
/* 1171 */     for (int j = this.size; j-- != 0; s.writeLong(i.nextLong()));
/*      */   }
/*      */   
...
/* 1185 */       int pos; long k = s.readLong();
/* 1186 */       if (this.strategy.equals(k, 0L))
/* 1187 */       { pos = this.n;
/* 1188 */         this.containsNull = true; }
/*      */       
/* 1190 */       else if (key[pos = HashCommon.mix(this.strategy.hashCode(k)) & this.mask] != 0L) { while (key[pos = pos + 1 & this.mask] != 0L); }
/*      */       
/* 1192 */       key[pos] = k;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\i\\unimi\dsi\fastutil\longs\LongLinkedOpenCustomHashSet.class
