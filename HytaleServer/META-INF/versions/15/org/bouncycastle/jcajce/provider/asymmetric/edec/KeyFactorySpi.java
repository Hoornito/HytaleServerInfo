...
public class KeyFactorySpi extends BaseKeyFactorySpi implements AsymmetricKeyInfoConverter {
  ...
  protected KeySpec engineGetKeySpec(Key paramKey, Class paramClass) throws InvalidKeySpecException {
    ...
    } else if (paramClass.isAssignableFrom(EdECPrivateKeySpec.class)) {
      if (paramKey instanceof EdECPrivateKey) {
        Optional<byte[]> optional = ((EdECPrivateKey)paramKey).getBytes();
        if (optional.isPresent())
          return (KeySpec)new EdECPrivateKeySpec(((EdECPrivateKey)paramKey).getParams(), optional.get()); 
        throw new IllegalArgumentException("no byte[] data associated with key");
      } 
    } else if (paramClass.isAssignableFrom(EdECPublicKeySpec.class) && paramKey instanceof EdECPublicKey) {
      return (KeySpec)new EdECPublicKeySpec(((EdECPublicKey)paramKey).getParams(), ((EdECPublicKey)paramKey).getPoint());
    } 
  ...
  protected PrivateKey engineGeneratePrivate(KeySpec paramKeySpec) throws InvalidKeySpecException {
    ...
    if (paramKeySpec instanceof EdECPrivateKeySpec) {
      EdECPrivateKeySpec edECPrivateKeySpec = (EdECPrivateKeySpec)paramKeySpec;
      try {
        Ed25519PrivateKeyParameters ed25519PrivateKeyParameters;
        if (NamedParameterSpec.ED448.getName().equalsIgnoreCase(edECPrivateKeySpec.getParams().getName())) {
          Ed448PrivateKeyParameters ed448PrivateKeyParameters = SignatureSpi.getEd448PrivateKey(edECPrivateKeySpec.getBytes());
        } else if (NamedParameterSpec.ED25519.getName().equalsIgnoreCase(edECPrivateKeySpec.getParams().getName())) {
          ed25519PrivateKeyParameters = SignatureSpi.getEd25519PrivateKey(edECPrivateKeySpec.getBytes());
        } else {
          throw new InvalidKeySpecException("unrecognized named parameters: " + edECPrivateKeySpec.getParams().getName());
        } 
        return (PrivateKey)new BC15EdDSAPrivateKey((AsymmetricKeyParameter)ed25519PrivateKeyParameters);
  ...
  protected PublicKey engineGeneratePublic(KeySpec paramKeySpec) throws InvalidKeySpecException {
    if (paramKeySpec instanceof X509EncodedKeySpec) {
      ...
      if (paramKeySpec instanceof EdECPublicKeySpec) {
        EdECPublicKeySpec edECPublicKeySpec = (EdECPublicKeySpec)paramKeySpec;
        try {
          Ed25519PublicKeyParameters ed25519PublicKeyParameters;
          if (NamedParameterSpec.ED448.getName().equalsIgnoreCase(edECPublicKeySpec.getParams().getName())) {
            Ed448PublicKeyParameters ed448PublicKeyParameters = SignatureSpi.getEd448PublicKey(edECPublicKeySpec.getPoint());
          } else if (NamedParameterSpec.ED25519.getName().equalsIgnoreCase(edECPublicKeySpec.getParams().getName())) {
            ed25519PublicKeyParameters = SignatureSpi.getEd25519PublicKey(edECPublicKeySpec.getPoint());
          } else {
            throw new InvalidKeySpecException("unrecognized named parameters: " + edECPublicKeySpec.getParams().getName());
          } 
          return (PublicKey)new BC15EdDSAPublicKey((AsymmetricKeyParameter)ed25519PublicKeyParameters);
        } catch (InvalidKeyException invalidKeyException) {
          throw new InvalidKeySpecException(invalidKeyException.getMessage(), invalidKeyException);
        } 
      } 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\15\org\bouncycastle\jcajce\provider\asymmetric\edec\KeyFactorySpi.class
