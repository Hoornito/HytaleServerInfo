...
public class DERSequence extends ASN1Sequence {
  ...
  public DERSequence(ASN1Encodable paramASN1Encodable1, ASN1Encodable paramASN1Encodable2) {
    super(paramASN1Encodable1, paramASN1Encodable2);
  }
  ...
  void encode(ASN1OutputStream paramASN1OutputStream, boolean paramBoolean) throws IOException {
    ...
    if (this.contentsLength >= 0 || i > 16) {
      ...
      for (byte b = 0; b < i; b++) {
        ASN1Primitive aSN1Primitive = this.elements[b].toASN1Primitive().toDERObject();
        aSN1Primitive.encode((ASN1OutputStream)dEROutputStream, true);
      } 
    } else {
      int j = 0;
      ...
      for (b = 0; b < i; b++)
        arrayOfASN1Primitive[b].encode((ASN1OutputStream)dEROutputStream, true); 
    } 
  }
  
  ASN1BitString toASN1BitString() {
    return (ASN1BitString)new DERBitString(BERBitString.flattenBitStrings(getConstructedBitStrings()), false);
  }
  
  ASN1External toASN1External() {
    return (ASN1External)new DERExternal(this);
  }
  
  ASN1OctetString toASN1OctetString() {
    return (ASN1OctetString)new DEROctetString(BEROctetString.flattenOctetStrings(getConstructedOctetStrings()));
  }
  
  ASN1Set toASN1Set() {
    return (ASN1Set)new DLSet(false, toArrayInternal());
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\asn1\DERSequence.class
