...
class Poly {
  ...
  public Poly(MLDSAEngine paramMLDSAEngine) {
    this.engine = paramMLDSAEngine;
    this.symmetric = paramMLDSAEngine.GetSymmetric();
    this.polyUniformNBlocks = (768 + this.symmetric.stream128BlockBytes - 1) / this.symmetric.stream128BlockBytes;
  }
  
  void copyTo(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly) {
    System.arraycopy(this.coeffs, 0, paramPoly.coeffs, 0, 256);
  }
  ...
  public void uniformBlocks(byte[] paramArrayOfbyte, short paramShort) {
    ...
    int i;
    for (i = rejectUniform(this, 0, 256, arrayOfByte, j); i < 256; i += rejectUniform(this, i, 256 - i, arrayOfByte, j)) {
      int k = j % 3;
      for (byte b = 0; b < k; b++)
  ...
  private static int rejectUniform(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly, int paramInt1, int paramInt2, byte[] paramArrayOfbyte, int paramInt3) {
    byte b2 = 0;
    byte b1 = b2;
  ...
  public void uniformEta(byte[] paramArrayOfbyte, short paramShort) {
    ...
    for (int i = rejectEta(this, 0, 256, arrayOfByte, m, k); i < 256; i += rejectEta(this, i, 256 - i, arrayOfByte, this.symmetric.stream256BlockBytes, k))
      this.symmetric.stream256squeezeBlocks(arrayOfByte, 0, this.symmetric.stream256BlockBytes); 
  }
  
  private static int rejectEta(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly, int paramInt1, int paramInt2, byte[] paramArrayOfbyte, int paramInt3, int paramInt4) {
    byte b2 = 0;
    byte b1 = b2;
  ...
  public void pointwiseMontgomery(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly1, org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly2) {
    for (byte b = 0; b < 'Ā'; b++)
  ...
  public void pointwiseAccountMontgomery(PolyVecL paramPolyVecL1, PolyVecL paramPolyVecL2) {
    org.bouncycastle.pqc.crypto.mldsa.Poly poly = new org.bouncycastle.pqc.crypto.mldsa.Poly(this.engine);
  ...
  public void addPoly(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly) {
    for (byte b = 0; b < 'Ā'; b++)
  ...
  public void power2Round(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly) {
    Rounding.power2RoundAll(this.coeffs, paramPoly.coeffs);
  }
  ...
  private void unpackZ(byte[] paramArrayOfbyte) {
    int i = this.engine.getDilithiumGamma1();
    if (i == 131072) {
      for (byte b = 0; b < 64; b++) {
        ...
        setCoeffIndex(4 * b + 0, i - getCoeffIndex(4 * b + 0));
        setCoeffIndex(4 * b + 1, i - getCoeffIndex(4 * b + 1));
        setCoeffIndex(4 * b + 2, i - getCoeffIndex(4 * b + 2));
        setCoeffIndex(4 * b + 3, i - getCoeffIndex(4 * b + 3));
      } 
    } else if (i == 524288) {
      for (byte b = 0; b < ''; b++) {
  ...
  public void decompose(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly) {
    int i = this.engine.getDilithiumGamma2();
    for (byte b = 0; b < 'Ā'; b++) {
      int[] arrayOfInt = Rounding.decompose(getCoeffIndex(b), i);
      setCoeffIndex(b, arrayOfInt[1]);
      paramPoly.setCoeffIndex(b, arrayOfInt[0]);
    } 
  }
  
  void packW1(byte[] paramArrayOfbyte, int paramInt) {
    int i = this.engine.getDilithiumGamma2();
    if (i == 95232) {
      for (byte b = 0; b < 64; b++) {
        paramArrayOfbyte[paramInt + 3 * b + 0] = (byte)((byte)getCoeffIndex(4 * b + 0) | getCoeffIndex(4 * b + 1) << 6);
        paramArrayOfbyte[paramInt + 3 * b + 1] = (byte)((byte)(getCoeffIndex(4 * b + 1) >> 2) | getCoeffIndex(4 * b + 2) << 4);
        paramArrayOfbyte[paramInt + 3 * b + 2] = (byte)((byte)(getCoeffIndex(4 * b + 2) >> 4) | getCoeffIndex(4 * b + 3) << 2);
      } 
    } else if (this.engine.getDilithiumGamma2() == 261888) {
      for (byte b = 0; b < ''; b++)
        paramArrayOfbyte[paramInt + b] = (byte)(getCoeffIndex(2 * b + 0) | getCoeffIndex(2 * b + 1) << 4); 
    } 
  }
  
  public void challenge(byte[] paramArrayOfbyte, int paramInt1, int paramInt2) {
    int j = 0;
    byte[] arrayOfByte = new byte[this.symmetric.stream256BlockBytes];
    SHAKEDigest sHAKEDigest = new SHAKEDigest(256);
    sHAKEDigest.update(paramArrayOfbyte, paramInt1, paramInt2);
    ...
    for (i = 0; i < 256; i++)
      setCoeffIndex(i, 0); 
    i = 256 - this.engine.getDilithiumTau();
    while (i < 256) {
      while (true) {
  ...
  public void subtract(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly) {
    for (byte b = 0; b < 'Ā'; b++)
      setCoeffIndex(b, getCoeffIndex(b) - paramPoly.getCoeffIndex(b)); 
  }
  
  public int polyMakeHint(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly1, org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly2) {
    int i = 0;
    for (byte b = 0; b < 'Ā'; b++) {
  ...
  public void polyUseHint(org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly1, org.bouncycastle.pqc.crypto.mldsa.Poly paramPoly2) {
    for (byte b = 0; b < 'Ā'; b++)
      setCoeffIndex(b, Rounding.useHint(paramPoly1.getCoeffIndex(b), paramPoly2.getCoeffIndex(b), this.engine.getDilithiumGamma2())); 
  }
  
  public void zPack(byte[] paramArrayOfbyte, int paramInt) {
    int i = this.engine.getDilithiumGamma1();
    if (i == 131072) {
      for (byte b = 0; b < 64; b++) {
        int j = i - getCoeffIndex(4 * b + 0);
        int k = i - getCoeffIndex(4 * b + 1);
        int m = i - getCoeffIndex(4 * b + 2);
        int n = i - getCoeffIndex(4 * b + 3);
        paramArrayOfbyte[paramInt + 9 * b + 0] = (byte)j;
        paramArrayOfbyte[paramInt + 9 * b + 1] = (byte)(j >> 8);
        paramArrayOfbyte[paramInt + 9 * b + 2] = (byte)((byte)(j >> 16) | k << 2);
        paramArrayOfbyte[paramInt + 9 * b + 3] = (byte)(k >> 6);
        paramArrayOfbyte[paramInt + 9 * b + 4] = (byte)((byte)(k >> 14) | m << 4);
        paramArrayOfbyte[paramInt + 9 * b + 5] = (byte)(m >> 4);
        paramArrayOfbyte[paramInt + 9 * b + 6] = (byte)((byte)(m >> 12) | n << 6);
        paramArrayOfbyte[paramInt + 9 * b + 7] = (byte)(n >> 2);
        paramArrayOfbyte[paramInt + 9 * b + 8] = (byte)(n >> 10);
      } 
    } else if (i == 524288) {
      for (byte b = 0; b < ''; b++) {
        int j = i - getCoeffIndex(2 * b + 0);
        int k = i - getCoeffIndex(2 * b + 1);
        paramArrayOfbyte[paramInt + 5 * b + 0] = (byte)j;
        paramArrayOfbyte[paramInt + 5 * b + 1] = (byte)(j >> 8);
        paramArrayOfbyte[paramInt + 5 * b + 2] = (byte)((byte)(j >> 16) | k << 4);
        paramArrayOfbyte[paramInt + 5 * b + 3] = (byte)(k >> 4);
        paramArrayOfbyte[paramInt + 5 * b + 4] = (byte)(k >> 12);
      } 
    } else {
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypto\mldsa\Poly.class
