...
class GFCalculator {
  ...
  static int mult(int paramInt1, int paramInt2) {
    int i = Utils.toUnsigned16Bits(-paramInt1 >> 31);
    i &= Utils.toUnsigned16Bits(-paramInt2 >> 31);
    return Utils.toUnsigned16Bits(i & exp[mod(log[paramInt1] + log[paramInt2])]);
  }
  
  static int mod(int paramInt) {
    int i = Utils.toUnsigned16Bits(paramInt - 255);
    int j = Utils.toUnsigned8bits(-(i >> 15));
    return Utils.toUnsigned16Bits(i + (j & 0xFF));
  }
  
  static int inverse(int paramInt) {
    int i = Utils.toUnsigned16Bits(-paramInt >> 31);
    return i & exp[255 - log[paramInt]];
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypto\hqc\GFCalculator.class
