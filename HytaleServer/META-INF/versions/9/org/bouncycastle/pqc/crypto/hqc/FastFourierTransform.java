...
class FastFourierTransform {
  ...
  static void computeRadixBig(int[] paramArrayOfint1, int[] paramArrayOfint2, int[] paramArrayOfint3, int paramInt1, int paramInt2) {
    ...
    i <<= paramInt1 - 2;
    int j = 1 << paramInt2 - 2;
    int[] arrayOfInt1 = new int[2 * j + 1];
    int[] arrayOfInt2 = new int[2 * j + 1];
  ...
  static void fastFourierTransformGetError(byte[] paramArrayOfbyte, int[] paramArrayOfint1, int paramInt, int[] paramArrayOfint2) {
    ...
    paramArrayOfbyte[0] = (byte)(paramArrayOfbyte[0] ^ 0x1 ^ Utils.toUnsigned16Bits(-paramArrayOfint1[paramInt] >> 15));
    for (byte b2 = 1; b2 < paramInt; b2++) {
      int i = c - paramArrayOfint2[arrayOfInt2[b2]];
      paramArrayOfbyte[i] = (byte)(paramArrayOfbyte[i] ^ 0x1 ^ Math.abs(-paramArrayOfint1[b2] >> 15));
      i = c - paramArrayOfint2[arrayOfInt2[b2] ^ 0x1];
      paramArrayOfbyte[i] = (byte)(paramArrayOfbyte[i] ^ 0x1 ^ Math.abs(-paramArrayOfint1[paramInt + b2] >> 15));
    } 
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypto\hqc\FastFourierTransform.class
