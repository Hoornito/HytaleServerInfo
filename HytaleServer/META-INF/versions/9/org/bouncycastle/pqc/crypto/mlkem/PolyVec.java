...
class PolyVec {
  ...
  public PolyVec(MLKEMEngine paramMLKEMEngine) {
    ...
    for (byte b = 0; b < this.kyberK; b++)
      this.vec[b] = new Poly(paramMLKEMEngine); 
  }
  ...
  public static void pointwiseAccountMontgomery(Poly paramPoly, org.bouncycastle.pqc.crypto.mlkem.PolyVec paramPolyVec1, org.bouncycastle.pqc.crypto.mlkem.PolyVec paramPolyVec2, MLKEMEngine paramMLKEMEngine) {
    Poly poly = new Poly(paramMLKEMEngine);
    Poly.baseMultMontgomery(paramPoly, paramPolyVec1.getVectorIndex(0), paramPolyVec2.getVectorIndex(0));
    for (byte b = 1; b < paramMLKEMEngine.getKyberK(); b++) {
  ...
  public void addPoly(org.bouncycastle.pqc.crypto.mlkem.PolyVec paramPolyVec) {
    for (byte b = 0; b < this.kyberK; b++)
  ...
  public String toString() {
    ...
    for (byte b = 0; b < this.kyberK; b++) {
      stringBuilder.append(this.vec[b].toString());
      if (b != this.kyberK - 1)
        stringBuilder.append(", "); 
    } 
    stringBuilder.append("]");
  ...
  static int checkModulus(MLKEMEngine paramMLKEMEngine, byte[] paramArrayOfbyte) {
    int i = -1;
    byte b = 0;
    int j = paramMLKEMEngine.getKyberK();
    while (b < j) {
      i &= Poly.checkModulus(paramArrayOfbyte, b * 384);
      b++;
    } 
    return i;
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypto\mlkem\PolyVec.class
