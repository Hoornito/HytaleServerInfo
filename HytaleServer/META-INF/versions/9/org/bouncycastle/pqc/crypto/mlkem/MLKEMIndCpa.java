...
class MLKEMIndCpa {
  ...
  public MLKEMIndCpa(MLKEMEngine paramMLKEMEngine) {
    this.engine = paramMLKEMEngine;
    this.kyberK = paramMLKEMEngine.getKyberK();
    this.indCpaPublicKeyBytes = paramMLKEMEngine.getKyberPublicKeyBytes();
    this.polyVecBytes = paramMLKEMEngine.getKyberPolyVecBytes();
    this.indCpaBytes = paramMLKEMEngine.getKyberIndCpaBytes();
    this.polyVecCompressedBytes = paramMLKEMEngine.getKyberPolyVecCompressedBytes();
  ...
  byte[][] generateKeyPair(byte[] paramArrayOfbyte) {
    ...
    this.symmetric.hash_g(arrayOfByte1, Arrays.append(paramArrayOfbyte, (byte)this.kyberK));
    byte[] arrayOfByte2 = new byte[32];
    byte[] arrayOfByte3 = new byte[32];
    System.arraycopy(arrayOfByte1, 0, arrayOfByte2, 0, 32);
    System.arraycopy(arrayOfByte1, 32, arrayOfByte3, 0, 32);
    byte b = 0;
    PolyVec[] arrayOfPolyVec = new PolyVec[this.kyberK];
    byte b1;
    for (b1 = 0; b1 < this.kyberK; b1++)
      arrayOfPolyVec[b1] = new PolyVec(this.engine); 
    generateMatrix(arrayOfPolyVec, arrayOfByte2, false);
    for (b1 = 0; b1 < this.kyberK; b1++) {
      polyVec1.getVectorIndex(b1).getEta1Noise(arrayOfByte3, b);
      b = (byte)(b + 1);
    } 
    for (b1 = 0; b1 < this.kyberK; b1++) {
      polyVec3.getVectorIndex(b1).getEta1Noise(arrayOfByte3, b);
      b = (byte)(b + 1);
    } 
    polyVec1.polyVecNtt();
    polyVec3.polyVecNtt();
    ...
    polyVec2.reducePoly();
    return new byte[][] { packPublicKey(polyVec2, arrayOfByte2), packSecretKey(polyVec1) };
  }
  ...
  public byte[] encrypt(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2, byte[] paramArrayOfbyte3) {
    ...
    byte[] arrayOfByte = unpackPublicKey(polyVec2, paramArrayOfbyte1);
    poly3.fromMsg(paramArrayOfbyte2);
    byte b;
  ...
  public byte[] decrypt(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2) {
    null = new byte[MLKEMEngine.getKyberIndCpaMsgBytes()];
    PolyVec polyVec1 = new PolyVec(this.engine);
    PolyVec polyVec2 = new PolyVec(this.engine);
    Poly poly1 = new Poly(this.engine);
    Poly poly2 = new Poly(this.engine);
    unpackCipherText(polyVec1, poly1, paramArrayOfbyte2);
    unpackSecretKey(polyVec2, paramArrayOfbyte1);
    polyVec1.polyVecNtt();
    PolyVec.pointwiseAccountMontgomery(poly2, polyVec2, polyVec1, this.engine);
    poly2.polyInverseNttToMont();
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypto\mlkem\MLKEMIndCpa.class
