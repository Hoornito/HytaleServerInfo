...
public class SubjectPublicKeyInfoFactory {
  public static SubjectPublicKeyInfo createSubjectPublicKeyInfo(AsymmetricKeyParameter paramAsymmetricKeyParameter) throws IOException {
    ...
    if (paramAsymmetricKeyParameter instanceof SLHDSAPublicKeyParameters) {
      SLHDSAPublicKeyParameters sLHDSAPublicKeyParameters = (SLHDSAPublicKeyParameters)paramAsymmetricKeyParameter;
      byte[] arrayOfByte = sLHDSAPublicKeyParameters.getEncoded();
    ...
    if (paramAsymmetricKeyParameter instanceof MLKEMPublicKeyParameters) {
    ...
    if (paramAsymmetricKeyParameter instanceof MLDSAPublicKeyParameters) {
      MLDSAPublicKeyParameters mLDSAPublicKeyParameters = (MLDSAPublicKeyParameters)paramAsymmetricKeyParameter;
    ...
    if (paramAsymmetricKeyParameter instanceof MayoPublicKeyParameters) {
      MayoPublicKeyParameters mayoPublicKeyParameters = (MayoPublicKeyParameters)paramAsymmetricKeyParameter;
      byte[] arrayOfByte = mayoPublicKeyParameters.getEncoded();
      AlgorithmIdentifier algorithmIdentifier = new AlgorithmIdentifier(Utils.mayoOidLookup(mayoPublicKeyParameters.getParameters()));
      return new SubjectPublicKeyInfo(algorithmIdentifier, (ASN1Encodable)new DEROctetString(arrayOfByte));
    } 
    if (paramAsymmetricKeyParameter instanceof SnovaPublicKeyParameters) {
      SnovaPublicKeyParameters snovaPublicKeyParameters = (SnovaPublicKeyParameters)paramAsymmetricKeyParameter;
      byte[] arrayOfByte = snovaPublicKeyParameters.getEncoded();
      AlgorithmIdentifier algorithmIdentifier = new AlgorithmIdentifier(Utils.snovaOidLookup(snovaPublicKeyParameters.getParameters()));
      return new SubjectPublicKeyInfo(algorithmIdentifier, (ASN1Encodable)new DEROctetString(arrayOfByte));
    } 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\pqc\crypt\\util\SubjectPublicKeyInfoFactory.class
