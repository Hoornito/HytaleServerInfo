...
public abstract class ECPoint {
  ...
  public String toString() {
    ...
    stringBuilder.append(getRawXCoord());
    stringBuilder.append(',');
    stringBuilder.append(getRawYCoord());
    for (byte b = 0; b < this.zs.length; b++) {
      stringBuilder.append(',');
      stringBuilder.append(this.zs[b]);
    } 
    stringBuilder.append(')');
  ...
  public int getEncodedLength(boolean paramBoolean) {
    return isInfinity() ? 1 : (paramBoolean ? (1 + getXCoord().getEncodedLength()) : (1 + getXCoord().getEncodedLength() + getYCoord().getEncodedLength()));
  }
  
  public void encodeTo(boolean paramBoolean, byte[] paramArrayOfbyte, int paramInt) {
    if (isInfinity()) {
      paramArrayOfbyte[paramInt] = 0;
      return;
    } 
    org.bouncycastle.math.ec.ECPoint eCPoint = normalize();
    ECFieldElement eCFieldElement1 = eCPoint.getXCoord();
    ECFieldElement eCFieldElement2 = eCPoint.getYCoord();
    if (paramBoolean) {
      paramArrayOfbyte[paramInt] = (byte)(eCPoint.getCompressionYTilde() ? 3 : 2);
      eCFieldElement1.encodeTo(paramArrayOfbyte, paramInt + 1);
      return;
    } 
    paramArrayOfbyte[paramInt] = 4;
    eCFieldElement1.encodeTo(paramArrayOfbyte, paramInt + 1);
    eCFieldElement2.encodeTo(paramArrayOfbyte, paramInt + 1 + eCFieldElement1.getEncodedLength());
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\math\ec\ECPoint.class
