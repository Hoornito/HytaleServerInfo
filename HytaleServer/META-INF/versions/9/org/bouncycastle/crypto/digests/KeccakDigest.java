...
public class KeccakDigest implements ExtendedDigest {
  ...
  public KeccakDigest(CryptoServicePurpose paramCryptoServicePurpose) {
    this(288, paramCryptoServicePurpose);
  }
  ...
  public KeccakDigest(int paramInt, CryptoServicePurpose paramCryptoServicePurpose) {
  ...
  public KeccakDigest(org.bouncycastle.crypto.digests.KeccakDigest paramKeccakDigest) {
    this.purpose = paramKeccakDigest.purpose;
  ...
  protected KeccakDigest(byte[] paramArrayOfbyte) {
    this.purpose = getCryptoServicePurpose(paramArrayOfbyte[0]);
    int i = 1;
    Pack.bigEndianToLong(paramArrayOfbyte, i, this.state, 0, this.state.length);
    i += this.state.length * 8;
    System.arraycopy(paramArrayOfbyte, i, this.dataQueue, 0, this.dataQueue.length);
    i += this.dataQueue.length;
    this.rate = Pack.bigEndianToInt(paramArrayOfbyte, i);
    i += 4;
    this.bitsInQueue = Pack.bigEndianToInt(paramArrayOfbyte, i);
    i += 4;
    this.fixedOutputLength = Pack.bigEndianToInt(paramArrayOfbyte, i);
    i += 4;
    this.squeezing = (paramArrayOfbyte[i] != 0);
  }
  
  protected KeccakDigest(byte[] paramArrayOfbyte, CryptoServicePurpose paramCryptoServicePurpose) {
    this(paramArrayOfbyte);
    if (!this.purpose.equals(paramCryptoServicePurpose))
      throw new IllegalStateException("digest encoded for a different purpose"); 
  }
  
  private static CryptoServicePurpose getCryptoServicePurpose(byte paramByte) {
    CryptoServicePurpose[] arrayOfCryptoServicePurpose = CryptoServicePurpose.values();
    return arrayOfCryptoServicePurpose[paramByte];
  }
  
  protected void copyIn(org.bouncycastle.crypto.digests.KeccakDigest paramKeccakDigest) {
    if (this.purpose != paramKeccakDigest.purpose)
  ...
  protected CryptoServiceProperties cryptoServiceProperties() {
    return Utils.getDefaultProperties((Digest)this, getDigestSize() * 8, this.purpose);
  }
  
  protected byte[] getEncodedState(byte[] paramArrayOfbyte) {
    paramArrayOfbyte[0] = (byte)this.purpose.ordinal();
    int i = 1;
    for (byte b = 0; b != this.state.length; b++) {
      Pack.longToBigEndian(this.state[b], paramArrayOfbyte, i);
      i += true;
    } 
    System.arraycopy(this.dataQueue, 0, paramArrayOfbyte, i, this.dataQueue.length);
    i += this.dataQueue.length;
    Pack.intToBigEndian(this.rate, paramArrayOfbyte, i);
    i += 4;
    Pack.intToBigEndian(this.bitsInQueue, paramArrayOfbyte, i);
    i += 4;
    Pack.intToBigEndian(this.fixedOutputLength, paramArrayOfbyte, i);
    i += 4;
    paramArrayOfbyte[i] = this.squeezing ? 1 : 0;
    return paramArrayOfbyte;
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\META-INF\versions\9\org\bouncycastle\crypto\digests\KeccakDigest.class
