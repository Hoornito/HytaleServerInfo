...
public class IETFUtils {
  private static String unescape(String paramString) {
    ...
    for (byte b2 = b1; b2 != paramString.length(); b2++) {
      ...
      if (c1 == '"') {
        ...
        if (bool1 && isHexDigit(c1)) {
          if (c) {
            stringBuilder.append((char)(convertHex(c) * 16 + convertHex(c1)));
            bool1 = false;
    ...
    if (stringBuilder.length() > 0)
      while (stringBuilder.charAt(stringBuilder.length() - 1) == ' ' && i != stringBuilder.length() - 1)
  ...
  public static void appendRDN(StringBuilder paramStringBuilder, RDN paramRDN, Hashtable paramHashtable) {
    if (paramRDN.isMultiValued()) {
      ...
      for (byte b = 0; b != arrayOfAttributeTypeAndValue.length; b++) {
        ...
        appendTypeAndValue(paramStringBuilder, arrayOfAttributeTypeAndValue[b], paramHashtable);
      } 
    } else if (paramRDN.getFirst() != null) {
      appendTypeAndValue(paramStringBuilder, paramRDN.getFirst(), paramHashtable);
    } 
  }
  ...
  public static void appendTypeAndValue(StringBuilder paramStringBuilder, AttributeTypeAndValue paramAttributeTypeAndValue, Hashtable paramHashtable) {
    String str = (String)paramHashtable.get(paramAttributeTypeAndValue.getType());
    if (str != null) {
      paramStringBuilder.append(str);
    } else {
      paramStringBuilder.append(paramAttributeTypeAndValue.getType().getId());
    } 
    paramStringBuilder.append('=');
    paramStringBuilder.append(valueToString(paramAttributeTypeAndValue.getValue()));
  }
  ...
  public static String valueToString(ASN1Encodable paramASN1Encodable) {
    ...
    if (paramASN1Encodable instanceof ASN1String && !(paramASN1Encodable instanceof org.bouncycastle.asn1.ASN1UniversalString)) {
      ...
      try {
        stringBuilder.append('#');
        stringBuilder.append(Hex.toHexString(paramASN1Encodable.toASN1Primitive().getEncoded("DER")));
    ...
    byte b = 0;
    if (stringBuilder.length() > 0)
      while (stringBuilder.length() > b && stringBuilder.charAt(b) == ' ') {
        stringBuilder.insert(b, "\\");
        b += 2;
      }  
    for (int k = stringBuilder.length() - 1; k >= b && stringBuilder.charAt(k) == ' '; k--)
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\asn1\x500\style\IETFUtils.class
