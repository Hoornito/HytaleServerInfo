...
public class BaseBlockCipher extends BaseWrapCipher implements PBE {
  ...
  protected BaseBlockCipher(int paramInt, BlockCipherProvider paramBlockCipherProvider) {
    this.baseEngine = paramBlockCipherProvider.get();
    this.engineProvider = paramBlockCipherProvider;
  ...
  protected BaseBlockCipher(AEADBlockCipher paramAEADBlockCipher) {
    this(0, paramAEADBlockCipher);
  }
  
  protected BaseBlockCipher(int paramInt, AEADBlockCipher paramAEADBlockCipher) {
    this.keySizeInBits = paramInt;
  ...
  protected BaseBlockCipher(AEADBlockCipher paramAEADBlockCipher, boolean paramBoolean, int paramInt) {
    this(0, paramAEADBlockCipher, paramBoolean, paramInt);
  }
  
  protected BaseBlockCipher(int paramInt1, AEADBlockCipher paramAEADBlockCipher, boolean paramBoolean, int paramInt2) {
  ...
  protected BaseBlockCipher(int paramInt1, BlockCipher paramBlockCipher, int paramInt2) {
    this.keySizeInBits = paramInt1;
    this.baseEngine = paramBlockCipher;
    this.fixedIv = true;
    this.cipher = new BufferedGenericBlockCipher(paramBlockCipher);
    this.ivLength = paramInt2 / 8;
  }
  ...
  protected BaseBlockCipher(int paramInt1, BufferedBlockCipher paramBufferedBlockCipher, int paramInt2) {
    this.keySizeInBits = paramInt1;
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (this.scheme == 2 || paramKey instanceof org.bouncycastle.jcajce.PKCS12Key) {
      ...
      if (parametersWithIV2 instanceof ParametersWithIV)
        this.ivParam = parametersWithIV2; 
    } else if (paramKey instanceof PBKDF2Key) {
      PBKDF2Key pBKDF2Key = (PBKDF2Key)paramKey;
      if (paramAlgorithmParameterSpec instanceof PBEParameterSpec)
        this.pbeSpec = (PBEParameterSpec)paramAlgorithmParameterSpec; 
      ...
      parametersWithIV2 = (ParametersWithIV)PBE.Util.makePBEParameters(pBKDF2Key.getEncoded(), 1, 9, this.keySizeInBits, 0, this.pbeSpec, this.cipher.getAlgorithmName());
      if (parametersWithIV2 instanceof ParametersWithIV)
        this.ivParam = parametersWithIV2; 
    ...
    AlgorithmParameterSpec algorithmParameterSpec = paramAlgorithmParameterSpec;
    if (paramAlgorithmParameterSpec instanceof PBEParameterSpec) {
      algorithmParameterSpec = ((PBEParameterSpec)paramAlgorithmParameterSpec).getParameterSpec();
      if (algorithmParameterSpec instanceof IvParameterSpec && (((IvParameterSpec)algorithmParameterSpec).getIV()).length == 0)
    ...
    if (algorithmParameterSpec instanceof AEADParameterSpec) {
      ...
      ParametersWithSBox parametersWithSBox = new ParametersWithSBox((CipherParameters)new KeyParameter(paramKey.getEncoded()), ((GOST28147ParameterSpec)algorithmParameterSpec).getSBox());
      ...
      RC2Parameters rC2Parameters = new RC2Parameters(paramKey.getEncoded(), ((RC2ParameterSpec)algorithmParameterSpec).getEffectiveKeyBits());
      ...
      RC5Parameters rC5Parameters = new RC5Parameters(paramKey.getEncoded(), ((RC5ParameterSpec)algorithmParameterSpec).getRounds());
      ...
      if (algorithmParameterSpec instanceof FPEParameterSpec) {
        ...
        aEADParameters = this.aeadParams = GcmSpecUtil.extractAeadParameters(keyParameter, algorithmParameterSpec);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\symmetri\\util\BaseBlockCipher.class
