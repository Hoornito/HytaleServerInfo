...
public class BCECGOST3410PrivateKey implements ECPrivateKey, ECPrivateKey, PKCS12BagAttributeCarrier, ECPointEncoder {
  ...
  private void populateFromPrivKeyInfo(PrivateKeyInfo paramPrivateKeyInfo) throws IOException {
    ...
    if (aSN1Primitive instanceof ASN1Sequence && (ASN1Sequence.getInstance(aSN1Primitive).size() == 2 || ASN1Sequence.getInstance(aSN1Primitive).size() == 3)) {
      ...
      this.ecSpec = (ECParameterSpec)new ECNamedCurveSpec(ECGOST3410NamedCurves.getName(gOST3410PublicKeyAlgParameters.getPublicKeyParamSet()), ellipticCurve, EC5Util.convertPoint(eCNamedCurveParameterSpec.getG()), eCNamedCurveParameterSpec.getN(), eCNamedCurveParameterSpec.getH());
      ASN1Encodable aSN1Encodable1 = paramPrivateKeyInfo.parsePrivateKey();
      if (aSN1Encodable1 instanceof ASN1Integer) {
        this.d = ASN1Integer.getInstance(aSN1Encodable1).getPositiveValue();
      } else {
        byte[] arrayOfByte1 = ASN1OctetString.getInstance(aSN1Encodable1).getOctets();
      ...
      if (x962Parameters.isNamedCurve()) {
        ...
        String str = ECUtil.getCurveName(aSN1ObjectIdentifier);
        EllipticCurve ellipticCurve = EC5Util.convertCurve(x9ECParameters.getCurve(), x9ECParameters.getSeed());
        this.ecSpec = (ECParameterSpec)new ECNamedCurveSpec(str, ellipticCurve, EC5Util.convertPoint(x9ECParameters.getG()), x9ECParameters.getN(), x9ECParameters.getH());
  ...
  private ASN1BitString getPublicKeyDetails(BCECGOST3410PublicKey paramBCECGOST3410PublicKey) {
    try {
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\asymmetric\ecgost\BCECGOST3410PrivateKey.class
