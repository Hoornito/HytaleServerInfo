...
class MLKEMCipherSpi extends CipherSpi {
  ...
  MLKEMCipherSpi(String paramString) {
    this.algorithmName = paramString;
    this.mlkemParamters = null;
  }
  
  MLKEMCipherSpi(MLKEMParameters paramMLKEMParameters) {
    this.mlkemParamters = paramMLKEMParameters;
    this.algorithmName = paramMLKEMParameters.getName();
  }
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (paramInt == 3) {
      if (paramKey instanceof BCMLKEMPublicKey) {
        this.wrapKey = (BCMLKEMPublicKey)paramKey;
        this.kemGen = new MLKEMGenerator(CryptoServicesRegistrar.getSecureRandom(paramSecureRandom));
      } else {
      ...
      if (paramKey instanceof BCMLKEMPrivateKey) {
        this.unwrapKey = (BCMLKEMPrivateKey)paramKey;
      } else {
    ...
    if (this.mlkemParamters != null) {
      String str = MLKEMParameterSpec.fromName(this.mlkemParamters.getName()).getName();
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameters paramAlgorithmParameters, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (paramAlgorithmParameters != null)
      try {
        algorithmParameterSpec = paramAlgorithmParameters.getParameterSpec((Class)KTSParameterSpec.class);
      } catch (Exception exception) {
  ...
  protected Key engineUnwrap(byte[] paramArrayOfbyte, String paramString, int paramInt) throws InvalidKeyException, NoSuchAlgorithmException {
    ...
    try {
      ...
      arrayOfByte = mLKEMExtractor.extractSecret(Arrays.copyOfRange(paramArrayOfbyte, 0, mLKEMExtractor.getEncapsulationLength()));
      Wrapper wrapper = WrapUtil.getKeyUnwrapper(this.kemParameterSpec, arrayOfByte);
      byte[] arrayOfByte1 = Arrays.copyOfRange(paramArrayOfbyte, mLKEMExtractor.getEncapsulationLength(), paramArrayOfbyte.length);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\asymmetric\mlkem\MLKEMCipherSpi.class
