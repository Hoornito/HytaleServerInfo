...
public class BCDSTU4145PublicKey implements ECPublicKey, ECPublicKey, ECPointEncoder {
  ...
  public BCDSTU4145PublicKey(ECPublicKeySpec paramECPublicKeySpec) {
    ...
    this.ecPublicKey = new ECPublicKeyParameters(EC5Util.convertPoint(this.ecSpec, paramECPublicKeySpec.getW()), EC5Util.getDomainParameters(null, this.ecSpec));
  }
  ...
  private void populateFromPubKeyInfo(SubjectPublicKeyInfo paramSubjectPublicKeyInfo) {
    ...
    if (paramSubjectPublicKeyInfo.getAlgorithm().getAlgorithm().equals((ASN1Primitive)UAObjectIdentifiers.dstu4145le))
    ...
    if (aSN1Sequence.getObjectAt(0) instanceof org.bouncycastle.asn1.ASN1Integer) {
      ...
      eCParameterSpec = new ECParameterSpec(x9ECParameters.getCurve(), x9ECParameters.getG(), x9ECParameters.getN(), x9ECParameters.getH(), x9ECParameters.getSeed());
    } else {
    ...
    if (this.dstuParams != null) {
      ...
      if (this.dstuParams.isNamedCurve()) {
        String str = this.dstuParams.getNamedCurve().getId();
        this.ecSpec = (ECParameterSpec)new ECNamedCurveSpec(str, ellipticCurve, eCPoint, eCParameterSpec.getN(), eCParameterSpec.getH());
      } else {
        this.ecSpec = new ECParameterSpec(ellipticCurve, eCPoint, eCParameterSpec.getN(), eCParameterSpec.getH().intValue());
      } 
    } else {
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\asymmetric\dstu\BCDSTU4145PublicKey.class
