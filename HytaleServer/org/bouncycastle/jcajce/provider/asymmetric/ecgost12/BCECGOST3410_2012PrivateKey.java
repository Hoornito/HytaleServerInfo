...
public class BCECGOST3410_2012PrivateKey implements ECPrivateKey, ECPrivateKey, PKCS12BagAttributeCarrier, ECPointEncoder {
    ...
    if (aSN1Primitive instanceof ASN1Sequence && ASN1Sequence.getInstance(aSN1Primitive).size() <= 3) {
      ...
      if ((aSN1OctetString.getOctets()).length == 32 || (aSN1OctetString.getOctets()).length == 64) {
        ...
        if (aSN1Encodable instanceof ASN1Integer) {
          ...
          this.d = new BigInteger(1, Arrays.reverse(arrayOfByte));
        } 
      } 
    } else {
      ...
      if (x962Parameters.isNamedCurve()) {
        ...
        if (x9ECParameters == null) {
          X9ECParameters x9ECParameters1 = ECGOST3410NamedCurves.getByOIDX9(aSN1ObjectIdentifier);
          EllipticCurve ellipticCurve = EC5Util.convertCurve(x9ECParameters1.getCurve(), x9ECParameters1.getSeed());
          this.ecSpec = (ECParameterSpec)new ECNamedCurveSpec(ECGOST3410NamedCurves.getName(aSN1ObjectIdentifier), ellipticCurve, EC5Util.convertPoint(x9ECParameters1.getG()), x9ECParameters1.getN(), x9ECParameters1.getH());
        } else {
  ...
  private ASN1BitString getPublicKeyDetails(BCECGOST3410_2012PublicKey paramBCECGOST3410_2012PublicKey) {
    SubjectPublicKeyInfo subjectPublicKeyInfo = SubjectPublicKeyInfo.getInstance(paramBCECGOST3410_2012PublicKey.getEncoded());
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\asymmetric\ecgost12\BCECGOST3410_2012PrivateKey.class
