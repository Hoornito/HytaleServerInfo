...
public class GMCipherSpi extends CipherSpi {
  ...
  public byte[] engineDoFinal(byte[] paramArrayOfbyte, int paramInt1, int paramInt2) throws IllegalBlockSizeException, BadPaddingException {
    ...
    try {
      if (this.state == 1 || this.state == 3)
        try {
          this.engine.init(true, (CipherParameters)new ParametersWithRandom((CipherParameters)this.key, this.random));
      ...
      if (this.state == 2 || this.state == 4)
        try {
          this.engine.init(false, (CipherParameters)this.key);
          return this.engine.processBlock(this.buffer.getBuf(), 0, this.buffer.size());
        } catch (Exception exception) {
          throw new BadBlockException("unable to process block", exception);
        }  
  ...
  public static class SM2withSha256 extends GMCipherSpi {
    public SM2withSha256() {
      super(new SM2Engine((Digest)SHA256Digest.newInstance()));
    }
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jcajce\provider\asymmetric\ec\GMCipherSpi.class
