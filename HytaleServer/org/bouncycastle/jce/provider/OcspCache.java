...
class OcspCache {
  ...
  static OCSPResponse getOcspResponse(CertID paramCertID, PKIXCertRevocationCheckerParameters paramPKIXCertRevocationCheckerParameters, URI paramURI, X509Certificate paramX509Certificate, List<Extension> paramList, JcaJceHelper paramJcaJceHelper) throws CertPathValidatorException {
    ...
    if (map != null) {
      ...
      if (oCSPResponse != null) {
        ...
        boolean bool = isCertIDFoundAndCurrent(basicOCSPResponse, paramPKIXCertRevocationCheckerParameters.getValidDate(), paramCertID);
        if (bool)
          return oCSPResponse; 
    ...
    for (byte b = 0; b != list.size(); b++) {
      ...
      if (OCSPObjectIdentifiers.id_pkix_ocsp_nonce.equals((ASN1Primitive)aSN1ObjectIdentifier))
        arrayOfByte = Arrays.clone(dEROctetString.getOctets()); 
      aSN1EncodableVector2.add((ASN1Encodable)new Extension(aSN1ObjectIdentifier, extension.isCritical(), (ASN1OctetString)dEROctetString));
    } 
    ...
    try {
      ...
      if (0 == oCSPResponse.getResponseStatus().getIntValue()) {
        ...
        if (responseBytes.getResponseType().equals((ASN1Primitive)OCSPObjectIdentifiers.id_pkix_ocsp_basic)) {
          ...
          bool = (ProvOcspRevocationChecker.validatedOcspResponse(basicOCSPResponse, paramPKIXCertRevocationCheckerParameters, arrayOfByte, paramX509Certificate, paramJcaJceHelper) && isCertIDFoundAndCurrent(basicOCSPResponse, paramPKIXCertRevocationCheckerParameters.getValidDate(), paramCertID)) ? true : false;
        } 
        if (!bool)
  ...
  private static boolean isCertIDFoundAndCurrent(BasicOCSPResponse paramBasicOCSPResponse, Date paramDate, CertID paramCertID) {
    ResponseData responseData = ResponseData.getInstance(paramBasicOCSPResponse.getTbsResponseData());
    ...
    for (byte b = 0; b != aSN1Sequence.size(); b++) {
      ...
      if (paramCertID.equals(singleResponse.getCertID())) {
        ...
        try {
          if (aSN1GeneralizedTime != null && paramDate.after(aSN1GeneralizedTime.getDate()))
            return false; 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jce\provider\OcspCache.class
