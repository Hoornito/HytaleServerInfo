...
class RFC3281CertPathUtilities {
  ...
  protected static void checkCRLs(X509AttributeCertificate paramX509AttributeCertificate, PKIXExtendedParameters paramPKIXExtendedParameters, Date paramDate1, Date paramDate2, X509Certificate paramX509Certificate, List paramList, JcaJceHelper paramJcaJceHelper) throws CertPathValidatorException {
    if (paramPKIXExtendedParameters.isRevocationEnabled())
      if (paramX509AttributeCertificate.getExtensionValue(NO_REV_AVAIL) == null) {
        ...
        try {
          arrayList.addAll(CertPathValidatorUtilities.getAdditionalStoresFromCRLDistributionPoint(cRLDistPoint, paramPKIXExtendedParameters.getNamedCRLStoreMap(), paramDate2, paramJcaJceHelper));
        } catch (AnnotatedException annotatedException1) {
        ...
        if (cRLDistPoint != null) {
          ...
          try {
            for (byte b = 0; b < arrayOfDistributionPoint.length && certStatus.getCertStatus() == 11 && !reasonsMask.isAllReasons(); b++) {
              ...
              checkCRL(arrayOfDistributionPoint[b], paramX509AttributeCertificate, pKIXExtendedParameters, paramDate1, paramDate2, paramX509Certificate, certStatus, reasonsMask, paramList, paramJcaJceHelper);
              bool = true;
            } 
          } catch (AnnotatedException annotatedException1) {
        ...
        if (certStatus.getCertStatus() == 11 && !reasonsMask.isAllReasons())
          try {
            X500Name x500Name;
            try {
              x500Name = PrincipalUtils.getEncodedIssuerPrincipal(paramX509AttributeCertificate);
            } catch (Exception exception) {
            ...
            PKIXExtendedParameters pKIXExtendedParameters = (PKIXExtendedParameters)paramPKIXExtendedParameters.clone();
            checkCRL(distributionPoint, paramX509AttributeCertificate, pKIXExtendedParameters, paramDate1, paramDate2, paramX509Certificate, certStatus, reasonsMask, paramList, paramJcaJceHelper);
            bool = true;
          } catch (AnnotatedException annotatedException1) {
            annotatedException = new AnnotatedException("No valid CRL for distribution point found.", annotatedException1);
          }  
  ...
  protected static void processAttrCert5(X509AttributeCertificate paramX509AttributeCertificate, Date paramDate) throws CertPathValidatorException {
    try {
      paramX509AttributeCertificate.checkValidity(paramDate);
  ...
  protected static void processAttrCert3(X509Certificate paramX509Certificate, PKIXExtendedParameters paramPKIXExtendedParameters) throws CertPathValidatorException {
    ...
    if (arrayOfBoolean != null && (arrayOfBoolean.length <= 0 || !arrayOfBoolean[0]) && (arrayOfBoolean.length <= 1 || !arrayOfBoolean[1]))
    ...
    if (paramX509AttributeCertificate.getHolder().getEntityNames() != null) {
      ...
      for (byte b = 0; b < arrayOfPrincipal.length; b++) {
        try {
          ...
          PKIXCertStoreSelector pKIXCertStoreSelector = (new PKIXCertStoreSelector.Builder((CertSelector)x509CertStoreSelector)).build();
          CertPathValidatorUtilities.findCertificates(linkedHashSet, pKIXCertStoreSelector, paramPKIXExtendedParameters.getCertStores());
        } catch (AnnotatedException annotatedException) {
          throw new ExtCertPathValidatorException("Public key certificate for attribute certificate cannot be searched.", annotatedException);
        } catch (IOException iOException) {
  ...
  private static void checkCRL(DistributionPoint paramDistributionPoint, X509AttributeCertificate paramX509AttributeCertificate, PKIXExtendedParameters paramPKIXExtendedParameters, Date paramDate1, Date paramDate2, X509Certificate paramX509Certificate, CertStatus paramCertStatus, ReasonsMask paramReasonsMask, List paramList, JcaJceHelper paramJcaJceHelper) throws AnnotatedException, RecoverableCertPathValidatorException {
    ...
    PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters = new PKIXCertRevocationCheckerParameters(paramPKIXExtendedParameters, paramDate2, null, -1, paramX509Certificate, null);
    Set set = CertPathValidatorUtilities.getCompleteCRLs(pKIXCertRevocationCheckerParameters, paramDistributionPoint, paramX509AttributeCertificate, paramPKIXExtendedParameters, paramDate2);
    boolean bool = false;
    AnnotatedException annotatedException = null;
    ...
    while (iterator.hasNext() && paramCertStatus.getCertStatus() == 11 && !paramReasonsMask.isAllReasons()) {
      try {
        ...
        if (paramPKIXExtendedParameters.isUseDeltasEnabled()) {
          Set set2 = CertPathValidatorUtilities.getDeltaCRLs(paramDate1, x509CRL1, paramPKIXExtendedParameters.getCertStores(), paramPKIXExtendedParameters.getCRLStores(), paramJcaJceHelper);
        ...
        RFC3280CertPathUtilities.processCRLI(paramDate2, x509CRL2, paramX509AttributeCertificate, paramCertStatus, paramPKIXExtendedParameters);
        RFC3280CertPathUtilities.processCRLJ(paramDate2, x509CRL1, paramX509AttributeCertificate, paramCertStatus);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\jce\provider\RFC3281CertPathUtilities.class
