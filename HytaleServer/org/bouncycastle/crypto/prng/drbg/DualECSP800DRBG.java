...
public class DualECSP800DRBG implements SP80090DRBG {
  ...
  private static final DualECPoints[] nistPoints = new DualECPoints[] { createDualECPoints("P-256", 128, p256_Px, p256_Py, p256_Qx, p256_Qy, 1), createDualECPoints("P-384", 192, p384_Px, p384_Py, p384_Qx, p384_Qy, 1), createDualECPoints("P-521", 256, p521_Px, p521_Py, p521_Qx, p521_Qy, 1) };
  ...
  private static DualECPoints createDualECPoints(String paramString, int paramInt1, BigInteger paramBigInteger1, BigInteger paramBigInteger2, BigInteger paramBigInteger3, BigInteger paramBigInteger4, int paramInt2) {
    ECCurve.AbstractFp abstractFp = (ECCurve.AbstractFp)CustomNamedCurves.getByNameLazy(paramString).getCurve();
    return new DualECPoints(paramInt1, abstractFp.createPoint(paramBigInteger1, paramBigInteger2), abstractFp.createPoint(paramBigInteger3, paramBigInteger4), paramInt2);
  }
  ...
  public int generate(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2, boolean paramBoolean) {
    ...
    if (k < paramArrayOfbyte1.length) {
      ...
      if (arrayOfByte.length >= this._outlen) {
        ...
        int n = this._outlen - arrayOfByte.length;
        if (n < m)
          System.arraycopy(arrayOfByte, 0, paramArrayOfbyte1, k + n, m - n); 
      } 
      this._reseedCounter++;
    } 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\prng\drbg\DualECSP800DRBG.class
