...
public class CCMBlockCipher implements CCMModeCipher {
  ...
  public static CCMModeCipher newInstance(BlockCipher paramBlockCipher) {
    return new CCMBlockCipher(paramBlockCipher);
  }
  ...
  public void init(boolean paramBoolean, CipherParameters paramCipherParameters) throws IllegalArgumentException {
    ...
    if (paramCipherParameters instanceof AEADParameters) {
      ...
      this.macSize = getMacSize(paramBoolean, aEADParameters.getMacSize());
      ...
      this.macSize = getMacSize(paramBoolean, 64);
  ...
  public int processPacket(byte[] paramArrayOfbyte1, int paramInt1, int paramInt2, byte[] paramArrayOfbyte2, int paramInt3) throws IllegalStateException, InvalidCipherTextException, DataLengthException {
    ...
    cTRModeCipher.init(this.forEncryption, (CipherParameters)new ParametersWithIV(this.keyParam, arrayOfByte));
    int m = paramInt1;
    int n = paramInt3;
    if (this.forEncryption) {
      ...
      cTRModeCipher.processBlock(this.macBlock, 0, arrayOfByte1, 0);
      while (m < paramInt1 + paramInt2 - this.blockSize) {
        cTRModeCipher.processBlock(paramArrayOfbyte1, m, paramArrayOfbyte2, n);
        n += this.blockSize;
        m += this.blockSize;
      } 
      byte[] arrayOfByte2 = new byte[this.blockSize];
      System.arraycopy(paramArrayOfbyte1, m, arrayOfByte2, 0, paramInt2 + paramInt1 - m);
      cTRModeCipher.processBlock(arrayOfByte2, 0, arrayOfByte2, 0);
      ...
      cTRModeCipher.processBlock(arrayOfByte1, 0, arrayOfByte1, 0);
  ...
  private int getMacSize(boolean paramBoolean, int paramInt) {
    if (paramBoolean && (paramInt < 32 || paramInt > 128 || 0 != (paramInt & 0xF)))
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\modes\CCMBlockCipher.class
