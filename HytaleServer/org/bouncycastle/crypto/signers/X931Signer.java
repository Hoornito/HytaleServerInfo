...
public class X931Signer implements Signer {
  ...
  public byte[] generateSignature() throws CryptoException {
    ...
    int i = BigIntegers.getUnsignedByteLength(this.kParam.getModulus());
    return BigIntegers.asUnsignedByteArray(i, bigInteger);
  }
  
  private void createSignatureBlock(int paramInt) {
    int j;
    ...
    if (paramInt == 188) {
      ...
      this.digest.doFinal(this.block, j);
      this.block[this.block.length - 2] = (byte)(paramInt >>> 8);
      this.block[this.block.length - 1] = (byte)paramInt;
    } 
    this.block[0] = 107;
    for (int k = j - 2; k != 0; k--)
      this.block[k] = -69; 
  ...
  public boolean verifySignature(byte[] paramArrayOfbyte) {
    ...
    if (this.trailer == 15052 && !bool) {
      this.block[this.block.length - 2] = 64;
      bool = Arrays.constantTimeAreEqual(this.block, arrayOfByte);
    } 
    clearBlock(this.block);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\signers\X931Signer.class
