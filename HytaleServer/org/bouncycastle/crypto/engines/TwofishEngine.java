...
public final class TwofishEngine implements BlockCipher {
  ...
  public void init(boolean paramBoolean, CipherParameters paramCipherParameters) {
    if (paramCipherParameters instanceof KeyParameter) {
      ...
      int i = this.workingKey.length * 8;
      switch (i) {
        case 128:
        case 192:
        case 256:
      ...
      CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), i, paramCipherParameters, Utils.getPurpose(paramBoolean)));
  ...
  private void setKey(byte[] paramArrayOfbyte) {
    ...
    for (i = 0; i < this.k64Cnt; i++) {
      int i1 = i * 8;
      arrayOfInt1[i] = Pack.littleEndianToInt(paramArrayOfbyte, i1);
      arrayOfInt2[i] = Pack.littleEndianToInt(paramArrayOfbyte, i1 + 4);
  ...
  private void encryptBlock(byte[] paramArrayOfbyte1, int paramInt1, byte[] paramArrayOfbyte2, int paramInt2) {
    int i = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1) ^ this.gSubKeys[0];
    int j = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 4) ^ this.gSubKeys[1];
    int k = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 8) ^ this.gSubKeys[2];
    int m = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 12) ^ this.gSubKeys[3];
    byte b1 = 8;
    for (byte b2 = 0; b2 < 16; b2 += 2) {
      int n = Fe32_0(i);
      int i1 = Fe32_3(j);
      k ^= n + i1 + this.gSubKeys[b1++];
      k = Integers.rotateRight(k, 1);
      m = Integers.rotateLeft(m, 1) ^ n + 2 * i1 + this.gSubKeys[b1++];
      n = Fe32_0(k);
      i1 = Fe32_3(m);
      i ^= n + i1 + this.gSubKeys[b1++];
      i = Integers.rotateRight(i, 1);
      j = Integers.rotateLeft(j, 1) ^ n + 2 * i1 + this.gSubKeys[b1++];
    } 
    Pack.intToLittleEndian(k ^ this.gSubKeys[4], paramArrayOfbyte2, paramInt2);
    Pack.intToLittleEndian(m ^ this.gSubKeys[5], paramArrayOfbyte2, paramInt2 + 4);
    Pack.intToLittleEndian(i ^ this.gSubKeys[6], paramArrayOfbyte2, paramInt2 + 8);
    Pack.intToLittleEndian(j ^ this.gSubKeys[7], paramArrayOfbyte2, paramInt2 + 12);
  }
  
  private void decryptBlock(byte[] paramArrayOfbyte1, int paramInt1, byte[] paramArrayOfbyte2, int paramInt2) {
    int i = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1) ^ this.gSubKeys[4];
    int j = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 4) ^ this.gSubKeys[5];
    int k = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 8) ^ this.gSubKeys[6];
    int m = Pack.littleEndianToInt(paramArrayOfbyte1, paramInt1 + 12) ^ this.gSubKeys[7];
    byte b1 = 39;
    for (byte b2 = 0; b2 < 16; b2 += 2) {
      int n = Fe32_0(i);
      int i1 = Fe32_3(j);
      m ^= n + 2 * i1 + this.gSubKeys[b1--];
      k = Integers.rotateLeft(k, 1) ^ n + i1 + this.gSubKeys[b1--];
      m = Integers.rotateRight(m, 1);
      n = Fe32_0(k);
      i1 = Fe32_3(m);
      j ^= n + 2 * i1 + this.gSubKeys[b1--];
      i = Integers.rotateLeft(i, 1) ^ n + i1 + this.gSubKeys[b1--];
      j = Integers.rotateRight(j, 1);
    } 
    Pack.intToLittleEndian(k ^ this.gSubKeys[0], paramArrayOfbyte2, paramInt2);
    Pack.intToLittleEndian(m ^ this.gSubKeys[1], paramArrayOfbyte2, paramInt2 + 4);
    Pack.intToLittleEndian(i ^ this.gSubKeys[2], paramArrayOfbyte2, paramInt2 + 8);
    Pack.intToLittleEndian(j ^ this.gSubKeys[3], paramArrayOfbyte2, paramInt2 + 12);
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\engines\TwofishEngine.class
