...
public class DESEngine extends DESBase implements BlockCipher {
  ...
  public DESEngine() {
    CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), 56));
  }
  
  public void init(boolean paramBoolean, CipherParameters paramCipherParameters) {
    if (paramCipherParameters instanceof KeyParameter) {
      if (((KeyParameter)paramCipherParameters).getKeyLength() > 8)
        throw new IllegalArgumentException("DES key too long - should be 8 bytes"); 
      this.forEncryption = paramBoolean;
      this.workingKey = generateWorkingKey(paramBoolean, ((KeyParameter)paramCipherParameters).getKey());
      CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), 56, paramCipherParameters, Utils.getPurpose(this.forEncryption)));
      return;
    } 
  ...
  protected void desFunc(int[] paramArrayOfint, byte[] paramArrayOfbyte1, int paramInt1, byte[] paramArrayOfbyte2, int paramInt2) {
    int k = Pack.bigEndianToInt(paramArrayOfbyte1, paramInt1);
    ...
    k ^= i;
    j ^= i << 4;
    Pack.intToBigEndian(j, paramArrayOfbyte2, paramInt2);
    Pack.intToBigEndian(k, paramArrayOfbyte2, paramInt2 + 4);
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\engines\DESEngine.class
