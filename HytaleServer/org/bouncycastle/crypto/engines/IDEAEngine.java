...
public class IDEAEngine implements BlockCipher {
  ...
  public IDEAEngine() {
    CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), 128));
  }
  
  public void init(boolean paramBoolean, CipherParameters paramCipherParameters) {
    if (paramCipherParameters instanceof KeyParameter) {
      byte[] arrayOfByte = ((KeyParameter)paramCipherParameters).getKey();
      this.workingKey = generateWorkingKey(paramBoolean, arrayOfByte);
      this.forEncryption = paramBoolean;
      CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), arrayOfByte.length * 8, paramCipherParameters, Utils.getPurpose(paramBoolean)));
      return;
    } 
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\engines\IDEAEngine.class
