...
public class Grainv1Engine implements StreamCipher {
  ...
  public void init(boolean paramBoolean, CipherParameters paramCipherParameters) throws IllegalArgumentException {
    ...
    this.workingIV = new byte[arrayOfByte2.length];
    this.workingKey = new byte[arrayOfByte2.length];
    this.lfsr = new int[5];
    this.nfsr = new int[5];
    this.out = new byte[2];
    ...
    CryptoServicesRegistrar.checkConstraints((CryptoServiceProperties)new DefaultServiceProperties(getAlgorithmName(), 80, paramCipherParameters, Utils.getPurpose(paramBoolean)));
    reset();
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\engines\Grainv1Engine.class
