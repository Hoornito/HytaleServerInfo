...
class HKDF {
  ...
  protected byte[] LabeledExtract(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2, String paramString, byte[] paramArrayOfbyte3) {
    ...
    byte[] arrayOfByte = Arrays.concatenate(VERSION_LABEL, paramArrayOfbyte2, getBytes(paramString), paramArrayOfbyte3);
  ...
  protected byte[] LabeledExpand(byte[] paramArrayOfbyte1, byte[] paramArrayOfbyte2, String paramString, byte[] paramArrayOfbyte3, int paramInt) {
    ...
    byte[] arrayOfByte1 = Arrays.concatenate(Pack.shortToBigEndian((short)paramInt), VERSION_LABEL, paramArrayOfbyte2, getBytes(paramString));
  ...
  private static byte[] getBytes(String paramString) {
    return Strings.toByteArray(paramString);
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\crypto\hpke\HKDF.class
