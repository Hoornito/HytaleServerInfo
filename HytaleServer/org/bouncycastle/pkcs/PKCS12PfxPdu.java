  ...
  public boolean isMacValid(PKCS12MacCalculatorBuilderProvider paramPKCS12MacCalculatorBuilderProvider, char[] paramArrayOfchar) throws PKCSException {
    if (hasMac()) {
      ...
      if (PKCSObjectIdentifiers.id_PBMAC1.equals((ASN1Primitive)macData.getMac().getAlgorithmId().getAlgorithm())) {
        ...
        macDataGenerator = new MacDataGenerator(paramPKCS12MacCalculatorBuilderProvider.get(new AlgorithmIdentifier(macData.getMac().getAlgorithmId().getAlgorithm(), (ASN1Encodable)pBMAC1Params)));
      } else {
        macDataGenerator = new MacDataGenerator(paramPKCS12MacCalculatorBuilderProvider.get(new AlgorithmIdentifier(macData.getMac().getAlgorithmId().getAlgorithm(), (ASN1Encodable)new PKCS12PBEParams(macData.getSalt(), macData.getIterationCount().intValue()))));
      } 
      try {
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\pkcs\PKCS12PfxPdu.class
