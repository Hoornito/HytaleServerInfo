...
public class CMSAuthenticatedData implements Encodable {
  ...
  public CMSAuthenticatedData(ContentInfo paramContentInfo, DigestCalculatorProvider paramDigestCalculatorProvider) throws CMSException {
    ...
    CMSProcessableByteArray cMSProcessableByteArray = new CMSProcessableByteArray(contentInfo.getContentType(), ASN1OctetString.getInstance(contentInfo.getContent()).getOctets());
    if (this.authAttrs != null) {
      ...
      try {
        CMSEnvelopedHelper.CMSDigestAuthenticatedSecureReadable cMSDigestAuthenticatedSecureReadable = new CMSEnvelopedHelper.CMSDigestAuthenticatedSecureReadable(paramDigestCalculatorProvider.get(authenticatedData.getDigestAlgorithm()), contentInfo.getContentType(), cMSProcessableByteArray);
      ...
      CMSEnvelopedHelper.CMSAuthEnveSecureReadable cMSAuthEnveSecureReadable = new CMSEnvelopedHelper.CMSAuthEnveSecureReadable(this.macAlg, contentInfo.getContentType(), cMSProcessableByteArray);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\cms\CMSAuthenticatedData.class
