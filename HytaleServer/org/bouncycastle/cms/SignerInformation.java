...
public class SignerInformation {
  ...
  protected SignerInformation(SignerInformation paramSignerInformation) {
    this(paramSignerInformation, paramSignerInformation.info);
  }
  
  protected SignerInformation(SignerInformation paramSignerInformation, SignerInfo paramSignerInfo) {
    this.info = paramSignerInfo;
  ...
  private boolean doVerify(SignerInformationVerifier paramSignerInformationVerifier) throws CMSException {
    ...
    AlgorithmIdentifier algorithmIdentifier = (this.signedAttributeSet != null) ? this.info.getDigestAlgorithm() : translateBrokenRSAPkcs7(this.encryptionAlgorithm, this.info.getDigestAlgorithm());
    try {
      contentVerifier = paramSignerInformationVerifier.getContentVerifier(this.encryptionAlgorithm, algorithmIdentifier);
    ...
    try {
      ...
      if (this.resultDigest == null) {
        DigestCalculator digestCalculator = paramSignerInformationVerifier.getDigestCalculator(algorithmIdentifier);
    ...
    AttributeTable attributeTable = getSignedAttributes();
    verifyAlgorithmIdentifierProtectionAttribute(attributeTable);
    verifyMessageDigestAttribute();
    verifyCounterSignatureAttribute(attributeTable);
    try {
      if (this.signedAttributeSet == null && this.resultDigest != null && contentVerifier instanceof RawContentVerifier) {
        ...
        if (str.equals("RSA")) {
          DigestInfo digestInfo = new DigestInfo(new AlgorithmIdentifier(algorithmIdentifier.getAlgorithm(), (ASN1Encodable)DERNull.INSTANCE), this.resultDigest);
  ...
  private void verifyContentTypeAttributeValue() throws CMSException {
    ASN1Primitive aSN1Primitive = getSingleValuedSignedAttribute(CMSAttributes.contentType, "content-type");
  ...
  private void verifyMessageDigestAttribute() throws CMSException {
    ASN1Primitive aSN1Primitive = getSingleValuedSignedAttribute(CMSAttributes.messageDigest, "message-digest");
    if (aSN1Primitive == null) {
      if (this.signedAttributeSet != null)
  ...
  private void verifyAlgorithmIdentifierProtectionAttribute(AttributeTable paramAttributeTable) throws CMSException {
    AttributeTable attributeTable = getUnsignedAttributes();
    if (attributeTable != null && attributeTable.getAll(CMSAttributes.cmsAlgorithmProtect).size() > 0)
      throw new CMSException("A cmsAlgorithmProtect attribute MUST be a signed attribute"); 
    if (paramAttributeTable != null) {
      ASN1EncodableVector aSN1EncodableVector = paramAttributeTable.getAll(CMSAttributes.cmsAlgorithmProtect);
  ...
  private void verifyCounterSignatureAttribute(AttributeTable paramAttributeTable) throws CMSException {
    if (paramAttributeTable != null && paramAttributeTable.getAll(CMSAttributes.counterSignature).size() > 0)
  ...
  private static AlgorithmIdentifier translateBrokenRSAPkcs7(AlgorithmIdentifier paramAlgorithmIdentifier1, AlgorithmIdentifier paramAlgorithmIdentifier2) {
    return (PKCSObjectIdentifiers.rsaEncryption.equals((ASN1Primitive)paramAlgorithmIdentifier1.getAlgorithm()) && (OIWObjectIdentifiers.sha1WithRSA.equals((ASN1Primitive)paramAlgorithmIdentifier2.getAlgorithm()) || PKCSObjectIdentifiers.sha1WithRSAEncryption.equals((ASN1Primitive)paramAlgorithmIdentifier2.getAlgorithm()))) ? new AlgorithmIdentifier(OIWObjectIdentifiers.idSHA1, (ASN1Encodable)DERNull.INSTANCE) : paramAlgorithmIdentifier2;
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\cms\SignerInformation.class
