...
public abstract class JceKeyAgreeRecipient implements KeyAgreeRecipient {
  ...
  public JceKeyAgreeRecipient setUnwrappingProvider(Provider paramProvider) {
    this.unwrappingHelper = new EnvelopedDataHelper(new ProviderJcaJceExtHelper(paramProvider));
    return this;
  }
  
  public JceKeyAgreeRecipient setUnwrappingProvider(String paramString) {
    this.unwrappingHelper = new EnvelopedDataHelper(new NamedJcaJceExtHelper(paramString));
    return this;
  }
  ...
  public JceKeyAgreeRecipient setPrivateKeyAlgorithmIdentifier(AlgorithmIdentifier paramAlgorithmIdentifier) {
    this.privKeyAlgID = paramAlgorithmIdentifier;
    return this;
  }
  ...
  private SecretKey calculateAgreedWrapKey(AlgorithmIdentifier paramAlgorithmIdentifier1, AlgorithmIdentifier paramAlgorithmIdentifier2, PublicKey paramPublicKey, ASN1OctetString paramASN1OctetString, PrivateKey paramPrivateKey, KeyMaterialGenerator paramKeyMaterialGenerator) throws CMSException, GeneralSecurityException, IOException {
    ...
    if (CMSUtils.isMQV(paramAlgorithmIdentifier1.getAlgorithm())) {
      ...
      SubjectPublicKeyInfo subjectPublicKeyInfo = new SubjectPublicKeyInfo(getPrivateKeyAlgorithmIdentifier(), mQVuserKeyingMaterial.getEphemeralPublicKey().getPublicKeyData());
    ...
    if (CMSUtils.isEC(paramAlgorithmIdentifier1.getAlgorithm())) {
      ...
      if (paramASN1OctetString != null) {
        arrayOfByte = paramKeyMaterialGenerator.generateKDFMaterial(paramAlgorithmIdentifier2, this.keySizeProvider.getKeySize(paramAlgorithmIdentifier2), paramASN1OctetString.getOctets());
      } else {
        arrayOfByte = paramKeyMaterialGenerator.generateKDFMaterial(paramAlgorithmIdentifier2, this.keySizeProvider.getKeySize(paramAlgorithmIdentifier2), null);
      } 
  ...
  protected Key unwrapSessionKey(ASN1ObjectIdentifier paramASN1ObjectIdentifier1, SecretKey paramSecretKey, ASN1ObjectIdentifier paramASN1ObjectIdentifier2, byte[] paramArrayOfbyte) throws CMSException, InvalidKeyException, NoSuchAlgorithmException {
    Cipher cipher = this.unwrappingHelper.createCipher(paramASN1ObjectIdentifier1);
  ...
  protected Key extractSecretKey(AlgorithmIdentifier paramAlgorithmIdentifier1, AlgorithmIdentifier paramAlgorithmIdentifier2, SubjectPublicKeyInfo paramSubjectPublicKeyInfo, ASN1OctetString paramASN1OctetString, byte[] paramArrayOfbyte) throws CMSException {
    try {
      ...
      try {
        ...
        if (algorithmIdentifier.getAlgorithm().equals((ASN1Primitive)CryptoProObjectIdentifiers.id_Gost28147_89_None_KeyWrap) || algorithmIdentifier.getAlgorithm().equals((ASN1Primitive)CryptoProObjectIdentifiers.id_Gost28147_89_CryptoPro_KeyWrap)) {
        ...
        if (paramASN1OctetString != null)
          try {
            SecretKey secretKey = calculateAgreedWrapKey(paramAlgorithmIdentifier1, algorithmIdentifier, publicKey, paramASN1OctetString, this.recipientKey, simple_ecc_cmsGenerator);
            return unwrapSessionKey(algorithmIdentifier.getAlgorithm(), secretKey, paramAlgorithmIdentifier2.getAlgorithm(), paramArrayOfbyte);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\cms\jcajce\JceKeyAgreeRecipient.class
