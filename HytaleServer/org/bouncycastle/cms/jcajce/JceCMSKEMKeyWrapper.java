...
class JceCMSKEMKeyWrapper extends KEMKeyWrapper {
  private final KemEncapsulationLengthProvider kemEncLenProvider = (KemEncapsulationLengthProvider)new DefaultKemEncapsulationLengthProvider();
  ...
  private int getKemEncLength(PublicKey paramPublicKey) {
    return this.kemEncLenProvider.getEncapsulationLength(SubjectPublicKeyInfo.getInstance(paramPublicKey.getEncoded()).getAlgorithm());
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\cms\jcajce\JceCMSKEMKeyWrapper.class
