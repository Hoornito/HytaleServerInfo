...
public abstract class JceKeyTransRecipient implements KeyTransRecipient {
  ...
  protected Key extractSecretKey(AlgorithmIdentifier paramAlgorithmIdentifier1, AlgorithmIdentifier paramAlgorithmIdentifier2, byte[] paramArrayOfbyte) throws CMSException {
    ...
    if (CMSObjectIdentifiers.id_ori_kem.equals((ASN1Primitive)paramAlgorithmIdentifier1.getAlgorithm())) {
      KEMRecipientInfo kEMRecipientInfo = KEMRecipientInfo.getInstance(paramAlgorithmIdentifier1.getParameters());
      JceAsymmetricKeyUnwrapper jceAsymmetricKeyUnwrapper1 = this.helper.createAsymmetricUnwrapper(kEMRecipientInfo.getKem(), this.recipientKey).setMustProduceEncodableUnwrappedKey(this.unwrappedKeyMustBeEncodable);
      if (!this.extraMappings.isEmpty())
        for (ASN1ObjectIdentifier aSN1ObjectIdentifier : this.extraMappings.keySet())
          jceAsymmetricKeyUnwrapper1.setAlgorithmMapping(aSN1ObjectIdentifier, (String)this.extraMappings.get(aSN1ObjectIdentifier));  
      try {
        Key key = this.helper.getJceKey(paramAlgorithmIdentifier2, jceAsymmetricKeyUnwrapper1.generateUnwrappedKey(paramAlgorithmIdentifier2, paramArrayOfbyte));
    ...
    try {
      Key key = this.helper.getJceKey(paramAlgorithmIdentifier2, jceAsymmetricKeyUnwrapper.generateUnwrappedKey(paramAlgorithmIdentifier2, paramArrayOfbyte));
      if (this.validateKeySize)
        if (paramArrayOfbyte.equals(CMSObjectIdentifiers.id_alg_cek_hkdf_sha256)) {
          this.helper.keySizeCheck(AlgorithmIdentifier.getInstance(paramAlgorithmIdentifier2.getParameters()), key);
        } else {
          this.helper.keySizeCheck(paramAlgorithmIdentifier2, key);
        }  
      return key;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\cms\jcajce\JceKeyTransRecipient.class
