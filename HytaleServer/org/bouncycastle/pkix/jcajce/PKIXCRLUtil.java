...
abstract class PKIXCRLUtil {
  ...
  static Set findCRLs(PKIXCRLStoreSelector paramPKIXCRLStoreSelector, Date paramDate, List paramList1, List paramList2) throws AnnotatedException {
    ...
    for (X509CRL x509CRL1 : hashSet) {
      X509CRL x509CRL2 = x509CRL1;
      Date date = x509CRL2.getNextUpdate();
      if (date == null || date.after(paramDate)) {
        X509Certificate x509Certificate = paramPKIXCRLStoreSelector.getCertificateChecking();
        if (null == x509Certificate || x509CRL2.getThisUpdate().before(x509Certificate.getNotAfter()))
          hashSet1.add(x509CRL2); 
      } 
    } 
    return hashSet1;
  }
  
  private static void findCRLs(Set paramSet, PKIXCRLStoreSelector paramPKIXCRLStoreSelector, List paramList) throws AnnotatedException {
    ...
    for (Store store : paramList) {
      if (store instanceof Store) {
        Store store1 = store;
        try {
          paramSet.addAll(store1.getMatches((Selector)paramPKIXCRLStoreSelector));
          bool = true;
      ...
      try {
        paramSet.addAll(PKIXCRLStoreSelector.getCRLs(paramPKIXCRLStoreSelector, certStore));
        bool = true;
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\pkix\jcajce\PKIXCRLUtil.class
