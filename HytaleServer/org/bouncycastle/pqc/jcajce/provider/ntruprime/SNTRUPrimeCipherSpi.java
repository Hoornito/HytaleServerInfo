...
class SNTRUPrimeCipherSpi extends CipherSpi {
  ...
  SNTRUPrimeCipherSpi(String paramString) throws NoSuchAlgorithmException {
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (paramInt == 3) {
      if (paramKey instanceof BCSNTRUPrimePublicKey) {
        this.wrapKey = (BCSNTRUPrimePublicKey)paramKey;
        this.kemGen = new SNTRUPrimeKEMGenerator(CryptoServicesRegistrar.getSecureRandom(paramSecureRandom));
      } else {
        throw new InvalidKeyException("Only a " + this.algorithmName + " public key can be used for wrapping");
      } 
    } else if (paramInt == 4) {
      if (paramKey instanceof BCSNTRUPrimePrivateKey) {
        this.unwrapKey = (BCSNTRUPrimePrivateKey)paramKey;
      } else {
  ...
  protected Key engineUnwrap(byte[] paramArrayOfbyte, String paramString, int paramInt) throws InvalidKeyException, NoSuchAlgorithmException {
    ...
    try {
      ...
      byte[] arrayOfByte1 = sNTRUPrimeKEMExtractor.extractSecret(Arrays.copyOfRange(paramArrayOfbyte, 0, sNTRUPrimeKEMExtractor.getEncapsulationLength()));
      Wrapper wrapper = WrapUtil.getWrapper(this.kemParameterSpec.getKeyAlgorithmName());
      ...
      byte[] arrayOfByte2 = Arrays.copyOfRange(paramArrayOfbyte, sNTRUPrimeKEMExtractor.getEncapsulationLength(), paramArrayOfbyte.length);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\pqc\jcajce\provider\ntruprime\SNTRUPrimeCipherSpi.class
