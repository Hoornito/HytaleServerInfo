...
class FrodoCipherSpi extends CipherSpi {
  ...
  FrodoCipherSpi(String paramString) throws NoSuchAlgorithmException {
  ...
  protected void engineInit(int paramInt, Key paramKey, AlgorithmParameterSpec paramAlgorithmParameterSpec, SecureRandom paramSecureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
    ...
    if (paramInt == 3) {
      if (paramKey instanceof BCFrodoPublicKey) {
        this.wrapKey = (BCFrodoPublicKey)paramKey;
        this.kemGen = new FrodoKEMGenerator(CryptoServicesRegistrar.getSecureRandom(paramSecureRandom));
      } else {
        throw new InvalidKeyException("Only a " + this.algorithmName + " public key can be used for wrapping");
      } 
    } else if (paramInt == 4) {
      if (paramKey instanceof BCFrodoPrivateKey) {
        this.unwrapKey = (BCFrodoPrivateKey)paramKey;
      } else {
  ...
  protected Key engineUnwrap(byte[] paramArrayOfbyte, String paramString, int paramInt) throws InvalidKeyException, NoSuchAlgorithmException {
    ...
    try {
      FrodoKEMExtractor frodoKEMExtractor = new FrodoKEMExtractor((FrodoKeyParameters)this.unwrapKey.getKeyParams());
      byte[] arrayOfByte1 = frodoKEMExtractor.extractSecret(Arrays.copyOfRange(paramArrayOfbyte, 0, frodoKEMExtractor.getEncapsulationLength()));
      Wrapper wrapper = WrapUtil.getWrapper(this.kemParameterSpec.getKeyAlgorithmName());
      ...
      byte[] arrayOfByte2 = Arrays.copyOfRange(paramArrayOfbyte, frodoKEMExtractor.getEncapsulationLength(), paramArrayOfbyte.length);
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\pqc\jcajce\provider\frodo\FrodoCipherSpi.class
