...
class NTT {
  private static final short[] BitReverseTable = new short[] { 
  ...
  static void bitReverse(short[] paramArrayOfshort) {
    for (byte b = 0; b < 'Ѐ'; b++) {
      short s = BitReverseTable[b];
      if (b < s) {
        short s1 = paramArrayOfshort[b];
        paramArrayOfshort[b] = paramArrayOfshort[s];
        paramArrayOfshort[s] = s1;
      } 
    } 
  }
  
  static void core(short[] paramArrayOfshort1, short[] paramArrayOfshort2) {
    for (byte b = 0; b < 10; b += 2) {
      int i = 1 << b;
      byte b1;
      for (b1 = 0; b1 < i; b1++) {
        byte b2 = 0;
        int j;
        for (j = b1; j < 1023; j += 2 * i) {
          int k = paramArrayOfshort1[j] & 0xFFFF;
          int m = paramArrayOfshort1[j + i] & 0xFFFF;
          short s = paramArrayOfshort2[b2++];
          paramArrayOfshort1[j] = (short)(k + m);
          paramArrayOfshort1[j + i] = Reduce.montgomery(s * (k + 36867 - m));
        } 
      } 
      i <<= 1;
      for (b1 = 0; b1 < i; b1++) {
        ...
        for (j = b1; j < 1023; j += 2 * i) {
          ...
          int m = paramArrayOfshort1[j + i] & 0xFFFF;
          short s = paramArrayOfshort2[b2++];
          paramArrayOfshort1[j] = Reduce.barrett((short)(k + m));
          paramArrayOfshort1[j + i] = Reduce.montgomery(s * (k + 36867 - m));
        } 
      } 
    } 
  ...
  static void mulCoefficients(short[] paramArrayOfshort1, short[] paramArrayOfshort2) {
    for (byte b = 0; b < 'Ѐ'; b++) {
      int i = paramArrayOfshort1[b] & 0xFFFF;
      int j = paramArrayOfshort2[b] & 0xFFFF;
      paramArrayOfshort1[b] = Reduce.montgomery(i * j);
    } 
  }
}
