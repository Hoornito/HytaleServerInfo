...
class OperatorHelper {
  ...
  Cipher createAsymmetricWrapper(AlgorithmIdentifier paramAlgorithmIdentifier, Map paramMap) throws OperatorCreationException {
    ...
    try {
      ...
      if (paramMap != null && !paramMap.isEmpty())
        str = (String)paramMap.get(aSN1ObjectIdentifier); 
      if (str == null)
        str = (String)asymmetricWrapperAlgNames.get(aSN1ObjectIdentifier); 
      if (str != null) {
        if (str.indexOf("OAEPPadding") > 0)
          try {
            RSAESOAEPparams rSAESOAEPparams = RSAESOAEPparams.getInstance(paramAlgorithmIdentifier.getParameters());
            if (rSAESOAEPparams != null) {
              ASN1ObjectIdentifier aSN1ObjectIdentifier1 = rSAESOAEPparams.getHashAlgorithm().getAlgorithm();
              OAEPParamsValue oAEPParamsValue = (OAEPParamsValue)oaepParamsMap.get(aSN1ObjectIdentifier1);
              if (oAEPParamsValue != null && oAEPParamsValue.matches(rSAESOAEPparams.withDefaultPSource()))
                str = oAEPParamsValue.getCipherName(); 
            } 
        ...
        try {
          ...
          } else if (str.indexOf("ECB/OAEPWith") > 0) {
            int i = str.indexOf("ECB");
            try {
              return this.helper.createCipher(str.substring(0, i) + "NONE" + str.substring(i + 3));
            } catch (NoSuchAlgorithmException noSuchAlgorithmException1) {}
          } 
        } 
      } 
  ...
  private static class OAEPParamsValue {
    ...
    static void add(Map<ASN1ObjectIdentifier, OAEPParamsValue> param1Map, String param1String, ASN1ObjectIdentifier param1ASN1ObjectIdentifier) {
      try {
        RSAESOAEPparams rSAESOAEPparams = createOAEPParams(param1ASN1ObjectIdentifier);
        byte[] arrayOfByte = getDEREncoding(rSAESOAEPparams);
        param1Map.put(param1ASN1ObjectIdentifier, new OAEPParamsValue(param1String, arrayOfByte));
      } catch (Exception exception) {
        throw new RuntimeException(exception);
      } 
    }
    
    private OAEPParamsValue(String param1String, byte[] param1ArrayOfbyte) {
      this.cipherName = param1String;
      this.derEncoding = param1ArrayOfbyte;
    }
    ...
    boolean matches(RSAESOAEPparams param1RSAESOAEPparams) throws IOException {
      return Arrays.areEqual(this.derEncoding, getDEREncoding(param1RSAESOAEPparams));
    }
    
    private static RSAESOAEPparams createOAEPParams(ASN1ObjectIdentifier param1ASN1ObjectIdentifier) {
      AlgorithmIdentifier algorithmIdentifier1 = new AlgorithmIdentifier(param1ASN1ObjectIdentifier, (ASN1Encodable)DERNull.INSTANCE);
      AlgorithmIdentifier algorithmIdentifier2 = new AlgorithmIdentifier(PKCSObjectIdentifiers.id_mgf1, (ASN1Encodable)algorithmIdentifier1);
      return new RSAESOAEPparams(algorithmIdentifier1, algorithmIdentifier2, RSAESOAEPparams.DEFAULT_P_SOURCE_ALGORITHM);
    }
    
    private static byte[] getDEREncoding(RSAESOAEPparams param1RSAESOAEPparams) throws IOException {
      return param1RSAESOAEPparams.getEncoded("DER");
    }
  }
...
/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\operator\jcajce\OperatorHelper.class
