...
public class JceAsymmetricKeyUnwrapper extends AsymmetricKeyUnwrapper {
  ...
  public GenericKey generateUnwrappedKey(AlgorithmIdentifier paramAlgorithmIdentifier, byte[] paramArrayOfbyte) throws OperatorException {
    try {
      Key key = null;
      Cipher cipher = this.helper.createAsymmetricWrapper(getAlgorithmIdentifier(), this.extraMappings);
      AlgorithmParameters algorithmParameters = this.helper.createAlgorithmParameters(getAlgorithmIdentifier());
      try {
        if (algorithmParameters != null && !getAlgorithmIdentifier().getAlgorithm().equals((ASN1Primitive)OIWObjectIdentifiers.elGamalAlgorithm)) {
      ...
      if (key == null) {
        if (algorithmParameters != null) {
          cipher.init(2, this.privKey, algorithmParameters);
        } else {
      ...
      throw new OperatorException("invalid algorithm parameters: " + invalidAlgorithmParameterException.getMessage(), invalidAlgorithmParameterException);
    } 
  }
}


/* Location:              C:\Users\Agustin\AppData\Roaming\Hytale\install\release\package\game\latest\Server\HytaleServer.jar!\org\bouncycastle\operator\jcajce\JceAsymmetricKeyUnwrapper.class
